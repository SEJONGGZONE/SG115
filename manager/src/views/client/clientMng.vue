 ...
 * 전역변수 관리
 */
const CUR_CLIENT_OBJ = ref({}) // 선택된 거래처객체
...
function initMap() {
    ...
    naver.maps.Event.addListener(mapInstance.value, "click", function (e) {
        ...
        if (CUR_CLIENT_OBJ.value != null) {
            // 기준점 마커 표시 세팅..
            rootPositionMarkerSet(
                new naver.maps.LatLng(CUR_POSITION_OBJ.value.latitude, CUR_POSITION_OBJ.value.longitude)
            );
            // 상세데이타 반영
            selectRowData.value.LAT = CUR_POSITION_OBJ.value.latitude;
            selectRowData.value.LON = CUR_POSITION_OBJ.value.longitude;
        }
      ...
      icon: {
          content: [
                      ...
                      '<div class="rootPosition">',
                        '<span class="rootPositionTitle">' + CUR_CLIENT_OBJ.value.CLNAME + '</span>',
...
const resizeScreenSet = () => {
  ...
  setTimeout(() => {
    ...
    var grid_clientmnglist = document.getElementsByClassName("ag-theme-alpine")[0];
    grid_clientmnglist.style.height = calcHeight + "px";
...
const requestErpClientSel = async () => {
  ...
  let reqData = {
      '@I_COMPANYCD': "00002",
      '@I_DEVICENO': "",
      '@I_BOUND_DIST': CUR_POSITION_OBJ.boundDist ?? "",
  ...
  try {
    ...
    if (data.RecordCount > 0) {
      ...
      if (infos.length > 0) {
        ...
        var sHtmlString = "거래처관리 - <small><small>" + infos.length + "건</small></small>";
      ...
      // // 줌레벨 판단
      // var distance = CUR_POSITION_OBJ.boundDist*1;
      // switch (distance) {
...
const doSave = async () => {
  ...
  // 파라미터세팅
  let reqData = {
      '@I_CLCODE': selectRowData.value.CLCODE,
      '@I_LAT': CUR_POSITION_OBJ.value.latitude ?? "",
      '@I_LON': CUR_POSITION_OBJ.value.longitude ?? "",
      '@I_ADDRESS': "",
      '@I_INPUT_USER': "",
  }
...
const rollbackPosition = async () => {
    // 원래위치로 복원
    CUR_CLIENT_OBJ.value.LAT = ORG_POSITION_OBJ.value.LAT;
    CUR_CLIENT_OBJ.value.LON = ORG_POSITION_OBJ.value.LON;
    // 거래처위치로 지도이동/줌 
    moveClientPosition(CUR_CLIENT_OBJ.value);  
};
...
const cellWasClicked = (event) => {
  ...
  onmap_clientdetail_area.style.display = "flex";
    ...
    detailShow.value = false;
    // 전역변수저장
    CUR_CLIENT_OBJ.value = event.data;
    ORG_POSITION_OBJ.value.LAT = CUR_CLIENT_OBJ.value.LAT;
    ORG_POSITION_OBJ.value.LON = CUR_CLIENT_OBJ.value.LON;

    setTimeout(() => { // 애니메이션효과 추가로 지연시간이 필요함..(2023.10.16)
        // 상세데이타 세팅
        selectRowData.value = CUR_CLIENT_OBJ.value;
        // 상세영역 열기
        detailShow.value = true;
        // 거래처 이미지 조회
        requestErpClientImage("", CUR_CLIENT_OBJ.value.CLCODE);
    }, 100);  
    // 거래처위치로 지도이동/줌 
    moveClientPosition(CUR_CLIENT_OBJ.value);
};
...
const optionBoundChange = () => {
  ...
  CUR_POSITION_OBJ.boundDist = event.target.value; // 반경거리 세팅하기
  //console.log(CUR_POSITION_OBJ.boundDist);
  ...
  <div class="section__management_ilove">
    <div class="group__contents_ilove">
      ...
      <div class="grid_clientmnglist">
        <!-- 타이틀 -->
        <div class="info_title" style="text-align: center;"><span id="mng_title">거래처 관리</span></div>
        <!-- 검색어 -->
                ...
                <div class="grid_data_detail" v-if="detailShow"
                    ...
                    <div class="client_detail_area" data-aos="flip-left" data-aos-delay="50" 
                        ...
                        <div class="client_detail_title">
                            <span>{{ selectRowData.CLNAME }} - </span>
                        ...
                        <!-- 거래처이미지 리스트 -->
                        <div class="client_image_area" data-aos="flip-right" data-aos-delay="100" style="padding-left:0.1rem;">
                        ...
                        <div v-if="clientImageList.length == 0" style="text-align:center; padding-top:0.5rem; color:#b1b0b0;">- 등록된 이미지가 없습니다. -</div>
                    </div>
                    <!-- 버튼영역 -->
                    <div class="item__buttons" style="margin:0.5rem 0rem 0.5rem 0rem; border:0px solid red;">
                        ...
                        <button class="btn_save" @click="saveBtn">
                            <i class="fa-regular fa-circle-check"></i><span>위치정보 저장</span>
                        </button>
