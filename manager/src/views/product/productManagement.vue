...
const doFavItemCnt = async () => {
  ...
  let data;
  try {
    data = await productApi.getFavItemCntList(param);
...
const selectImageList = async () => {
  ...
  let data;
  try {
    data = await productApi.selectImageList(param);
...
const excelFileDownload = () => {
  //엑셀 Btn
  excelDownload(table.rows, "productManagerMent");
};
  ...
  <div class="section section__management" style="">
    ...
    <div class="group__contents">
      <div class="part__data_list" style="height: 100%; padding-bottom: 0px">
        ...
        <!-- 더보기버튼 -->
        <div class="item__buttons" v-if="table.isShowMoreBtn" style="gap: 0px">
