...
const instance = axios.create({
  baseURL: '/api/DatabaseController/runProcedure',
  timeout: 50000,
  headers: {
...
const instanceCVOUtil = axios.create({
  baseURL: '/cvoapi/GzoneUtilController',
  timeout: 50000,
  headers: {
    'Content-Type': 'application/json',
    CodyApiKey: '1Bb6wI6DnVL8S8LQAbvAO+pSnKXCf7HwQ7GEn+FnrJk=',
...
export const utils = {
  ...
  addHoverClassToTr(mouseEvent) {
    ...
    //cursor를 포함한 hover
    mouseEvent.target.classList.add('hover')
    utils.addCursorClassToTr(mouseEvent)
  },
  removeHoverClassFromTr(mouseEvent) {
    //grid 마우스 호버 액션 제거
    //cursor를 포함한 hover
    mouseEvent.target.classList.remove('hover')
    utils.removeCursorClassFromTr(mouseEvent)
  },
  addHoverClassToTr1(mouseEvent) {
    //grid 마우스 호버 액션 추가(커서액션 제외)
    mouseEvent.target.classList.add('hover')
  ...
  getDateFromString(timeString) {
    ...
    // d.toLocaleString('euc-KR', { timeZone: 'Asia/Seoul' })
    return d
  },
  /**
   * 현재일시 리턴하기(YYYY_MM_DD_HH_MM_SS)
   */
  nowYYYY_MM_DD_HH_MM_SS() {
    const now = new Date()

    return utils.setDateFormat(now) + ' ' + utils.formatTime_HH_MM_SS(now)
  },
  /**
   * 년월일시분초, 문자없이 숫자만 리턴한다.
   * @param date 날짜값
   * @returns
   */
  getDtmFormatOnlyDigits(date: Date) {
    let returnDate
