            ...
            <div class="swipe_box" ID="js_swipe_board_ctg">
                <ul>
					...
					<?php foreach($_categoryload as $ck=>$cv){?>
						<li class="<?php echo $b_category!='' && $cv==$b_category?'hit':'';?>"><a href="/?pn=board.list&_menu=<?php echo $_menu;?>&searchWord=<?php echo $searchWord ?>&searchMode=<?php echo $searchMode ?>&b_category=<?php echo $cv ?>" class="ctg"><?php echo $cv;?></a></li>
					<?php }?>
                </ul>
            </div>
            <script>
...
<div class="c_board_news">
	...
	<?php if( count($listPost) < 1) { // 내용 없을때 ?>
        ...
        <?php foreach($listPost as $k=>$v) {  ?>
            <ul>
                <li class="this_photo">
                    <?php // 썸네일 사이즈 : 350 x free ?>
                    <div class="photo"><?php echo $v['thumb']; ?></div>
                </li>
                <li class="this_post">
                    <div class="posting">
                        ...
                        <span class="state_icon">
                            <?php  if($v['iconReply'] === true) { ?><span class="ic_reply"><img src="<?php echo $SkinData['skin_url']; ?>/images/c_img/board_reply.svg" alt="댓글"><em><?php echo $v['talkCnt']; ?></em></span><?php } ?>
