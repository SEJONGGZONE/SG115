...
if(isset($_GET['__pskin']) || isset($__pskin)) { // 쿠키로 지속적인 스킨 미리보기 설정
	// 지정된 스킨이 없는경우 처리
	if(!is_dir($_SERVER['DOCUMENT_ROOT'].'/skin/site_m/'.$_GET['__pskin'])) {
...
if(isset($_COOKIE['temp_skin']) && empty($_pskin)) {
	// 지정된 스킨이 없는경우 처리
	if(!is_dir($_SERVER['DOCUMENT_ROOT'].'/skin/site_m/'.$_COOKIE['temp_skin'])) {
		samesiteCookie('temp_skin', '', time() -3600, '/', '.'.str_replace('www.', '', reset(explode(':', $system['host']))));
		error_loc_msg('/', '스킨이 존재하지 않습니다.', 'top');
	}
...
if($_pskin) {
	// 지정된 스킨이 없는경우 처리
	if( !is_dir($_SERVER['DOCUMENT_ROOT'].'/skin/site_m/'.$_pskin)) error_loc_msg('/', '스킨이 존재하지 않습니다.', 'top');
...
define('OD_BOARD_SKIN_DIR', OD_SKIN_DIR.'/board_m'); // PC 게시판 스킨 폴더
...
define('OD_SITE_SKIN_DIR', OD_SKIN_DIR.'/site_m'); // PC 스킨 폴더
...
# 스킨 리스트
$_skin_list = array();
$skinTmp = dir(OD_SKIN_ROOT.'/site_m/');
while($entry = $skinTmp->read()) {
	if(in_array($entry, array('..', '.'))) continue;

	$SkinInfo = array();
	if(file_exists(OD_SKIN_ROOT.'/site_m/'.$entry.'/skin.xml')) $SkinInfo = xml2array(file_get_contents(OD_SKIN_ROOT.'/site_m/'.$entry.'/skin.xml'));
