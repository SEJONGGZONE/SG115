...
/* 2022-09-20 : 마이그레이션 서포트 작성 -- PHP 7 SUPPORT */
if( !function_exists('_MQ_insert_id')){
    function _MQ_insert_id(){
        global $con;
...
if( !function_exists('_MQ_query')){
    function _MQ_query($query){
        global $con;
...
if( !function_exists('_MQ_fetch_assoc')){
    function _MQ_fetch_assoc($result){
        if( function_exists('mysql_fetch_assoc') ){
            return mysql_fetch_assoc($result);
        }else if( function_exists('mysqli_fetch_assoc') ) {
            return mysqli_fetch_assoc($result);
        }
    }
}

if( !function_exists('_MQ_fetch_array')){
    function _MQ_fetch_array($result){
        if( function_exists('mysql_fetch_array') ){
            return mysql_fetch_array($result);
        }else if( function_exists('mysqli_fetch_array') ) {
            return mysqli_fetch_array($result);
        }
    }
}

if( !function_exists('_MQ_affected_rows')){
    function _MQ_affected_rows(){
        global $con;
...
if( !function_exists('_MQ_connect')){
    function _MQ_connect($DB_host , $DB_id,$DB_pass,$DB_name){
        global $con;
        if( function_exists('mysql_connect') ){
            return @mysql_connect($DB_host , $DB_id,$DB_pass);
        }else if( function_exists('mysqli_connect') ) {
            return @mysqli_connect("$DB_host","$DB_id","$DB_pass","$DB_name");
        }
    }
}

if( !function_exists('_MQ_select_db')){
    function _MQ_select_db($DB_name){
        global $con;
        if( function_exists('mysql_select_db') ){
            return @mysql_select_db($DB_name);
        }else if( function_exists('mysqli_select_db') ) {
            return @mysqli_select_db($con , $DB_name);
        }
    }
}

if( !function_exists('_MQ_num_rows')){
    function _MQ_num_rows($query){
        global $con;
...
if( !function_exists('_MQ_query_result')){
    function _MQ_query_result($res){
        global $con;
        if( function_exists('mysql_fetch_array') ){
            $arr = array();
            if( mysqli_num_rows($res) > 0 ) {
                $arr_val =mysql_fetch_array($res);
                $val = $arr_val[0];
            ...
            $arr = array();
            if( mysqli_num_rows($res) > 0 ) {
                $arr_val =mysqli_fetch_array($res);
                $val = $arr_val[0];
...
if( !function_exists('_MQ_free_result')){
    function _MQ_free_result($res){
        global $con;
        if( function_exists('mysql_free_result') ){
            return mysql_free_result($res);
        }else if( function_exists('mysqli_free_result') ) {
            return mysqli_free_result($res);
        }
    }
}

if( !function_exists('_MQ_tablename')){
    function _MQ_tablename($res,$row){
        global $con;
        if( function_exists('mysql_tablename') ){
            return mysql_tablename($res,$row);
        }else if( function_exists('mysqli_free_result') ) {
...
// MYSQL 버전이 높아 PASSWORD() 프로시저가 없을 경우
if(!function_exists('db_password')) {
	function db_password($value='') {
		$row = _MQ(" select CONCAT('*', UPPER(SHA1(UNHEX(SHA1('".$value."'))))) as pass ");
		return strtoupper($row['pass']);
	}
}
/* 2022-09-20 : 마이그레이션 서포트 작성 -- PHP 7 SUPPORT -- END */

// 2017-08-21 mysql passwrod형으로 변경
if(!function_exists('db_password_old')) {
	function db_password_old($value='') {
...
if(!function_exists('error_msgPopup')) {
	function error_msgPopup($msg) {
		// LCY : {하이앱}
		if( function_exists('error_app_msgPopup')){
			error_app_msgPopup($msg);
		}
		else{
...
if(!function_exists('error_loc')) {
	function error_loc($loc,$target=null) {
		...
		// LCY : 2023-01-10 : 익스에서 AJAX 처리시 meta http-equiv='Refresh 먹히지 않음
		// else { echo "<meta http-equiv='Refresh' content='0;url=${loc}'>"; }
		else { echo "<script language='javascript'>location.href=('${loc}');</script>"; }
		exit;
	}
}
...
function mailer($_email, $_title, $_content, $cc=array()) {
	...
	$headers .= "From: ". (!preg_match("/@daum.net|@hamail.net/i" , $_email) ? "=?UTF-8?B?".base64_encode($siteInfo['s_adshop'])."?=" : $siteInfo['s_adshop']) ." <{$siteInfo['s_ademail']}>\r\n";
...
function get_mail_content($mailling_content) {
	...
	$banner_info = info_banner($_skin.',mailing,not_set_view,not_set_term,not_set_link_target,not_set_banner_link', 1, 'data'); // 공통 메일링 상단 로고
	if(!$banner_info[0]['b_img']) $banner_info = info_banner($_skin.',mobile_top_logo', 1, 'data'); // 로고가 없다면 사이트 로고 호출
	...
	$mail_body = '<!DOCTYPE HTML>
		...
		<body>
		<div style="overflow:hidden; padding:20px 10px; background:#F1F1F6; font-family:맑은고딕; font-size:12px; font-weight:400; color:#666">
			<div style="max-width:600px; margin:0 auto; background:#fff; box-sizing:border-box;border-collapse: inherit; padding:25px">
				<table style="width:100%; border-spacing:0;">
					<tbody>
						<tr>
							<td style="background:#fff;">
								<a href="'.$mailing_url.'" style="max-width:280px; display:inline-block" target="_blank">
									<img src="'.$mailling_logo.'" alt="'.addslashes(htmlspecialchars($siteInfo['s_glbtlt'])).'" style="max-width:100%; height:25px; border:0 !important;"/>
								</a>
							</td>
						</tr>
					</tbody>
				...
				<table style="width:100%; border-top:2px solid #000;  border-spacing:0; margin-top:30px">
					<tbody>
						<tr>
							<td style="text-align:center; padding:20px 0">
								<a href="'.$mailing_url.'" style="background:#fff; border:1px solid #000; font-size:12px; font-weight:600; color:#000; padding:12px 50px; text-decoration:none; display:inline-block;" target="_blank">홈페이지 바로가기</a>
							</td>
						</tr>
						<tr>
							<td style="padding-top:20px; color:#666; font-size:11px; border-top:1px solid #EBEBF2">
								본 메일은 발신전용으로 회신 할 경우 답변되지 않습니다. 문의사항은 <a href="'.$mailing_url.'/?pn=service.main" style="color:#666" target="_blank">고객센터</a>('.$siteInfo['s_glbtel'].')로 연락주십시오.<br/>
								'.$deny_content.'
								<br/>
								...
								<strong style="letter-spacing:0px; font-weight:400; font-size:11px">Copyright ⓒ '.htmlspecialchars($siteInfo['s_adshop']).' All rights reserved</strong>
							</td>
						</tr>
					</tbody>
				</table>
...
/*function pagelisting($cur_page, $total_page, $n, $url , $depth=null) {
	$start_page = ( ( (int)( ($cur_page - 1 ) / 5 ) ) * 5 ) + 1;		//KAY :: 관리자에서 사용되는 페이지네이트 숫자 5개 노출되도록 수정
...
function pagelisting($cur_page, $total_page, $n, $url , $depth=null) {
	...
	$start_page = ( ( (int)( ($cur_page - 1 ) / $view_page ) ) * $view_page ) + 1;
	$end_page = $start_page + ($view_page-1);
    ...
    //$retValue = "	<span class='lineup'>";
    if($cur_page > 1 ) {
        $retValue .= "<span class='nextprev prev' data-set='1'>";
        $retValue .= "<a href='" . $url . ($cur_page-1) . "' class='btn' data-pn='".($cur_page-1)."' title='이전' ><span class='ic'></span></a>";

        if( $cur_page > $view_page){
        	$retValue .= "<span class='btn_num'><a href='" .$url . "1' data-pn='1' >1</a><span class='more'></span></span>";
    	}

        $retValue .= "</span>";
    } else {
        $retValue .= "<span class='nextprev prev'>";
        $retValue .= "<span class='btn no'><span class='ic'></span></span>";
        //$retValue .= "<span class='btn_num'><span class='no'><span class='icon ic_first'></span></span><a href='" .$url . "1' class='ok' title='처음' ><span class='icon ic_first'></span></a></span>";
        $retValue .= "</span>";
    }
    ...
    $retValue .= "<span class='number'>";
    for($k=$start_page;$k<= $end_page;$k++)
        if($cur_page != $k) $retValue .= "<a href='" . $url . $k . "' data-pn='".$k."'>${k}</a>";
        else $retValue .= "<a href='#none' onclick='return false;' class='hit'>${k}</a>";
    $retValue .= "</span>";

    if($cur_page < $total_page  ) {
        $retValue .= "<span class='nextprev next'>";

        if(  $total_page > $end_page){
        	// 끝 페이지 번호 노출
        	$retValue .= "<span class='btn_num'><span class='more'></span><a href='" . $url . $total_page . "'  data-pn='".$total_page."' >".$total_page."</a></span>";
    	}

        $retValue .= "<a href='" . $url . ($cur_page+1) . "' data-pn='".($cur_page+1)."' class='btn' title='다음' ><span class='ic'></span></a>";
        $retValue .= "</span>";
    } else {
        $retValue .= "<span class='nextprev next'>";
        //$retValue .= "<span class='btn_num'><span class='no'><span class='icon ic_last'></span></span><a href='" . $url . $total_page . "' class='ok' title='끝' ><span class='icon ic_last'></span></a></span>";
        $retValue .= "<span class='btn no'><span class='ic'></span></span>";
        $retValue .= "</span>";
    }
    // $retValue .= "</span>";
...
// 페이지네이트
function pagelisting_bak($cur_page, $total_page, $n, $url , $depth=null) {
	$view_page = 5;
	...
	if($cur_page > 1) {
		...
		if( $cur_page > $view_page){
			$retValue .= "
				<span class='number'>
					<a href='".$url."1' title='' class='num' data-pn='1'>1</a><span class='more'></span>
				</span>
			";
		}
	} else {
	...
	if($cur_page < $total_page) {
		...
		if(  $total_page > $end_page){
			$retValue .= "
				<span class='number'>
					<span class='more'></span><a href='".$url.$total_page."' title='' data-pn='".$total_page."'  class='num'>".$total_page."</a>
				</span>
			";
		}
		$retValue .= "<span class='btn click'><span class='no'><span class='icon ic_next'></span></span><a href='" . $url . ($cur_page+1) . "' class='ok' title='다음' ><span class='icon ic_next'></span></a></span>";
...
function pagelisting_mobile($cur_page, $total_page, $n, $url , $depth=null,$type=null) {
	...
	if($cur_page > 1) {
        ...
        $retValue .= '
			<span class="btn click ic_prev">
				...
				<a href="'.$url.($cur_page-1).'" title="이전페이지" data-pn="'.($cur_page-1).'" class="ok "><span class="icon "></span></a>
			</span>
		';
		if( $cur_page > $view_page){
			$retValue .= '
	...
	$retValue .= "<span class='number'>";
	for($k=$start_page;$k<=$end_page;$k++)
	if($cur_page != $k) $retValue .= "<a href='".$url.$k."' class='num' data-pn='".$k."'>${k}</a>";
	else $retValue .= "<a href='#none' onclick='return false;' class='num hit'>${k}</a>";
	$retValue .= "</span>";
	...
	if($cur_page < $total_page) {
		...
		$retValue .= '
			<span class="btn ic_next">
				<span title="다음페이지" class="no"><span class="icon "></span></span>
				<a href="'.$url.($cur_page+1).'" title="다음페이지" data-pn="'.($cur_page+1).'" class="ok "><span class="icon"></span></a>
			</span>
		';
		$retValue .= "</span>";
	}
...
// 상품리뷰, 상품문의 페이지네이트
// total_cnt => 상품리뷰, 상품문의 총 개수
// ajax_func => 상품리뷰, 상품문의 view 함수
function ajax_pagelisting_mobile($cur_page, $total_page, $n,$ajax_func,$total_cnt) {
	...
	if($cur_page > 1) {
        ...
        $retValue .= '
			<span class="btn click ic_prev">
				<span title="이전페이지" class="no"><span class="icon "></span></span>
				<a href="#none" title="이전페이지" onclick="'.$ajax_func.'('.$total_cnt.','.($cur_page-1).'); return false;" class="ok "><span class="icon "></span></a>
			</span>
		';
		if( $cur_page > $view_page){
			$retValue .= '
				<span class="btn_num">
					<a href="#none" onclick="'.$ajax_func.'('.$total_cnt.',1); return false;" title="" class="num">1</a><span class="more"></span>
				</span>
			';
		}
		$retValue .= "</span>";
	...
	$retValue .= "<span class='number'>";
	for($k=$start_page;$k<=$end_page;$k++)
	if($cur_page != $k) $retValue .= '<a href="#none" onclick="'.$ajax_func.'('.$total_cnt.','.$k.'); return false;" class="num" >'.$k.'</a>';
	else $retValue .= "<a href='#none' onclick='return false;' class='num hit'>${k}</a>";
	$retValue .= "</span>";
	...
	if($cur_page < $total_page) {
		...
		if(  $total_page > $end_page){
			$retValue .= '
				<span class="btn_num">
					<span class="more"></span><a href="#none" onclick="'.$ajax_func.'('.$total_cnt.','.$total_page.'); return false;" title=""  class="num">'.$total_page.'</a>
				</span>
			';
		}
		...
		$retValue .= '
			<span class="btn click ic_next">
				<span title="다음페이지" class="no"><span class="icon "></span></span>
				<a href="#none" title="다음페이지" class="ok " onclick="'.$ajax_func.'('.$total_cnt.','.($cur_page+1).'); return false;"><span class="icon"></span></a>
			</span>
		';
		$retValue .= "</span>";
	} else {
...
function _InputSelect( $_name , $_arr , $_chk , $_event , $_arr2=null , $initval =null) {
	...
	foreach( $_arr  as $key=>$val ) {
		...
		if( $val == 'list_none' ||  $val == '===========') { $_str .= "disabled"; }
...
function onedaynet_sms_multisend($arr_send = array()) {
    ...
    if(count($arr_send_image) > 0) {
        ...
        include_once($_SERVER['DOCUMENT_ROOT'].'/include/nusoapmime.php');
...
function _PhotoForm($photoLOC, $photoVAR, $photoOLD, $attr='', $addClass=array()) {
	...
	$_img_reg = '
		<div class="input_file"'.($attr?' '.$attr:null).'>
			<input type="text" class="fakeFileTxt" readonly="readonly" placeholder="파일찾기" disabled="">
			<div class="fileDiv">
	...
	if($photoOLD) {
		...
		$_img_reg .= '
			<div class="preview_thumb">
				...
				<a href="#none" class="c_btn h27 js_thumb_popup'.($addClass['js_thumb_popup']?' '.$addClass['js_thumb_popup']:null).'" data-img="'.$photoOLD_src.'">미리보기</a>
			</div>
...
function _PhotoPro( $photoLOC , $photoVAR , $popup=null) {
	...
	if($_FILES[$photoVAR][size]> 0){
		...
		if( !preg_match("/gif|jpg|jpeg|bmp|png|svg/i" , $app_ext) ) {
...
//		200 정상이며, 나머지 비정상 호출
function CurlExecHeader($url, $RequestTime=100, $sslchk = false) {
	$cu = curl_init();
	curl_setopt($cu, CURLOPT_URL,  $url ); // 데이타를 보낼 URL 설정
	//curl_setopt($cu, CURLOPT_USERAGENT,"Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1; .NET CLR 1.1.4322; InfoPath.1)"); // 해당 데이타를 보낼 http head 정의 : 삭제해도 되긴함
	curl_setopt($cu, CURLOPT_POST,0); // 데이타를 get/post 로 보낼지 설정
	...
	curl_setopt($cu, CURLOPT_RETURNTRANSFER,1); // REQUEST 에 대한 결과값을 받을건지 체크 #Resource ID 형태로 넘어옴 :: 내장 함수 curl_errno 로 체크
	$arr_url = parse_url($url);
	if( $arr_url['scheme'] == "https" && $sslchk === false) {curl_setopt($cu, CURLOPT_SSL_VERIFYPEER, 0);}
	curl_setopt($cu, CURLOPT_TIMEOUT, $RequestTime); // REQUEST 에 대한 결과값을 받는 시간타임 설정
	$str = curl_exec($cu); // 실행
...
// LCY : 2023-01-05 :  쇼핑몰 티켓번호 생성 ---
function shop_ticketnum_create($_type=null){

	$_code = chr(mt_rand(65,90)).chr(mt_rand(65,90)).'-';
	for($i=0;$i<10;$i++){
		$check = mt_rand(0,1);
		if( $check == 1){ $_code .= chr(mt_rand(65,90));  }
		else{ $_code .= mt_rand(0,9); }
	}

	// 티켓중복체크
	$check = _MQ("select count(*) as cnt from smart_order_product_ticket where opt_ticketnum = '".$_code."' ");
	if( $check['cnt'] > 0){ return shop_ticketnum_create(); }
...
function app_product_thumbnail($app_path, $_img_name, $_img_OLD, $mode="s"){
	...
	$dimentions = array(
		...
		, 'list' => array(
			'b' => array($SkinInfo['product']['list_image_width'], $SkinInfo['product']['list_image_height']),
			'thumbs_s_' => array($SkinInfo['product']['thumb_image_width'], $SkinInfo['product']['thumb_image_height'])
		)
		// -- 이미지 썸네일 오류 수정 -- 2019-06-18 LCY
...
function is_admin($ONLY_USER=false) {
	global $mem_info,$_SERVER;
	
	// 관리자페이지에서는 권한제한 x
	if(preg_match('/totalAdmin/i', $_SERVER['REQUEST_URI'])){
		$ONLY_USER = false;
	}


	// KAY :: 2023-12-18 :: 관리자가 사용자에서 로그인한 경우만 권한처리
	if($ONLY_USER==true){
		if($mem_info[in_userlevel] == 9){
			return true;
		}else{
			return false;
		}
...
// --- 암호화 ---
function onedaynet_encode( $str, $ver = 0 ){
	global $DB_id;
	...
	if( $rst_phpVersion == 7){
		...
		$data = openssl_encrypt($str, $cipher, $key, OPENSSL_RAW_DATA, $iv);
...
// --- 복호화 ---
function onedaynet_decode( $encode_str , $ver = 0 ){
	global $DB_id;
	...
	// --- PHP >= 7
	if( $rst_phpVersion == 7){
		$encode_str = enc('d',urldecode($encode_str));
		$cipher = "aes-128-ecb";
		$hash = 'sha256';
		$key = pack('H*', md5($DB_id));
		$key = substr($key, 0, 32);
		$ivlen = openssl_cipher_iv_length($cipher);
		$iv =  substr($key, 0, $ivlen);
		$decrypted = openssl_decrypt($encode_str, $cipher, $key, OPENSSL_RAW_DATA, $iv);
		return $decrypted;
	}
...
function AdminLogin($_uid) {
    ...
    if(is_mobile() === true) mk_sess('AuthAdminDiff', md5($siteInfo['s_license'].$_SERVER['HTTP_USER_AGENT'].$_uid)); // 모바일에서는 아이피가 수시로 바뀌기 때문에 아이피 항목 제외
    else mk_sess('AuthAdminDiff', md5($siteInfo['s_license'].$_SERVER['REMOTE_ADDR'].$_SERVER['HTTP_USER_AGENT'].$_uid));

    mk_cookie('AuthAdminDiff', $_SESSION['AuthAdminDiff']);

	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 {
		global $system;
		$AuthSecureToken = md5(json_encode($_SERVER)); // 토큰생성
		$checksum = md5($siteInfo['s_license'].$_SERVER['HTTP_USER_AGENT'].$_uid);
		samesiteCookie("AuthAdminSecureToken", $AuthSecureToken , 0 , "/" , "." . str_replace("www." , "" , reset(explode(':', $system['host']))));
		insert_secure_token('totalAdminLogin',$AuthSecureToken,$checksum);
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 }
...
function AdminLogout() {
	...
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 {
	delete_secure_token('totalAdminLogin',$_COOKIE['AuthAdminSecureToken']);
	@samesiteCookie("AuthAdminSecureToken", "" , 0, "/" , "." . str_replace("www." , "" , reset(explode(':', $system['host']))));
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 }
...
function AdminLoginCheck($_mode='move') {
    ...
    if(is_mobile() === true) $AuthDiff[] = md5($siteInfo['s_license'].$_SERVER['HTTP_USER_AGENT'].$_COOKIE['AuthAdmin']); // 모바일에서는 아이피가 수시로 바뀌기 때문에 아이피 항목 제외
    else $AuthDiff[] = md5($siteInfo['s_license'].$_SERVER['REMOTE_ADDR'].$_SERVER['HTTP_USER_AGENT'].$_COOKIE['AuthAdmin']);
	...
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 {
	$checksum = md5($siteInfo['s_license'].$_SERVER['HTTP_USER_AGENT'].$_COOKIE['AuthAdmin']);
	$secureToken = get_secure_token('totalAdminLogin',$_COOKIE['AuthAdminSecureToken'], $checksum );
	if(  empty($secureToken)  ){ $AuthDiffCookie = NULL;  }
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 }
...
function SubAdminLogin($_uid) {
	...
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 {
		...
		$AuthSecureToken = md5(json_encode($_SERVER)); // 토큰생성
		$checksum = md5($siteInfo['s_license'].$_SERVER['HTTP_USER_AGENT'].$_uid);
		samesiteCookie("AuthSubAdminSecureToken", $AuthSecureToken , 0 , "/" , "." . str_replace("www." , "" , reset(explode(':', $system['host']))));
		insert_secure_token('subAdminLogin',$AuthSecureToken,$checksum);
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 }
...
function SubAdminLogout() {
	...
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 {
	delete_secure_token('subAdminLogin',$_COOKIE['AuthSubAdminSecureToken']);
	@samesiteCookie("AuthSubAdminSecureToken", "" , 0, "/" , "." . str_replace("www." , "" , reset(explode(':', $system['host']))));
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 }
...
function SubAdminLoginCheck($_mode='move') {
	...
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 {
	$checksum = md5($siteInfo['s_license'].$_SERVER['HTTP_USER_AGENT'].$_COOKIE['AuthCompany']);
	$secureToken = get_secure_token('subAdminLogin',$_COOKIE['AuthSubAdminSecureToken'], $checksum );
	if(  empty($secureToken)  ){ $AuthDiffCookie = NULL;  }
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 }
...
function UserLogin($_uid) {
	...
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 {
		global $system;
		$AuthSecureToken = md5(json_encode($_SERVER)); // 토큰생성
		$checksum = md5($siteInfo['s_license'].$_SERVER['HTTP_USER_AGENT'].$_uid.$_uid);
		samesiteCookie("AuthUserSecureToken", $AuthSecureToken , 0 , "/" , "." . str_replace("www." , "" , reset(explode(':', $system['host']))));
		insert_secure_token('login',$AuthSecureToken,$checksum);
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 }
}
...
function UserLogout() {
	...
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 {
	delete_secure_token('login',$_COOKIE['AuthUserSecureToken']);
	@samesiteCookie("AuthUserSecureToken", "" , 0, "/" , "." . str_replace("www." , "" , reset(explode(':', $system['host']))));
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 }
...
function UserLoginCheck($_mode='move') {
	...
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 {
	$checksum = md5($siteInfo['s_license'].$_SERVER['HTTP_USER_AGENT'].$_COOKIE['AuthIndividualMember'].$_COOKIE['AuthShopCOOKIEID']);
	$secureToken = get_secure_token('login',$_COOKIE['AuthUserSecureToken'], $checksum );
	if( empty($secureToken) ){ $AuthDiffCookie = NULL;  }
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 }
	...
	if(count($AuthDiff) === 1 && $AuthDiffCookie) { return true; }
		...
		if( $DeveMode !== true){
			if($_mode == 'move' && !preg_match('/member.login.pro.php/i', $_SERVER['REQUEST_URI'])) {
				UserLogout();
				die("<script>alert('로그인 세션이 만료되었습니다.'); top.location.href=('".OD_PROGRAM_URL."/member.login.pro.php?_mode=logout')</script>");
			}
		}
		return false;
	}
			...
			// 보안서버 제외페이지
			 if( !$_SERVER['HTTPS'] ) {
				if(HTTPS_Check()){
					...
					// ssl_forced_reset();// SSL 사용안함 강제처리  // LCY : 2023-06-20 : 보안서버강제처리 중단
				}
			}
			break;
...
function URI_Rebuild($uri = '', $AddUri=array(), $RemoveUri=array()) {
	...
	$uri = str_replace('?=&', '?', urldecode($url_rebuild)); // 잔여 삭제// GET 변경 - sql injection 막기
	if($uri == '?') $uri = '';
	if($uri == '') $uri_prefix = '';
...
{
	$ua = $ua == false ? 'mobile' : strtolower($ua);
...
function printDateInfo($date,$_type = false)
{
	if($_type == false){
		...
		return $d.($hm != '00:00:00' ? '<div class="t_light">('.$hm.')</div>': null);
	}
	else if($_type == 1){
		$d = date('Y-m-d',strtotime($date));
		$hm = date('H:i:s',strtotime($date));
		return $d.($hm != '00:00:00' ? ' ('.$hm.')': null);
	}
	else if($_type == 2){
		$d = date('Y-m-d',strtotime($date));
		return $d;
...
*/
function SkinInfo($view='all', $type='site_m', $skin='auto') {
	global $siteInfo;
	if($type == 'auto') {
		$type = 'site_m';
	}
...
/*
	SkinInfo(출력 배열중 특정 배열을 지정하여 호출한다.(전체는 all), 스킨폴더 내부의 어떤폴더에서 불러올지 지정, 스킨을 직접지정(자동은 auto))
*/
function SkinInfoMo($view='all', $type='site_m', $skin='auto') {
	global $siteInfo;
	if($type == 'auto') {
		$type = 'site_m';
	}
...
if ( !function_exists( 'hex2bin' ) ) {
	...
	function fn_option_color($pouid , $color_type , $color_name){
		echo '
			...
			<div class="type_select">' . _InputRadio('po_info[' . $pouid . '][po_color_type]', array('color', 'img'), ( $color_type ? $color_type :'color'), ' class="color_type" data-pouid="' . $pouid . '" ', array('색상', '이미지')) . '</div>

			<!-- // 1) 컬러 선택 -->
			<div class="lineup-row right_box color" data-pouid="' . $pouid . '">
			...
			<!-- // 2) 이미지 선택 -->
			<div class="lineup-row type_multi right_box img" data-pouid="' . $pouid . '">
	...
	function fn_common_option_color($couid , $color_type , $color_name){
		echo '
			...
			<div class="type_select">' . _InputRadio('co_info[' . $couid . '][co_color_type]', array('color', 'img'), ( $color_type ? $color_type :'color'), ' class="color_type" data-couid="' . $couid . '" ', array('색상', '이미지')) . '</div>

			<!-- // 1) 컬러 선택 -->
			<div class="lineup-row right_box color" data-couid="' . $couid . '">
			...
			<!-- // 2) 이미지 선택 -->
			<div class="lineup-row type_multi right_box img" data-couid="' . $couid . '">
	...
	function printCouponSetBoon($couponSetData)
	{
		...
		if( $couponSetData['ocs_boon_type'] == 'discount'){ // 할인
			if( $couponSetData['ocs_dtype'] == 'per'){ // 할인율일경우
				$printBoonType = "구매시 ".number_format($couponSetData['ocs_per'])."% 할인";
				if($couponSetData['ocs_price_max_use'] == 'Y') $printBoonType .="<em>(최대 ".number_format($couponSetData['ocs_price_max']).'원까지 할인)</em>';
			}else{
			...
			if( $couponSetData['ocs_dtype'] == 'per'){ // 할인율일경우
				$printBoonType = "구매시 ".number_format($couponSetData['ocs_per'])."% 적립";
				if($couponSetData['ocs_price_max_use'] == 'Y') $printBoonType .="<em>(최대 ".number_format($couponSetData['ocs_price_max']).'원까지 적립)</em>';
			}else{
...
// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 기타 처리 {
if(!function_exists('msg_Error')) {
	function msg_Error($msg) {
		global $tool_db_debug , $con; // var.php 에서 적용가능
		$tool_db_debug =true;
		if( empty($tool_db_debug) || $tool_db_debug !== true){
			echo "
				<table cellspacing=0 cellpadding=10 border=0 bgcolor='#fff7c5' width=580>
					<tr><td><font size=2 color='darkred'>사이트에러가 발생하였습니다.</td></tr>
		...
		echo "
			<table cellspacing=0 cellpadding=10 border=0 bgcolor='#fff7c5' width=580>
				<tr><td><font size=2 color='darkred'> $msg </td></tr>
				<tr><td><font size=2 color='darkred'> ".$error."</td></tr>
				<tr><td><font size=2 color='darkred'> ".$errno."</td></tr>
				<tr><td><input type=button value='BACK' onClick='history.back()' style='font-size:10pt; color:#112244; border-width:1; border-color:#eeeeee;'></td></tr>
			</table>
		";
		exit();
...
function site_secure_encode($val = ''){
	if( empty($val)){ return ''; }
	$secureVal = onedaynet_encode($val);
	return $secureVal;
}
function site_secure_decode($val = ''){
	if( empty($val)){ return ''; }
	$secureVal = onedaynet_decode($val);
	if( empty($secureVal)){ return $val; } // 복호화가 안된다면 원문으로 보낸다.
	return $secureVal;
}
// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 쿠키 보안 }


// LCY : 2022-12-21 : 티켓기능 --  필요함수 추가 {

	// StartDate와 EndDate 사이의 모든 날짜를 추출 한다.
	function GetDateRangeArr($StartDate, $EndDate, $format='Y-m-d') {
		$arr = array();
		$diff_num = (strtotime($EndDate)-strtotime($StartDate))/86400;
		for($i=1; $i<$diff_num; $i++) {
			$arr[] = date($format, strtotime('+'.$i.'days', strtotime($StartDate)));
		}
		return $arr;
	}


	// StartDate와 EndDate 까지의 날짜 차이를 일 수로 가져온다.
	function getDateDiffDay($StartDate, $EndDate, $format='Y-m-d'){
		if( !$StartDate  || !$EndDate){ return false; }
		$startD = new DateTime($StartDate);
		$endD = new DateTime($EndDate);
		$result    = date_diff($startD, $endD);
	...
	// 특정 날자를 기준으로 오늘까지의  개월 수 를 구한다.
	function op_day_month($day)
	{

		$today = date('Y-m-d');

		$day_y = date('Y',strtotime($day));
		$day_m = date('m',strtotime($day));

		$today_y = date('Y',strtotime($today));
		$today_m = date('m',strtotime($today));

		$result = ($today_y - $day_y)*12 + ($today_m - $day_m);
	...
	// 요일계산 => $date  foramt => 2017-02-09
	function op_day_yoil($date)
	{
			$arr_yoil = array("일","월","화","수","목","금","토"); // 요일
			$sel_yoil = ($arr_yoil[date('w', strtotime($date))]); // 해당날자의 요일을 가져온다ㅏ.
			$result = $sel_yoil;// $print_date = date('Y-m-d',strtotime($sr['otpo_dateoption_date'])); // 년-월-일 사용 시
			return $result;
	}

	//  요일코드계산 => $date  foramt => 2017-02-09
	function op_day_yoil_code($date)
	{

			$sel_yoil_code = date('w', strtotime($date)); // 해당날자의 요일을 가져온다ㅏ. (0~6 까지 키값을 던져준다.)
			$result = $sel_yoil_code+1;// $print_date = date('Y-m-d',strtotime($sr['otpo_dateoption_date'])); // 년-월-일 사용 시
			return $result;
	}

	// 일반달력옵션의 경우 출력을 하나의 함수에서 처리
	function print_date_type($_date = '',$_loc='')
	{
		if($_date == '') return '';
		if( $_loc == 'print') return " (예약일 ".$_date.")";
		else if($_loc == 'ticket.view'){ return "<span class='if_date'>예약일 <strong>".$_date."</strong></span>";  }
		return "<span class='if_date'>예약일 <strong>".$_date."</strong></span><br>";
	}

	// 티켓유효기간에 따른 표기
	function print_expire_type($row = array()){
		if( count($row) < 1){return '없음'; }

		$_type = $row['p_expire_type'];
		if( $_type == 'day'){

			if( $row['p_expire_day'] < 1){
				return '발급당일';
			}
			else{
				return '발급일로부터 '.$row['p_expire_day'].'일 까지';
			}
		}
		else if($_type == 'date' && rm_str($row['p_expire_date']) > 0){
			return $row['p_expire_date'].'일 까지';
		}
		else{
			return '없음';
		}
	...
	// 달력옵션 상품에서 설정 가능 시작/종료 년월을 가져온다. (상세 설정에 대한 체크는 하지 않는다.)
	function get_product_dateoption_enable_date($pcode = false){
		$p_info = get_product_info($pcode);
		if( count($p_info) < 1 || $p_info['p_dateoption_use'] != 'Y'){ return array(); }

		$calendar = array();

		// 달력옵션에서 최소 설정 가능 시작일을 가져온다.
		if( $p_info['p_dateoption_date_type'] == 'day'){
			...
			if( date('Y-m',strtotime($p_info['p_dateoption_sdate'])) < date('Y-m')){
				$p_info['p_dateoption_sdate'] = date('Y-m-d');
			}
...
/**
	...
	* 카카오REST API ::  입력된 주소에 대한 상세 정보를 리턴;
**/
function kakao_addr_search($query)
{
	global $siteInfo;

	if($query == ''){ return false; }

	$apikey = $siteInfo['kakao_api'];
	...
	$apiUrl = 'https://dapi.kakao.com/v2/local/search/address.json'; //  다음주소 api
...
// LCY : 2022-11-24 : 티켓기능
/*
	** 구글도 있으나, 구글은 API 변동사항이 많아 다른 API로 처리
	** 옵션은 고정형태로 사용, 추후 변경시 이쪽 함수에서 받아서 처리하도록
	$data: URL 입력
	$option : https://goqr.me/api/doc/create-qr-code/ 참조
*/
if( !function_exists('create_qrcode_url')){
	function create_qrcode_url($data = '',$option =array()){
		$baseUrl = 'https://api.qrserver.com/v1/create-qr-code/';
		if($data == ''){ return false; }
		// 기본옵션
		$getOption = array(
			'size'=>$option['size'] ? $option['size'] : '500x500',
			'ecc'=>'H',
			'margin'=>isset($option['margin']) ? $option['margin'] : 20,
			'data'=>$data,
			'date'=>$option['date'] ? $option['date'] : date('Y-m-d'),
			'post'=>'now' //고정
		);

		$getUrl = $baseUrl.'?'.http_build_query($getOption);
		return $getUrl;
	}
}

// [LCY] -- 요일계산기 :: return -> X일, X:X:X 남음
function date_diff_datetime($sdate = false,$edate = false)
{
	/*
	ex
	input:
	...
	*/
	...
	if( $sdate === false) $sdate = date('Y-m-d H:i:s');
	if( $edate === false) $edate = date('Y-m-d H:i:s');

	// 타임 시간차를 계산
	$optTime = strtotime($edate)-strtotime($sdate);

	// 일 / 시/분/초 까지 표시
	$setDatetime = array();

	$setDatetime['day'] = $optTime/86400;
	$setDatetime['hour'] = ($optTime%86400) / 3600;
	$setDatetime['minut'] = (($optTime%86400)%3600) / 60;
	$setDatetime['second'] = ( (($optTime%86400)%3600) % 60 );
	foreach($setDatetime as $k=>$v){ $setDatetime[$k] = floor($v);  $setDatetime[$k] = ($setDatetime[$k] < 10 && $k != 'day' ? '':null).$setDatetime[$k]; }

	return $setDatetime;
}

// KAY :: 2023-03-27 :: 타임세일 시간체크
function date_diff_datetime2($sdate = false,$edate = false)
{
	/*
	ex
	input:
	...
	*/
	...
	$setDatetime['day'] = $optTime/86400;
	$setDatetime['hour'] = ($optTime%(86400*2)) / 3600;
	$setDatetime['minut'] = (($optTime%86400)%3600) / 60;
	$setDatetime['second'] = ( (($optTime%86400)%3600) % 60 );
	foreach($setDatetime as $k=>$v){ $setDatetime[$k] = floor($v);  $setDatetime[$k] = ($setDatetime[$k] < 10 && $k != 'day' ? '':null).$setDatetime[$k]; }
...
// 전체 관리자 답변용 작성자를 가져온다.
function get_reply_writer(){
	global $siteInfo;
	$writer = $siteInfo['s_reply_writer'] != '' ? $siteInfo['s_reply_writer'] : $siteInfo['s_adshop'];
	return $writer;
}


/*  LCY : 2023-03-20 : serialize helper
    ** Comment
    ** 직렬화된 데이터 예외처리를 위한 함수추가
    ** 데이터 저장 후 DB 추출할 시 내부 내장 DB 함수가 stripslashes를 하여 데이터가 불량이됨
    ** 이를 보존하기 위해 저장시 인코딩,추출시 디코딩 하여 사용
*/
