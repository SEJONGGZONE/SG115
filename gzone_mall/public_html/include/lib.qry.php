...
function info_banner($loc , $limit=1, $return_type = 'html'){
	$arr = array();
	$r = _MQ_assoc("select * from smart_banner where b_loc='" . $loc . "' and b_view='Y' and ( (b_sdate <= CURDATE() and b_edate >= CURDATE() ) or b_none_limit = 'Y' ) order by b_idx asc , b_uid desc limit 0, " . $limit );
	foreach($r as $k=>$v){
		if($return_type == "data" ) {
			$arr[$k] = $v;
		} else {
...
function shop_pointlog_insert( $_id , $_title , $_point , $_status , $_pro_date){
	...
	if($_appdate == DATE('Y-m-d')) {
		...
		if($row_point['in_id']){
			...
			// 회원 포인트 UPDATE
			_MQ_noreturn("update smart_individual set in_point = '". $calc_point ."' where  in_id = '".$apply_id."' ");// JJC : 2022-07-11 : 웹 접근 제어
			// 포인트 UPDATE
...
//				하나도 배송완료나,진행이없으면=> 주문상태 : 결제완료 or 결제대기
// 인자 : 주문번호
function order_status_update($ordernum) {
	...
	else if($order_info['o_paystatus'] == "N") $order_status ="결제대기";
	else {
...
//		}
		...
		else if($count['배송대기'] > 0) {
			$order_status ="배송대기";
		}

		// LCY : 2023-01-05 : 티켓 -- 발급상태 추가
		else if($count['발급완료'] > 0 && $order_info['o_order_type'] == 'ticket'){
			$order_status ="발급완료";	
		}
		else {
			$order_status ="결제완료";
	...
	// LCY : 2023-01-05 : 티켓 -- 발급상태 추가
	if(in_array($order_status, array('배송대기', '배송준비', '배송중', '배송완료','발급완료'))) {
...
function order_delivery_date_update($ordernum,$order_status)
{
	...
	if( $order_status == '배송대기' || $order_status == '배송준비'){
	...
	// LCY : 2023-01-05 : 티켓 -- 발급상태 추가
	else if($order_status == '발급완료'){
		$o_completedate = _MQ_result("select op_completedate from smart_order_product where op_oordernum = '".$ordernum."' and op_sendstatus = '발급완료' order by op_completedate desc  ");

		// 티켓일 경우에만 최종 발급완료, 나머지는 배송기준으로 업데이트
		_MQ_noreturn(" update smart_order set  o_completedate = '".$o_completedate."' where o_ordernum = '".$ordernum."' and o_order_type = 'ticket'  ");
	}
	return ;
}
...
//			: 쿠폰 가격
function give_coupon($coup_inid,$couponSetData,$_checksum = '') {  // LCY : 2023-01-18 : 쿠폰추가적용 -- $_checksum  추가 , 첫주문/구매 쿠폰일경우
	global $arrCouponSet;
	...
	// LCY : 2023-01-18 : 쿠폰추가적용 -- 특수데이터 저장, 첫주문/구매 쿠폰일경우
	if( $_checksum != ''){
		$arrSque[] = " coup_checksum = '".$_checksum."' ";
	}
...
function get_pro_1depth($cuid,$mode = 'name') {
	if(!$cuid) return;
	$r = _MQ("select c_uid,c_name from smart_category where c_uid = (select SUBSTRING_INDEX(c_parent,',',1) from smart_category where c_uid = '". rm_str($cuid) ."')");// JJC : 2022-07-11 : 웹 접근 제어
...
function point_update() {
	...
	foreach( $r as $k=>$v ){
		...
		if($apply_id <> ''){ // 회원아이디가 있을때만 실행
			...
			// LCY : 2022-10-25 : 포인트 지급예정에 다른 처리 보완 패치
			$v['in_point'] = _MQ_result(" select in_point from smart_individual where in_id = '".$apply_id."' ");	
...
function get_category_list($depth=1,$parent_cuid) {
	...
	$r = _MQ_assoc("select * from smart_category where c_view='Y' and c_depth='" . rm_str($depth) ."' ".$where." order by c_idx asc"); // JJC : 2022-07-11 : 웹 접근 제어
	return $r;
}
...
function get_cart_cnt() {
	...
	$r = _MQ("select count(*) as cnt from smart_cart where c_cookie = '".$_COOKIE["AuthShopCOOKIEID"]."' and c_cookie != ''  ");
...
function get_talk_total($pcode,$ttype='talk',$mtype='all', $where='') {
	...
	$row = _MQ("select count(*) as cnt from smart_product_talk as pt inner join smart_product as p on (pt.pt_pcode = p.p_code) where 1 and p_view='Y' ".$where);
...
// 인자 : 상품코드
// 리턴 : boolean
function is_cart($pcode) {

	$r = _MQ("select count(*) as cnt from smart_cart where c_pcode = '".$pcode."' and c_cookie='" . $_COOKIE["AuthShopCOOKIEID"] . "' ");
	if($r['cnt'] > 0) return true;
	else return false;
...
function get_order_ing_cnt($order_status=array()) {
	...
	$add_sql = "";
	//if(!in_array('결제대기', $order_status)) $add_sql = " and o_paystatus='Y'";
	...
	if(!in_array('결제대기', $arr_order_status_ordering)){
		$add_sql = " and o_paystatus='Y'";
	}else{
		unset($arr_order_status_ordering[array_search('결제대기', $arr_order_status_ordering)]);
		$add_sql = " or (o_status='결제대기' and o_paymethod='online') ";
	}
...
function get_send_info($arr){
    ...
    // 주석: LCY : 2023-05-31 : 입점업체 SMS추가
    global $arr_sms_text_com_allow_type, $SubAdminMode;

    $arr_send = array();
    $stringsAdd = array(); // 치환자 추가 SSJ : 2017-10-21

     // LCY : 2023-05-31 : 입점업체 SMS추가
    $arr_com_send = array(); // 입점업체 아이디
    ...
    if(count($arr) > 0){
        ...
        foreach($arr as $k=>$v){
            ...
            if($_onum <> ''){
	            ...
	            // 티켓치환자 추가 -> 티켓사용/미사용 시
	            else if(  in_array($v['type'], array( 'ticket_use','ticket_unuse' )  ) ){
	            	$arr_order[$_onum]['ticket_cnt'] ++;
	            }    
            }
        }
        ...
        foreach($arr_type as $k=>$v){
            if($v['cnt'] > 0){
                ...
                // 주석: LCY : 2023-05-31 : 입점업체 SMS추가
                $arr_type[$k]['infoCom']= _MQ("select * from smart_sms_set where ss_uid = 'com_".$k."' limit 1");
            }
        }
        ...
        foreach($arr_order as $k=>$v){
            ...
            // 티켓치환자 추가 -> 티켓사용/미사용 시
            if(  $v['ticket_cnt'] > 0 ){
            	$r = _MQ("select opt_ticketnum, o_paymethod, o_bank, o_oname, o_rdate, o_price_real, o_bank, o_sendcompany, o_sendnum, o_senddate from smart_order_product_ticket as opt left join smart_order as o on(o.o_ordernum = opt.opt_oordernum) where opt_uid = '".$k."'");
            	$arr_order[$k]['name'] = $r['o_oname'];
            	$arr_order[$k]['ticketnum'] = $r['opt_ticketnum'];
        ...
        foreach($arr as $k=>$v){
            ...
            if(is_array($ordernum)) {
                ...
                // LCY : 2023-05-31 : 입점업체 SMS추가
                $comid = $stringsAdd['{comid}'];
                $op_uid = $stringsAdd['{op_uid}'];
            }


			// LCY : 2023-05-31 : 입점업체 SMS추가 -- 업체번호 추출 {
	            if( in_array($type,array('ticket_use','ticket_unuse'))){ // 티켓일 경우  
	            	$op_uid = $k;
	            }
	            $arr_com = array();
	            if( in_array($type,$arr_sms_text_com_allow_type)){
					if( $comid != ''){ // 업체아이디 기준 1 순위
						$com_res = _MQ_assoc("select cp_tel2 from smart_company where cp_tel2 != '' and cp_id = '".$comid."'  ");
						foreach($com_res as $cpk=>$cpv){
							$arr_com[] = $cpv['cp_tel2'];
						}
					}
					else if($op_uid != ''){
						$op_res = _MQ_assoc( " select cp_tel2 from smart_order_product as op left join smart_company as cp on (op.op_partnerCode = cp.cp_id) where cp_tel2 != '' and op_uid = '". $op_uid ."' group by op_partnerCode " );
						if(count($op_res) > 0){
							foreach($op_res as $opk=>$opr){
								$arr_com[] = $opr['cp_tel2'];
							}
						}					
					}
					else{
						$op_res = _MQ_assoc( " select cp_tel2 from smart_order_product as op left join smart_company as cp on (op.op_partnerCode = cp.cp_id) where op_oordernum = '".$ordernum."' and cp_tel2 != '' group by op_partnerCode " );
						if(count($op_res) > 0){
							foreach($op_res as $opk=>$opr){
								$arr_com[] = $opr['cp_tel2'];
							}
					...
					$arr_com = array_filter($arr_com);
					if( count($arr_com) > 0){ $arr_com = array_unique($arr_com); }
				}
			// LCY : 2023-05-31 : 입점업체 SMS추가 -- 업체번호 추출 }
            ...
            // 티켓치환자 추가
            $opt_ticketnum = $arr_order[$ordernum]['ticketnum'];
            ...
            // LCY : 2023-05-31 : 입점업체 SMS추가 {
            $smsInfo = $arr_type[$type]['infoCom'];
            if($smsInfo['ss_status'] == "Y" && count($arr_com) > 0 && $SubAdminMode === true ) {
                $text = str_replace(array_keys($arr_replace),array_values($arr_replace), $smsInfo['ss_text']);
                // 문자/알림톡 통합 발송

                foreach($arr_com as $ck=>$cv){
                	$arr_send[] = array_merge(array('receive_num'=> $cv, 'send_num'=> $siteInfo['s_glbtel'], 'msg'=> $text, 'title'=>$smsInfo['ss_title'], 'image'=>$smsInfo['ss_file'], 'reserve_time'=>'' ) , smsinfo_array($smsInfo , $arr_replace));
                }
            }
            // LCY : 2023-05-31 : 입점업체 SMS추가 }
...
function shop_send_sms($to,$type,$ordernum="") {
    ...
    // {LCY} : 하이앱
    if( function_exists('shop_send_push') == true){ 
    	if( shop_send_push($to,$type,$ordernum) === true){ return true; }
    }
...
function shop_send_sms_multi($arr) {
    ...
    // {LCY} : 하이앱
    if( function_exists('shop_send_push_multi') == true){ 
    	if( shop_send_push_multi($arr) === true){ return true; }
    }
...
function get_product_list($query,$order,$limit,$img_size,$name_len='100',$subname_len='100',$img_type='p_img_list_square') {
	...
	$assoc = _MQ_assoc("select
						*,
						(select count(*) as cnt from smart_product_talk where pt_pcode=p_code and pt_type='상품리뷰' and pt_intype='normal') as review_cnt
...
function couponIssuedAutoType1($_ordernum)
{
	...
	foreach($resCouponSet as $csk=>$csv){
		// 쿠폰 기본체크
		$isChk = couponSetIssuedChk($csv, $mem_info,$_ordernum ); // LCY : 2023-01-18 : 쿠폰추가적용 -- $_ordernum  추가
		if( $isChk !== true){ continue; }

		give_coupon($mem_info['in_id'], $csv,$_ordernum); // 쿠폰발급 // LCY : 2023-01-18 : 쿠폰추가적용 -- $_ordernum  추가
	}
}
...
function _text_info_insert( $tablename , $datauid , $keytype , $value , $trigger=null ){
	...
	if(trim($value) || $trigger == "ignore") {
		if($r[cnt] > 0) {
			// 데이터 수정
			_MQ_noreturn(" update smart_table_text set ttt_value = '" . addslashes($value) . "' where ttt_tablename = '{$tablename}' and ttt_datauid = '{$datauid}' and ttt_keyword = '{$keytype}' ");
		}
		else {
			// 데이터 입력
			_MQ_noreturn(" insert smart_table_text set ttt_tablename = '{$tablename}' , ttt_datauid = '{$datauid}' , ttt_keyword = '{$keytype}' , ttt_value = '" . addslashes($value) . "'");
		}
	}
...
if(!function_exists('IsField')) {
    function IsField($Table, $Field) {
        ...
        $result = _MQ_query($sql);

        if(@_MQ_num_rows($result)) return true;
        else return false;
    }
}
...
if(!function_exists('IsTableData')) {
    function IsTableData($Table) {
        ...
        // 테이블 인덱스 정보
        $IndexResult = _MQ_query(' show index from ' . $Table);
        while($IndexData = _MQ_fetch_assoc($IndexResult)){
        ...
        // 테이블 컬럼 상세 정보
        $ColumnResult = _MQ_query(' show columns from ' . $Table);
        while($ColumnData = _MQ_fetch_assoc($ColumnResult)){
...
// KAY :: 팝업 정렬순서 :: 2023-03-08
function popup_resort(){
	// 정렬그룹의 유효성 검사
	$arr_idx = array();
	$arr_gidx = array();
	$arr_sort_group = array();
	$res = _MQ_assoc(" select p_uid, p_sort_group, p_sort_idx, p_idx from smart_popup where 1 order by p_sort_group asc , p_sort_idx asc , p_idx asc ");
	$pidx = 1;
	foreach($res as $k=>$v){
		$trigger_idx = true; // false 이면 업데이트
		...
		if($pidx <> $v['p_idx']){
			$arr_idx[$pidx-$v['p_idx']][] = $v['p_uid'];
		}
		if( ($arr_sort_group[$v['p_sort_group']]) <> $v['p_sort_idx']){
			$arr_gidx[$arr_sort_group[$v['p_sort_group']]-$v['p_sort_idx']][] = $v['p_uid'];
		}
	...
	if(sizeof($arr_idx)>0){
		foreach($arr_idx as $k=>$v){
			_MQ_noreturn(" update smart_popup set p_idx = (p_idx + '". $k ."') where p_uid in ('". implode("','" , $v) ."') ");
		}
	}
	if(sizeof($arr_gidx)>0){
		foreach($arr_gidx as $k=>$v){
			_MQ_noreturn(" update smart_popup set p_sort_idx = (p_sort_idx + '". $k ."') where p_uid in ('". implode("','" , $v) ."') ");
		}
	}
}
...
function clean_cart() {
	...
	$qur = "
		...
		where (1) and
			...
			(
				...
				(p.p_buy_limit > 0 and p_buy_limit < c.c_cnt ) or 
				(p.p_sale_type = 'T' and ( p_sale_sdate > curdate() or p_sale_edate < curdate() ) ) or 
				if( c_dateoption_use = 'Y', c_dateoption_date < curdate()  ,0) or

				if(
					c.c_pouid = 0,
					(if(c.c_old_price = p.p_price and c.c_supply_price = if(p_commission_type = '공급가', p.p_sPrice, (p.p_price-round(p.p_price*CONVERT(p.p_sPersent,DECIMAL )/100))) and c.c_cnt <= p.p_stock, 'Y', 'N')),
					(
						if(
							c.c_is_addoption != 'Y',
	...
	// LCY : 2023-01-19 : 중복구매 불가 상품 체크 및 처리 
	$clean_cart_duplicate_check = clean_cart_duplicate_check();

	// 변경된 상품 삭제 및 경고창 출력
	if(count($cr) > 0 || $clean_cart_duplicate_check > 0 ) {
		foreach($cr as $k=>$v) {
...
// 카트의 가격정보를 업데이트 한다 (그룹 혜택관련 정보)
function update_cart_update_check(){

	global $groupSetInfo;

	$res = _MQ_assoc("select  c.* , p_point_per , p_groupset_use , p_code , p_buy_limit from smart_product as p inner join smart_cart c on (c.c_pcode = p.p_code) where c.c_cookie = '".$_COOKIE["AuthShopCOOKIEID"]."' ");
	foreach($res as $k=>$v){

		// 현재개수
		$app_cnt = $v['c_cnt'];

		if(is_login() == true && $v['p_groupset_use'] == 'Y'  ){ // 로그인 중이고 등급할인혜택 적용이 Y 라면
			$c_old_price = $v['c_old_price'];
			$c_old_point = ( ($c_old_price*$app_cnt)*($v[p_point_per]/100) );
			$c_price = $c_old_price-getGroupSetPer( $c_old_price,'price',$v['p_code']);
			$c_point = $c_old_point + getGroupSetPer( ($c_old_price*$app_cnt),'point',$v['p_code']);
	    ...
	    $que = "update smart_cart set c_cnt='".$app_cnt."' ".$updateQue." where c_cookie = '".$_COOKIE["AuthShopCOOKIEID"]."' and c_uid = '".$v['c_uid']."' ";
		_MQ_noreturn($que);
...
// LCY : 2023-01-19 : 중복구매 불가 상품 체크 및 처리 
function clean_cart_duplicate_check(){

	if( is_login() !== true){ return false; }

	$res = _MQ_assoc("select p_code from smart_product as p left join smart_cart as c on(c.c_pcode = p.p_code) where c.c_cookie = '{$_COOKIE['AuthShopCOOKIEID']}' and p_duplicate_use = 'N'  ");
	...
	foreach($res as $k=>$v){
		...
		$orderCheckTmp = _MQ_result("
			...
			where op.op_pcode ='".$v['p_code']."' and op.op_cancel = 'N'
		");
		if($orderCheckTmp > 0) {
			// 일반건 
			$clean_pcode[] = $v['p_code'];
		}
		else{
	...
	if( count($clean_pcode) > 0){
		_MQ_noreturn(" delete from smart_cart where c_pcode in('".implode("','",$clean_pcode)."') and c_cookie = '{$_COOKIE['AuthShopCOOKIEID']}' ");
	}
		

	return count($clean_pcode);
}
...
function boardAuthChk($_uid,$_type)
{
	...
	if( is_admin($ONLY_USER) === true){ return true; }

	$auth_value = $row['bi_auth_'.$_type] == '' ? 0 : $row['bi_auth_'.$_type];
	switch($auth_value){
		case "9": // 관리자라면, 사용자,통합관리자 의 권한을 체크
			if( is_admin($ONLY_USER) !== true){ return array('code'=>'9996','msg'=>'관리자 이용 가능한 기능입니다.'); }
			else {return true; }
			break;
...
function postSecretChk($_uid)
{
	...
	if( is_admin($ONLY_USER) === true){ return true; } // 관리자는 무조건 권한이 있다
...
function link_credit_receipt($ordernum, $text='[전표보기]') {
	...
	switch($siteInfo[s_pg_type]) {
		...
		case "kcp":
			return "
			<div class='receipt_print'><a href='#none' class='c_btn h25 black line' onclick=\"showReceipt( '".$tmp[oc_tid]."', '".$ordernum."', '".$ordr[o_price_real]."' );return false;\">".$text."</a></div>
			<script>
		...
		case "inicis":
			return "
			<div class='receipt_print'><a href='#none' class='c_btn h25 black line' onclick=\"showReceipt('".$tmp[oc_tid]."');return false;\">".$text."</a></div>
			<script>
		...
		case "lgpay":
			...
			if($receiptUrl <> ''){
				return "
				<div class='receipt_print'><a href='#none' class='c_btn h25 black line' onclick=\"showReceipt('".$receiptUrl."');return false;\">".$text."</a></div>
				<script>
					function showReceipt(noTid) {
...
function odtFileUpload($key,$table,$tableUid)
{
	...
	if( count($_files) > 0){
		foreach($_files['name'] as $k=>$v){
			...
			if($_files["size"][$k] > 0){
				...
				// LCY : 2022-08-30 : 파일업로드 보완 패치 -- continue;
				if( !preg_match("/".implode("|",$arrUpfileConfig['ext']['all'])."/i" , $app_ext) ) { $errorMsg[] = '등록불가 확장자'; continue; }
	...
	else{ return $errorMsg; } // 에러디버깅이 필요할 경우 해당 변수를 이용하여 처리
}


/*
	-- KAY :: 2023-04-13 :: 상품 진열설정 초기화
	-- 스킨변경에 따른 기본값 업데이트
	-- 함수 실행시점 => 스킨변경 시
*/
function updateSkinPro()
{
	...
	//KAY :: 2023-04-12 :: 상품진열 업데이트 추가 
	// 카테고리 베스트 (목록,목록베스트)
	// -- 카테고리에 상품 진열 :: 기본값으로 초기화
	_MQ_noreturn("update smart_category set c_list_product_display = '".$updateSkinInfo['category']['pc_list_depth_default']."', c_list_product_mobile_display = '".$updateSkinInfo['category']['mo_list_depth_default']."' ,  c_best_product_display = '".$updateSkinInfo['category']['pc_list_best_depth_default']."', c_best_product_mobile_display = '".$updateSkinInfo['category']['mo_list_best_depth_default']."' ");


	// md픽 메인/ 목록
	_MQ_noreturn("update 
								smart_display_main_set set 
									dms_list_product_display = '".$updateSkinInfo['category']['pc_list_md_depth_default']."', 
									dms_list_product_mobile_display = '".$updateSkinInfo['category']['mo_list_md_depth_default']."' ,
									dms_main_product_display = '".$updateSkinInfo['category']['pc_main_md_depth_default']."', 
									dms_main_product_mobile_display = '".$updateSkinInfo['category']['mo_main_md_depth_default']."' 
								where dms_type = 'md' 
							");
	...
	_MQ_noreturn("update 
								smart_display_main_set set 
									dms_list_product_display = '".$updateSkinInfo['category']['pc_main_pro_depth_default']."', 
									dms_list_product_mobile_display = '".$updateSkinInfo['category']['mo_main_pro_depth_default']."' 
								where dms_type = 'main' 
							");

	// -- 타입별 진열 :: 기본값을 초기화
	_MQ_noreturn("update smart_display_type_set set dts_list_product_display = '".$updateSkinInfo['category']['pc_type_list_depth_default']."', dts_list_product_mobile_display = '".$updateSkinInfo['category']['mo_type_list_depth_default']."'  ");

	// -- 기획전, 브랜드, 검색상품 ,타임세일 (메인,목록) , 메인 베스트 카테고리, 상품상세
	//  진열 기본값으로 초기화
	_MQ_noreturn("update 
								smart_setup set 
									s_search_display = '".$updateSkinInfo['category']['pc_search_depth_default']."' ,
									s_search_mobile_display = '".$updateSkinInfo['category']['mo_search_depth_default']."' ,
									s_main_best_display = '".$updateSkinInfo['category']['pc_main_best_depth_default']."' ,
									s_main_best_mobile_display = '".$updateSkinInfo['category']['mo_main_best_depth_default']."' ,
									s_brand_display = '".$updateSkinInfo['category']['pc_brand_depth_default']."' ,
									s_brand_mobile_display = '".$updateSkinInfo['category']['mo_brand_depth_default']."' ,
									s_promotion_display = '".$updateSkinInfo['category']['pc_promotion_depth_default']."' ,
									s_promotion_mobile_display = '".$updateSkinInfo['category']['mo_promotion_depth_default']."' ,
									s_relation_display = '".$updateSkinInfo['category']['pc_relation_depth_default']."' ,
									s_relation_mobile_display = '".$updateSkinInfo['category']['mo_relation_depth_default']."' ,
									s_main_timesale_display = '".$updateSkinInfo['category']['pc_main_ts_depth_default']."' ,
									s_main_timesale_mobile_display = '".$updateSkinInfo['category']['mo_main_ts_depth_default']."' ,
									s_list_timesale_display = '".$updateSkinInfo['category']['pc_list_ts_depth_default']."' ,
									s_list_timesale_mobile_display = '".$updateSkinInfo['category']['mo_list_ts_depth_default']."' 
								where s_uid = '1' 
							");
...
function getGroupSetPer($price,$_type,$pcode=false)
{
	...
	function PromotionEventDeliveryChk($_mode='order')
	{
		...
		// LCY : 2022-12-21 : 무료배송이벤트 기능삭제 패치 -- 무조건 미사용 처리
		return false; 
	...
	*/
	...
	/*LCY::COUPON -- 쿠폰발급체크
		...
		- 다운로드형 쿠폰일경우 반드시 상품코드가 있어야한다.  // LCY : 2023-01-18 : 쿠폰추가적용 -- 다운로드형은 삭제 
...
function product_option_validate_check($pcode){
	...
	// LCY : 2024-01-09 : 옵션이없는경우 보완
	if( $res['p_chk'] > 0){
		// 옵션중 0원 상품이 있는경우 업데이트
		$zero_price_cnt = _MQ_result("SELECT	count(*) as cnt FROM smart_product_option where po_pcode ='".$pcode."' and po_depth ='".$res['max_depth']."' AND po_poptionprice <= 0 ");
	}
	...
	if( ($res['max_depth'] < $res['p_chk'] && $res['p_option_valid_chk'] <> 'N' ) || $zero_price_cnt['cnt']>0 ){
...
function editor_img_del($unique_num , $table) {
	...
	foreach($res as $k=>$v){
		_MQ_noreturn("
			UPDATE smart_editor_images_files SET
				eif_use_cnt = (SELECT COUNT(*) AS cnt FROM smart_editor_images_use WHERE eiu_eifuid ='{$v['eiu_eifuid']}'),
				eif_rdate = now()
			WHERE
				eif_uid = '". $v['eiu_eifuid'] ."'
		");
...
// LCY : 2021-07-09 : 디버깅 툴, // LCY : 2023-05-26 : 푸시개선작업 -- {
/*
    ex) insdebug('key:text','test');
    ex) insdebug('key:array',$_SERVER);
    ex) insdebug('key:objedt',json_encode($_SERVER));
    
    ※ $tracking_use는 기본 false, -> 상황에 따라 많은 데이터가 저장될 수 있다.. 필요할 경우 true
*/
if( !function_exists('insdebug') ) {
    function insdebug($key = false,$content = false ,$tracking_use = false,$backtrace = array()){

        global $DeveMode;

        // 시스템/개발모드 제어 --> 개발모드할경우 아래 주석 해제
        //if( $DeveMode !== true) return false;

        if(empty($key)) return false;
        if( !empty($content) &&  ( is_array($content)  || is_object($content)) ){ob_start(); viewarr($content);$content = ob_get_clean(); }

        $file = addslashes($_SERVER['SCRIPT_FILENAME']);

        // 트래킹 디버깅 내용 출력 - 내용이 많을 경우 용량상 문제 있음
        $_tracking = '';
        if( function_exists('debug_backtrace') ){
        	$backtrace = count($backtrace) > 0 ? $backtrace : debug_backtrace();
        	if( $tracking_use === true){
        		ob_start(); viewarr($backtrace); $_tracking = ob_get_clean();
        	}

        	// 트래킹 미사용시
        	else{
	        	$file = ($backtrace[0]['file'] != '' ? $backtrace[0]['file'] : $file).($backtrace[0]['line'] != '' ? ':'.$backtrace[0]['line']:null);
        	}
        }

        // 자동 인스톨 처리
        $InstallCK = _MQ_query(' desc insdebug_tb ');
        if(!@_MQ_num_rows($InstallCK)) {
            _MQ_noreturn("
                CREATE TABLE `insdebug_tb` (
                `d_uid` INT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
                `d_key` VARCHAR( 50 ) NULL COMMENT '검색 키',
                `d_content` TEXT NULL COMMENT '내용 ',
                `d_tracking` TEXT NULL COMMENT '실행추적(debug_backtrace() 참고)  ',
                `d_ip` VARCHAR( 20 ) NULL COMMENT '아이피',
                `d_file` VARCHAR( 255 ) NULL COMMENT '실행된 파일 경로',
                `d_rdate` DATETIME NOT NULL COMMENT '등록일'
                ) ENGINE = MYISAM COMMENT = '디버깅 :: lib.func.php 에 insdebug 함수 필요';
            ");
        }

        // 저장
        _MQ_noreturn("insert insdebug_tb set d_key = '".addslashes($key)."' , d_content = '".addslashes($content)."', d_tracking = '".addslashes($_tracking)."', d_ip = '".addslashes($_SERVER['REMOTE_ADDR'])."' , d_file = '".$file."', d_rdate = now() "); 
        return true;
    }
}
// LCY : 2023-05-26 : 푸시개선작업 -- }

// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 인증 정책 { 
	/*  
		ALTER TABLE `smart_setup` ADD `member_login_time` INT NOT NULL DEFAULT '60' COMMENT '로그인 시도 횟수가 넘을 경우 중지될 시간초'
	*/
	// DB생성
	function create_apply_auth_count_db() {

		if( !IsField('smart_setup','member_login_time')){
			_MQ_noreturn("ALTER TABLE `smart_setup` ADD `member_login_time` INT NOT NULL DEFAULT '60' COMMENT '로그인 시도 횟수가 넘을 경우 중지될 시간초'");		
		}

		if( !IsTable('smart_apply_auth_count')){
			_MQ_noreturn("
				CREATE TABLE `smart_apply_auth_count` (
					`aac_uid` INT(11) NOT NULL AUTO_INCREMENT,
					`aac_type` VARCHAR(50) NULL DEFAULT NULL COMMENT '타입 고유값정의 (필수)' COLLATE 'utf8_general_ci',
					`aac_key` VARCHAR(50) NULL DEFAULT NULL COMMENT '키값정의(필수)' COLLATE 'utf8_general_ci',
					`aac_count` INT(11) NULL DEFAULT NULL COMMENT '카운팅(duplicate key update  에 의해 증가)',
					`aac_udate` DATETIME NULL DEFAULT NULL COMMENT '업데이트일-최종 업데이트 시간기준',
					`aac_rdate` DATETIME NULL DEFAULT NULL COMMENT '등록일',
					PRIMARY KEY (`aac_uid`) USING BTREE,
					UNIQUE INDEX `aac_type_ac_key` (`aac_type`, `aac_key`) USING BTREE
				)
				COMMENT='특정 프로그램 시도 횟수 카운팅 DB'
	...
	// 추가
	function insert_apply_auth_count($type = false,$key = false) {
		if( empty($type) || empty($key)){ return false; }
		create_apply_auth_count_db(); // 테이블 생성 
		
		$key= addslashes($key);
		_MQ_noreturn("INSERT INTO smart_apply_auth_count(aac_type,aac_key, aac_count , aac_rdate) VALUES('".$type."','".$key."',1,now()) ON DUPLICATE KEY UPDATE aac_count = aac_count+1 , aac_udate = NOW()  ");
		return true;
	}
	// 조회
	function get_apply_auth_count($type = false,$key = false) {
		if( empty($type) || empty($key)){ return false; }
		create_apply_auth_count_db(); // 테이블 생성 
		
		$key= addslashes($key);
		$row = _MQ("SELECT * FROM smart_apply_auth_count where aac_type = '".$type."' and aac_key = '".$key."' ");
		return $row;
	}
	// 초기화
	function init_apply_auth_count($type = false,$key = false) {
		if( empty($type) || empty($key)){ return false; }
		create_apply_auth_count_db(); // 테이블 생성 

		$key= addslashes($key);
		_MQ_noreturn("UPDATE smart_apply_auth_count SET aac_count = 0 , aac_rdate = NOW() WHERE aac_type = '".$type."' and aac_key = '".$key."'   ");
...
// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 세션 보안 {
	// DB생성
	function create_secure_token_db() {
		if( !IsTable('smart_secure_token')){
			_MQ_noreturn("
				CREATE TABLE `smart_secure_token` (
					`st_uid` INT(11) NOT NULL AUTO_INCREMENT COMMENT '고유번호',
					`st_type` VARCHAR(50) NULL DEFAULT NULL COMMENT '타입' COLLATE 'utf8_general_ci',
					`st_key` VARCHAR(255) NULL DEFAULT NULL COMMENT '키값' COLLATE 'utf8_general_ci',
					`st_checksum` VARCHAR(255) NULL DEFAULT NULL COMMENT '체크섬(체크가 필요한 값을 추가로 넣어준다)' COLLATE 'utf8_general_ci',
					`st_udate` DATETIME NULL DEFAULT NULL COMMENT '업데이트(여유분필드)',
					`st_rdate` DATETIME NULL DEFAULT NULL COMMENT '등록일',
					PRIMARY KEY (`st_uid`) USING BTREE,
					UNIQUE INDEX `st_type_st_key` (`st_type`, `st_key`) USING BTREE,
					INDEX `st_type` (`st_type`) USING BTREE
				)
				COLLATE='utf8_general_ci'
				ENGINE=MyISAM
				;
	...
	// 추가
	function insert_secure_token($type = false,$key = false, $checksum = '') {
		if( empty($type) || empty($key)){ return false; }
		create_secure_token_db(); // 테이블 생성 

		$key= addslashes($key);
		if( $checksum != ''){ $checksum= addslashes($checksum); }

		_MQ_noreturn("INSERT INTO smart_secure_token(st_type,st_key, st_checksum , st_rdate) VALUES('".$type."','".$key."',".($checksum != '' ? "'".$checksum."'":"NULL").",now()) ON DUPLICATE KEY UPDATE 
			st_checksum = ".($checksum != '' ? "'".$checksum."'":"NULL")." , st_udate = NOW()
		");
		return true;
	}
	// 조회
	function get_secure_token($type = false,$key = false , $checksum = '') {
		if( empty($type) || empty($key)){ return false; }
		create_secure_token_db(); // 테이블 생성 
		...
		$sque = "";
		if($checksum != ''){
			$sque .= " and st_checksum = '".$checksum."' ";
		}

		$row = _MQ("SELECT st_key FROM smart_secure_token where st_type = '".$type."' and st_key = '".$key."' ".$sque);
		return $row['st_key'];
	}
	// 삭제 -- 삭제시에는 checksum 을 하지 않는다.
	function delete_secure_token($type = false,$key = false) {
		if( empty($type) || empty($key)){ return false; }
		create_secure_token_db(); // 테이블 생성 
		...
		// 삭제한다.
		_MQ_noreturn("DELETE FROM smart_secure_token WHERE st_type = '".$type."' and st_key = '".$key."'  ");
...
// 티켓: 상태값을 통한 티켓업데이트
function update_order_ticket_status($opt_uid = '',$app_status = ''){
	if( empty($opt_uid) || empty($app_status) ){ return false; }

	if( $app_status == '사용'){
		$ticket_log = date('Y-m-d H:i:s')." - 관리자 티켓 사용처리\n";
		_MQ_noreturn(" update smart_order_product_ticket set  opt_status  = '사용', opt_udatetime = now() , opt_log = ( if( opt_log is null , '".$ticket_log."' , CONCAT(opt_log,'".$ticket_log."') )  ) , opt_update_check = 'Y'  where opt_uid = '".$opt_uid."'    ");

	}
	else if($app_status == '대기'){
		$ticket_log = date('Y-m-d H:i:s')." - 관리자 티켓 대기처리\n";
		_MQ_noreturn(" update smart_order_product_ticket set  opt_status  = '대기' , opt_udatetime = '0000-00-00 00:00:00' , opt_log = ( if( opt_log is null , '".$ticket_log."' , CONCAT(opt_log,'".$ticket_log."') )  ) , opt_update_check = 'Y'  where opt_uid = '".$opt_uid."'    ");		
	}
	else if($app_status == '취소'){

		if(is_admin() === true){ 
			$ticket_log = date('Y-m-d H:i:s')." - 관리자 티켓 취소\n";
		}else{
			$ticket_log = date('Y-m-d H:i:s')." - 자동 또는 사용자 티켓 취소\n";
		}
		_MQ_noreturn("update smart_order_product_ticket set opt_status = '취소' , opt_log = ( if( opt_log is null , '".$ticket_log."' , CONCAT(opt_log,'".$ticket_log."') )  ),  opt_cdatetime = now() , opt_update_check = 'Y' where opt_uid = '".$opt_uid."' ");
	}

	return true;
}

// 티켓: 기간에 따른 티켓 자동으로 업데이트 처리 ($opt_uid 있을 경우 해당 값에 대해서 처리)
function update_order_ticket_expiration($opt_uid = ''){
	...
	if( !empty($opt_uid )){ 
		$_trigger = true;
		$s_query .= " and opt_uid = '".$opt_uid."' "; 
		$row = _MQ("select * from smart_order_product_ticket where opt_uid = '".$opt_uid."' ");
	}

	// 트리거가 없거나, 트리거가 있고, 상태값이 대기이고, 티켓 유효기간이 지났을 경우 
	if( $_trigger === false || ($_trigger === true && $row['opt_status'] == '대기' && $row['opt_expire_date'] < date('Y-m-d') ) ){

		// 대기 티켓중 기간지난 티켓은 만료처리 
		$ticket_log = date('Y-m-d H:i:s')." - 티켓 만료처리(자동업데이트)\n";
		_MQ_noreturn("update smart_order_product_ticket set opt_status = '만료', opt_log = ( if( opt_log is null , '".$ticket_log."' , CONCAT(opt_log,'".$ticket_log."') )  )   where opt_expire_date < curdate() and opt_status = '대기' ".$s_query);
	}

	// 트리거가 없거나, 트리거가 있고, 상태값이 만료이고, 티켓 유효기간이 남았을 경우 
	if( $_trigger === false || ($_trigger === true && $row['opt_status'] == '만료' && $row['opt_expire_date'] >= date('Y-m-d') ) ){
		$ticket_log = date('Y-m-d H:i:s')." - 티켓 만료복구처리(자동업데이트)\n";
		_MQ_noreturn("update smart_order_product_ticket set opt_status = '대기', opt_log = ( if( opt_log is null , '".$ticket_log."' , CONCAT(opt_log,'".$ticket_log."') )  )  where opt_expire_date >= curdate() and opt_status = '만료' ".$s_query);
	}
...
// 이전에 등록해둔 타임세일 상품이 타이머 작동하지않고 끝났을경우 숨김처리 필요
function update_time_sale_view(){
	
	// 판매기간이 기간판매일 경우 타임세일 무력화 
	_MQ_noreturn("update smart_product set p_time_sale = 'N' where p_sale_type = 'T' and p_time_sale = 'Y' ");

	$arr_timesale_pinfo ='';
	$arr_timesale_pinfo = _MQ_assoc("select p_code from smart_product where p_time_sale = 'Y' and concat(p_time_sale_sdate,' ', p_time_sale_sclock) <= now() AND concat(p_time_sale_edate,' ', p_time_sale_eclock) <= now() and p_view='Y' ");

	if(count($arr_timesale_pinfo) >0){
		foreach($arr_timesale_pinfo as $tk=>$tv){
			_MQ_noreturn("update smart_product set p_view = 'N' where p_code = '".$tv['p_code']."'  ");
		}
	}
	return true;
}
...
// KAY :: 2023-03-16 :: 에디터 이미지 사용 개수 체크
function edit_img_cnt($datauid,$table){
	$edit_img_cnt = _MQ("select count(*) as cnt from smart_editor_images_use where eiu_tablename='".$table."' and eiu_datauid='".$datauid."' ");
	return $edit_img_cnt;
}
...
if( !function_exists('creat_num_uniqueID') ) {
	...
	function creat_num_uniqueID($prefix="",$length = 12){
		...
		if( $idChk['cnt'] > 0 ){
			$id = creat_num_uniqueID($prefix="",$length = 12);
		}
		return $id ;
	}
}
...
// LCY : 2023-05-26 : 푸시개선작업 -- {
if( function_exists('app_debug') == false){
	function app_debug($msg,$return =false,$Debug = false)
	{
		global $VARAPP;
		insdebug('error',array($msg), false, debug_backtrace()); // 디버깅은 무조건 남긴다.
		if($Debug === false) return $return;
		viewarr($msg);
		return $return;
	}
}
...
function update_cart_check($_cookie = ''){
	if( empty($_cookie) ){ return false; } 
	// LCY 장바구니 중복 체크하여 수량 증가
	$r = _MQ_assoc(" select c_cookie, c_pcode, c_pouid, c_dateoption_date, c_dateoption_use,  sum(c_cnt) as sum, IFNULL(sum(c_point),0) as sum_point, count(*) as cnt from smart_cart where c_cookie = '".$_cookie."' group by c_cookie, c_pcode, c_pouid, c_dateoption_date, c_dateoption_use having cnt > 1 ");
	if( count($r) < 1){ return false; }
	foreach( $r as $k=>$v ){
		$p = _MQ(" select p_stock from smart_product where p_code = '".$v[c_pcode]."' ");
		 $v[sum] = $v[sum] > $p[p_stock] ? $p[p_stock] : $v[sum];

		if( $v['c_dateoption_date'] != '' && $v['c_dateoption_use'] == 'Y'){
			// LCY : 2022-12-21 : 티켓기능 -- 달력옵션
			$add_que_date = " and c_dateoption_date = '".$v['c_dateoption_date']."' and c_dateoption_use = '".$v['c_dateoption_use']."'  "; // 같은 날짜의 경우 제외한다.
			// LCY : 2022-12-21 : 티켓기능 -- 달력옵션
		}
		 $sr = _MQ(" select c_uid from smart_cart where c_cookie = '". $v['c_cookie'] ."' and c_pcode = '". $v['c_pcode'] ."' and c_pouid = '". $v['c_pouid'] ."' ".$add_que_date." order by c_uid desc limit 1 ");
		_MQ_noreturn(" update smart_cart set c_cnt = '".$v[sum]."' , c_point = '".$v[sum_point]."' where c_uid = '".$sr['c_uid']."' and c_cookie = '".$v[c_cookie]."' ");
		_MQ_noreturn(" delete from smart_cart where c_cookie = '".$v[c_cookie]."' and c_pcode  = '".$v[c_pcode]."' and c_pouid = '".$v[c_pouid]."' and c_uid != '".$sr['c_uid']."' ".$add_que_date." ");
	}
	return true;
}