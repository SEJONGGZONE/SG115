...
# 첨부파일 확인
if($_FILES['excel_file']['size'] <= 0) error_loc_msg("_product_ticket.upload.pop.php?", "첨부파일이 없습니다.");
...
// ----- 티켓유효기간 처리 -----
$arr_excel_expire_type = array('없음'=>'','만료일'=>'day','만료날짜'=>'date');
...
foreach($Excel as $key=>$val) {
	...
	// ----- 티켓유효기간 처리 -----
	$val['p_expire_type'] = $arr_excel_expire_type[trim($val['p_expire_type'])];
	$val['p_expire_day'] = rm_str(trim($val['p_expire_day']));
	$val['p_expire_date'] = rm_str($val['p_expire_date']) > 0 ?  date('Y-m-d',strtotime(str_replace(".", "-", $val['p_expire_date']))) : '';
	...
	// 달력옵션 사용여부
	$val['p_dateoption_use']=$val['p_dateoption_use']=='사용'?'Y':'N';

	// 달력옵션 일자,날짜지정 선택
	if($val['p_dateoption_date_type']=='일자지정'){
		$val['p_dateoption_date_type']='day';
		$val['p_dateoption_sdate'] = $val['p_dateoption_edate']='';
	}else{
		$val['p_dateoption_date_type']='date';
		$val['p_dateoption_sday'] = $val['p_dateoption_eday']='';
		$val['p_dateoption_sdate'] = rm_str($val['p_dateoption_sdate']) > 0 ?  date('Y-m-d',strtotime(str_replace(".", "-", $val['p_dateoption_sdate']))) : '';
		$val['p_dateoption_edate'] = rm_str($val['p_dateoption_edate']) > 0 ?  date('Y-m-d',strtotime(str_replace(".", "-", $val['p_dateoption_edate']))) : '';
	}
	
	$val['p_sale_sdate'] = rm_str($val['p_sale_sdate']) > 0 ?  date('Y-m-d',strtotime(str_replace(".", "-", $val['p_sale_sdate']))) : '';
	$val['p_sale_edate'] = rm_str($val['p_sale_edate']) > 0 ?  date('Y-m-d',strtotime(str_replace(".", "-", $val['p_sale_edate']))) : '';

	$arr_dateoption_ex_week = explode(',',$val['p_dateoption_ex_week']);
	$arr_dateoption_ex_week = array_filter($arr_dateoption_ex_week);
	foreach($arr_dateoption_ex_week as $dewk=>$dewv){
		$arr_dateoption_im_week[] = $arr_day_week_day[$dewv];
	}
	$val['p_dateoption_ex_week'] = implode(',',$arr_dateoption_im_week);

	// 좌표 설정 
	$val['p_apply_mapxy']=$val['p_apply_mapxy']=='사용'?'Y':'N';
	...
	$s_query = "
		...
		p_expire_type				= '" . $val['p_expire_type'] . "' ,
		p_expire_day				= '" . $val['p_expire_day'] . "',
		p_expire_date				= '" . $val['p_expire_date'] . "',
		p_buy_limit					= '".$val['p_buy_limit']."',
		p_duplicate_use			= '".$val['p_duplicate_use']."',
		p_dateoption_use			= '".$val['p_dateoption_use']."',
		p_dateoption_sday			= '".$val['p_dateoption_sday']."',
		p_dateoption_eday			= '".$val['p_dateoption_eday']."',
		p_dateoption_sdate			= '".$val['p_dateoption_sdate']."',
		p_dateoption_edate			= '".$val['p_dateoption_edate']."',
		p_dateoption_stime			= '".$val['p_dateoption_stime']."',
		p_dateoption_etime			= '".$val['p_dateoption_etime']."',
		p_com_juso					= '".$val['p_com_juso']."',
		p_apply_mapxy					= '".$val['p_apply_mapxy']."',
		p_com_mapx					= '".$val['p_com_mapx']."',
		p_com_mapy					= '".$val['p_com_mapy']."',
		p_com_locname					= '".$val['p_com_locname']."',
		p_com_tel					= '".$val['p_com_tel']."',
		p_dateoption_ex_date	= '".$val['p_dateoption_ex_date']."',
		p_dateoption_ex_week	= '".$val['p_dateoption_ex_week']."',
