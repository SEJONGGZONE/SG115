<?php
	...
	// LCY : 2022-12-28 : 이니시스TX모듈 교체
	 if( IsField('smart_setup','s_pg_apikey') == false){
	     _MQ_noreturn("ALTER TABLE `smart_setup` ADD COLUMN `s_pg_apikey` VARCHAR(255) NULL DEFAULT NULL COMMENT '이니시스 API 취소키', ADD COLUMN `s_pg_escrow_apikey` VARCHAR(255) NULL DEFAULT NULL COMMENT '이니시스 에스크로 API 키';");
	     $siteInfo = get_site_info();
	 }

	if( in_array($_pg_type, array_keys($arr_pg_type)) == false){ $_pg_type = ''; } // 기본제공되는 PG 이외것
...
<script>
	...
	// -- 저장
	function pgSubmit(){	$('#frmPg').submit();		}
