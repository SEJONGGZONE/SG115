<?PHP
	...
	if( in_array( $_mode , array("add" , "modify") ) ){
		...
		$_main_view = $_main_view=='Y'?'Y':'N';

		// KAY :: 2022-12-28 :: 메인노출은 1개만 가능
		if($_main_view=='Y'){
			$main_view_chk = _MQ("select count(*) as cnt from smart_bbs_info where bi_main_view='Y' and bi_view_type='service' ");

			if($main_view_chk['cnt']>0){
				_MQ_noreturn(" update smart_bbs_info set bi_main_view='N' where bi_main_view='Y' and bi_view_type='service' ");
			}
		}
	...
	switch( $_mode ){
		...
		case "delete":
			...
			if($r['bi_main_view']=='Y'){	error_msg("본 게시판은 고객센터 메인노출로 설정된 게시판으로 삭제가 불가능합니다.");}
