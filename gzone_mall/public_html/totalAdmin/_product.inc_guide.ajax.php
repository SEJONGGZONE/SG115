...
?>
	<div class="tab_conts" data-idx="<?php echo $k; ?>" style="<?php echo ($is_hit?null:'display:none;'); ?>">
        <div class="lineup-row">
            <!-- 사용여부/입력방식 선택 -->
            <?php echo _InputRadio('p_guide_type_'.$k, array('none' , 'manual' , 'list'), ($row['p_guide_type_'.$k] ? $row['p_guide_type_'.$k] : 'none'), ' class="js_chg_guide_type" data-idx="'. $k .'" ', array('미사용' , '직접입력' , '선택입력')); ?>
			...
			<a href="/<?php echo $AdminPath; ?>/_product.guide.list.php" class="c_btn sky line" target="_blank">이용안내 관리 바로가기</a>
        </div>
