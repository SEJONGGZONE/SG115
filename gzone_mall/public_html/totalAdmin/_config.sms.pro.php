<?PHP
	...
	$c_status = $c_status!='Y'?'N':'Y';
	$ck_status = ($ck_status?$ck_status:'N');
	...
	if( $SubAdminMode === true){
		/***** 입점관리자 발송 설정값 입력 *****/ unset($tmp,$s_query,$_file);
		$tmp = _MQ("select * from smart_sms_set where ss_uid = 'com_".$uid."'");
		$s_query = "
			...
			, kakao_status = '{$ck_status}'
			, kakao_templet_num = '{$ck_kakao_templet_num}'
			, kakao_add1 = '{$ck_kakao_add1}'
			, kakao_add2 = '{$ck_kakao_add2}'
			, kakao_add3 = '{$ck_kakao_add3}'
			, kakao_add4 = '{$ck_kakao_add4}'
			, kakao_add5 = '{$ck_kakao_add5}'
			, kakao_add6 = '{$ck_kakao_add6}'
			, kakao_add7 = '{$ck_kakao_add7}'
			, kakao_add8 = '{$ck_kakao_add8}'
		";
		if($tmp[ss_uid]) {
			if(!$_FILES['c_file'] && !$c_file_OLD) { _PhotoDel( $app_path , $tmp['ss_file'] ); $_file = ''; }
			else { $_file = _PhotoPro( $app_path , 'c_file' ); }
			$s_query .= " , ss_file = '".$_file."' ";
			_MQ_noreturn(" update smart_sms_set set " . $s_query . " where ss_uid = 'com_".$uid."' ");
		} else {
			$_file = _PhotoPro( $app_path , 'c_file' );
			$s_query .= " , ss_file = '".$_file."' , ss_uid = 'com_".$uid."' ";
			_MQ_noreturn(" insert smart_sms_set set " . $s_query);
		}
	}
