<?php include_once('wrap.header.php');
	...
	// AJAX데이터 서포트 
	if($viewUid != ''){
		$row = _MQ("select *from smart_category where c_uid = '".$viewUid."'  ");

		$app_uid = $_uid = $viewUid;
		$app_depth = $row['c_depth'];
		...
		switch($row['c_depth'])
		{
			...
			break;
			case "2":
				$locUid1 = $row['c_parent'];
				$locUid2 = $row['c_uid'];
			break;

			case "3":
				$app_parent = explode(",",$row['c_parent']);
				$locUid1 = $app_parent[0];
				$locUid2 = $app_parent[1];
				$locUid3 = $row['c_uid'];
			break;
...
?>
		...
		<form name="formCategoryData">
			...
			<input type="hidden" name="locUid3" value="<?=$locUid3?>"> <!-- 선택된 2차카테고리 -->
		</form>
		<!-- ● 카테고리설정 -->
		<div class="category">
			<ul class="table">
				<li class="td">
					<div class="depth_tt"><strong>1차 메뉴</strong><a href="#none" onclick="viewCategoryForm('add','1',''); return false;" class="c_btn h23 blue scrollto" data-scrollto="view-form">추가</a></div>
				...
				<li class="td">
					<div class="depth_tt"><strong>2차 메뉴</strong><a href="#none" onclick="viewCategoryForm('add','2',''); return false;" style="<?=$locUid1 == '' ? 'display:none;':''?>" class="c_btn h23 blue scrollto add-category" data-scrollto="view-form" data-depth="2">추가</a></div>
				...
				<li class="td">
					<div class="depth_tt"><strong>3차 메뉴</strong><a href="#none" onclick="viewCategoryForm('add','3',''); return false;" style="<?=$locUid2 == '' ? 'display:none;':''?>" class="c_btn h23 blue scrollto add-category" data-scrollto="view-form" data-depth="3">추가</a></div>
		...
		<?php // -- 메뉴 클릭 시 폼 ajax -- ?>
		<div class="view-category-form" data-name="view-form">
		<?php 
			// AJAX데이터 서포트 
			if( $app_uid > 0 ) {
				$_mode = 'modify'; 
				$_uid = $app_uid;
				$_depth = $app_depth;
				include dirname(__FILE__)."/_category.ajax_form.php";
			}
			?>
		</div>
		<?php // -- 메뉴 클릭 시 폼 ajax -- ?>
...
// -- 메뉴를 클릭 시 :: 3차메뉴는 제외
function viewCategoryList(_depth,_uid, cache )
{
	...
	if(_depth == 1){
		...
		$('form[name="formCategoryData"] [name="locUid3"]').val(_uid);
	}
	...
	$('.category-list-tr[data-depth="'+_depth+'"][data-uid="'+viewUid+'"]').addClass('hit');
		...
		if( typeof cache == 'undefined'){
			var locUid1 = $('form[name="formCategoryData"]').find('[name="locUid1"]').val();
			var locUid2 = $('form[name="formCategoryData"]').find('[name="locUid2"]').val();		
			var locUid3 = $('form[name="formCategoryData"]').find('[name="locUid3"]').val();		
			history.replaceState(null,null,"<?php echo $CURR_FILENAME ?>?locUid1="+locUid1+"&locUid2="+locUid2+"&locUid3="+locUid3+"&reload=true");		
		}
		// AJAX데이터 서포트 }
...
function viewCategoryForm(_mode,_depth,_uid)
{
	...
	if(_mode == 'add'){
		...
		$('.category-list-tr[data-depth="'+_depth+'"]').removeClass('hit');
		$('.category-list-tr[data-uid="'+_uid+'"]').addClass('hit');
		if( _depth != 3){ viewCategoryList(_depth,_uid,true); }
		// AJAX데이터 서포트 }
          ...
          success: function(data){
		      	if( data.rst == 'error'){
		      		...
		      		alert(data.msg);
							...
							// AJAX데이터 서포트 {
							var _uid = $('form[name="formCategory"]').find('[name="_uid"]').val();
							var _depth = $('form[name="formCategory"]').find('[name="_depth"]').val();
...
*/
	...
	// --- 페이지 네이트 ---
	$(document).on('click','.view-paginate .number a',function(){
	...
	$(document).on('click','.select-best-product-delete',function(){
		...
		if(confirm('선택된 베스트상품을 삭제 하시겠습니까?') == false){return false;}
	...
	function selectBestProductAddpop()
	{
		...
		window.open('_category.best_product.pop.php?pass_cuid='+cuid,'selectBestProductAddpop', 'width=1350, height=800, scrollbars=yes');
	}
