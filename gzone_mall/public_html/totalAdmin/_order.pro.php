<?PHP
	...
	if( in_array( $_mode , array("modify") ) ) {
		...
		if( in_array($row['o_order_type'], array('ticket','both')) > 0) {
			$_uname		= nullchk($_uname , "사용자 이름을 입력해주시기 바랍니다.");
			$_uhp		= nullchk($_uhp , "사용자 휴대폰번호을 입력해주시기 바랍니다.");	
			$_uemail	= nullchk($_uemail , "사용자 이메일을 입력해주시기 바랍니다.");
		}

		if( in_array($row['o_order_type'], array('delivery','both')) > 0) {
	...
	switch( $_mode ){
		...
		case "paycancel":
			...
			// 만약 티켓상품에 대한 주문을 막아야한다면? 
			if($row['o_order_type'] != 'delivery'){
				error_loc_msg("_order.form.php" . URI_Rebuild('?', array('view'=>$view, '_mode'=>'modify', '_ordernum'=>$_ordernum, '_PVSC'=>$_PVSC)), "티켓상품이 포함된 주문건은 입금취소가 불가능합니다.(주문취소만 가능)");
			}
		...
		case "delete":
			...
			error_loc_msg("_order.cancel_list.php?".enc('d' , $_PVSC), '정상적으로 삭제되었습니다.');
			break;
		...
		// 삭제
		case "mass_delete":
			if(count($chkVar) < 1) error_msg('삭제할 적립금 내역이 선택되지 않았습니다.');
			
			// 적립금 내역 삭제
			_MQ_noreturn("delete from smart_order_product where op_oordernum in ('". implode("','", $chkVar) ."') ");
			_MQ_noreturn("delete from smart_order where o_ordernum in ('". implode("','", $chkVar) ."') ");
		...
		case "mass_cancel":
            ...
            // 에러 메세지 추출
            $msg = '주문취소 처리가 완료되었습니다.';
            if(count($arr_error) > 0){
                if(count($chk_ordernum) == count($arr_error)){
                    $msg = '주문취소 시 오류가 발생하였습니다.\\n\\n'.implode("\\n", $arr_error);// --- JJC : 부분취소 개선 : 2021-02-10 ---
                }else{
                    $msg = '총 ['.count($chk_ordernum).']건의 주문중 ['.(count($chk_ordernum) - count($arr_error)).']건이 취소 되었습니다.\\n\\n- 취소실패 ['.count($arr_error).']건 -\\n'.implode("\\n", $arr_error);
                }
            }
...
<td>주문일시</td>
			...
			foreach($r as $k => $v) {
				$excel_print[] = "<td>".implode("</td><td>",$v)."</td>";
			}
