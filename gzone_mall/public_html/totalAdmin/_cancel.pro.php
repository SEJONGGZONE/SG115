<?php
	...
	$_POST['_mode'] = !empty($_POST['_mode']) ? $_POST['_mode'] : '';
	...
	switch($_mode) {
		...
		case "mass_del":
			unset($__trigger);
			foreach($OpUid as $k=>$v) {
				$data = "mode=restore&op_uid=" . $v;
				$res = json_decode(CurlPostExec($system['url'] . "/program/mypage.order.view.ajax.php" , $data));
				if($res->result != "OK") {$__trigger++;}
			}

			if($__trigger <= 0){ error_loc_msg("_cancel.list.php?_PVSC=${_PVSC}" , "선택된 부분취소 요청을 삭제하였습니다."); }
			else { error_loc_msg("_cancel.list.php?_PVSC=${_PVSC}" , "선택된 부분취소 요청이 삭제되지않았습니다."); }
