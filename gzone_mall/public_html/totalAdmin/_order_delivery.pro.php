...
if(in_array($_mode, array('get_excel', 'get_search_excel'))) { // Excel 다운로드 Start
	...
	# 모드별 쿼리 조건
	if($_mode == 'get_excel') $s_query = " and o.o_canceled != 'Y' and o.o_paystatus = 'Y' and `npay_order` = 'N' and o.o_ordernum in ('".implode("', '", $_uid)."') and o.o_order_type in('delivery','both') ";
...
<?php
	...
	$_oordernum = array();
	foreach($_uid as $k=>$v) {
		if(IN_ARRAY($select_sendstatus , array('배송대기' , '배송준비'))) $_oordernum[] = $v; // JJC : 배송대기/배송준비 시 택배사/송장번호 입력없이 변경 가능
	...
	foreach($EData as $k=>$v) {
		...
		if($v['_sendstatus'] == '배송중' || $v['_sendstatus'] == '배송완료') {
			if($v['_sendstatus'] == '배송준비' || $r['o_sendstatus'] == '배송대기' || !$r['o_sendstatus']) {
		...
		_MQ_noreturn("
			update smart_order_product set
				...
				, op_sendstatus = '{$v['_sendstatus']}'
				  {$que_product}
			where op_oordernum='{$r['o_ordernum']}' and op.op_ptype = 'delivery' /*티켓제외*/
		");

		// 문자발송
		if(($r['o_sendstatus'] == '배송대기' || $r['o_sendstatus'] == '배송준비') && ($v['_sendstatus'] == '배송중' || $v['_sendstatus'] == '배송완료')) {
