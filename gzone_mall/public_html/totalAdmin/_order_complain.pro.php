...
// - XSS 문제로 괄호 깨짐 현상 해결 -
// if(preg_match("/완료\/부분취소요청|PG연동/i" , $op_complain)) {
// 	$op_complain = '완료/부분취소요청(PG연동)';
// }
// else if(preg_match("/완료\/부분취소요청|적립금 환불/i" , $op_complain)) {
// 	$op_complain = '완료/부분취소요청(적립금 환불)';
// }

if(preg_match("/완료\/부분취소요청/i" , $op_complain)) {
	if(preg_match("/PG연동/i" , $op_complain)) {
		$op_complain = '완료/부분취소요청(PG연동)';
	}else if(preg_match("/완료\/부분취소요청/i" , $op_complain)) {
		$op_complain = '완료/부분취소요청(적립금 환불)';
	}
}
// - XSS 문제로 괄호 깨짐 현상 해결 -
