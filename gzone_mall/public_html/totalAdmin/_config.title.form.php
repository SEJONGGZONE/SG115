<?php
	...
	$arr_site_title_page = array(
		...
		'normal' => array(
			'name'=> '일반페이지',
			'list' => array(
				'/?pn=pages.view&type=pages&data=company' => array('name'=>'회사소개', 'default'=>'회사소개 - {사이트명}'),
...
<input type="hidden" name="menuUid" value="<?php echo $menuUid; ?>">
		...
		<?PHP
			if(sizeof($arr_site_title_page) > 0 ) {
				foreach( $arr_site_title_page as $k=>$v ){
					...
					echo '';
					echo '	<div class="group_title"><strong>'. $v['name'] .'</strong></div><table class="table_form type_column"><colgroup><col width="180"><col width="300"><col width="*"></colgroup><tbody>';
					// 세부목록
					$_idx = 0;
	...
	<div class="group_title">
		<strong>추가적용 페이지</strong>
		<div class="add_btn"><a href="#none" onclick="page_add();" class="c_btn h27 blue">페이지 추가하기</a></div>
	</div>

	<div class="data_form">
		<div ID="page_area" >

			<table class='table_form type_column'>
				<colgroup><col width="180"><col width="*"><col width="*"><col width="*"><col width="80"></colgroup>
				<tbody>
				<?PHP
					$_idx = -1;
					if(sizeof($arrTitleSet) > 0 ) {
						foreach( $arrTitleSet as $k=>$v ){
							...
							echo '
								<tr>
									<th><span class="num">'. ($_idx+1) .'</span></th>
									<td><input type="text" name="_name[]" value="'. stripslashes($v['sst_name']) .'" class="design js_input_name" style="width:100%" placeholder="페이지명"></td>
									<td><input type="text" name="_page[]" value="'. stripslashes($v['sst_page']) .'" class="design js_input_page" style="width:100%" placeholder="페이지 URL"></td>
									<td>
										<div class="js_drop_wrap">
											<input type="hidden" name="_uid[]" value="1">
									...
									<td><div class="lineup-column"><a href="#none" onclick="page_delete(this);" class="c_btn h22 dark line">삭제</a></div></td>
								</tr>
							';
						}
						echo "
	...
	<div class="group_title"><strong>참고사항</strong></div>
	<div class="data_form">
		<table class="table_form">
			<colgroup>
			...
			<tbody>
				<tr>
					<th>치환자 안내</th>
					<td>
						<div class="mobile_tip">치환자 드래그 기능은 모바일에서 제한적일 수 있습니다.</div>
						<div class="tip_box">
				...
				<tr>
					...
					<td>
						<div class="tip_box">
							...
							<?php echo _DescStr("추가적용할 페이지의 주소창에서 <em>도메인을 제외한 부분</em>을 모두 입력해주세요."); ?>
							<?php echo _DescStr("예) http://". $_SERVER['SERVER_NAME'] ."/?pn=member.login.form →  /?pn=member.login.form"); ?>
							<?php echo _DescStr("중복될 경우 나중에 등록된 내용으로 적용됩니다."); ?>
...
function page_add(){
	var _str = '';
	_str += '<tr>';
	_str += '	<th><span class="num">0</span></th>';
	_str += '	<td><input type="text" name="_name[]" value="" class="design js_input_name" style="width:100%" placeholder="페이지명"></td>';
	_str += '	<td><input type="text" name="_page[]" value="" class="design js_input_page" style="width:100%" placeholder="페이지 URL"></td>';
	_str += '	<td>';
	...
	_str += '		</div>';
	_str += '	</td>';
	_str += '	<td><div class="lineup-column"><a href="#none" onclick="page_delete(this);" class="c_btn h22 gray">삭제</a></div></td>';
	_str += '</tr>';
