<?php 
	...
	switch ($_mode) {
		...
		case 'delete': // 삭제 시
			...
			// -- 회원 키
			$arrKey = array('coup_uid'=>'쿠폰번호','coup_use'=>'쿠폰사용여부');

			if($ctrlMode == 'select'){
				$ctrlModeName = '선택';
				if( count($arr_coup_uid) < 1 ){ error_alt("삭제하실 회원의 쿠폰을 1개이상 선택해 주세요."); }
				...
				and find_in_set(coup_uid, '".implode(",",$arr_coup_uid)."' ) > 0  and coup_use != 'Y'
				 ";
				...
				$ctrlModeName = '검색';
				error_alt('실행이 올바르지 않습니다.');
			}


			$res = _MQ_assoc("select ".implode(",",array_keys($arrKey))."   ".$sque. '  '.$orderby);
			if( count($res) < 1 ){ error_alt('잘못된 접근입니다.'); }


			$couponSetData  = _MQ("select *from smart_individual_coupon_set where ocs_uid = '".$_uid."'    ");
			if( count($couponSetData) < 1){ error_alt('삭제할 회원의 쿠폰이 존재하지 않습니다.');  }

			$arrCoupUid = array();
			foreach($res as $k=>$v){
				if( $v['coup_use'] == 'Y'){ continue; }
				$arrCoupUid[] = $v['coup_uid'];
			}

			if( count($arrCoupUid) < 1){  error_alt("삭제가능한 발급 쿠폰이 존재하지 않습니다."); }


			
			_MQ_noreturn("delete from smart_individual_coupon where find_in_set(coup_uid , '".implode(",",$arrCoupUid)."' ) > 0 and coup_use != 'Y' ");

	
			error_frame_loc("_coupon.list.php".($_PVSC ? '?'.enc('d' , $_PVSC) : '?_uid='.$_uid));				
