...
if( in_array($pass_type , array("insert")) && $pass_depth && $pass_uid ) {
	...
	// LCY : 2023-11-17 : 옵션 오류 체크 추가
	product_option_validate_check($pass_code);
...
// pass_uid  -옵션 고유번호
    ...
    $resOption1 = _MQ_assoc("select *from smart_product_option where po_pcode = '".$pass_code."' and po_depth = '1' order by po_sort asc ");
    $option_sort1 = 0;
    foreach($resOption1 as $k=>$v){
        _MQ_noreturn("update smart_product_option set po_sort = '".$option_sort1."' where po_uid = '".$v['po_uid']."' and po_pcode = '".$pass_code."' and po_depth = '1' ");
        $option_sort1 ++;

        $option_sort2 = 0;
        $resOption2 = _MQ_assoc("select *from smart_product_option where po_pcode = '".$pass_code."' and po_parent = '".$v['po_uid']."' and po_depth = '2' order by po_sort asc ");
        foreach($resOption2 as $sk=>$sv){
            _MQ_noreturn("update smart_product_option set po_sort = '".$option_sort2."' where po_uid = '".$sv['po_uid']."' and po_pcode = '".$pass_code."' and po_depth = '2' ");
            $option_sort2 ++;

            $option_sort3 = 0;
            $resOption3 = _MQ_assoc("select *from smart_product_option where po_pcode = '".$pass_code."' and po_parent='" . $v['po_uid'].",".$sv['po_uid'] . "' and po_depth = '3' order by po_sort asc ");
            foreach($resOption3 as $ssk=>$svv){
                _MQ_noreturn("update smart_product_option set po_sort = '".$option_sort3."' where po_uid = '".$ssv['po_uid']."' and po_pcode = '".$pass_code."' and po_depth = '3' ");
                $option_sort2 ++;
            }
    ...
    // 0보다 크고 max 보다 적어야 함.
    if( $target_r[po_sort] >= 0 && $target_r['po_sort'] <= $maxr['max_sort'] ) { // LCY : 2023-03-10 : 순서조정 보정 -- >= 0 으로 변경 
...
# 2015-11-18 실시간 저장을 위하여 정보를 한번씩 더 저장 한다.# 2015-11-18 실시간 저장을 위하여 정보를 한번씩 더 저장 한다.
if( sizeof($po_info) > 0 ) {
    ...
    foreach( $po_info as $k=>$v ){
        ...
        if($v['po_color_type'] == 'color') {
            $app_color_name = addslashes(trim($v['po_color_name_c']));
            // 이미지일 경우 삭제
            ...
            if($_FILES["po_info_img_" . $k]['size'] > 0 ) {
                app_img_thumbnail($app_path , $app_color_name , ${"po_info_img_" . $k . "_OLD"} , 100 , 100 );
                $trigger_img ++;
            }
            // 삭제시 재시작
        ...
        $que = "
            update smart_product_option set
                po_poptionname     ='".addslashes(trim($v['po_poptionname']))."',
