<?PHP
	...
	// arr_sms_text_com_allow_type
	// LCY : 2023-05-31 : 입점업체 SMS추가 -- 최초1회만 추가
	if( $SubAdminMode === true){
		foreach($arr_sms_text_com_allow_type as $k=>$v){
			if( empty(${"com_".$v."_status"})){
				$row = _MQ("select ss_text from smart_sms_set where ss_uid ='admin_".$v."' ");
				_MQ_noreturn("INSERT smart_sms_set SET ss_uid = 'com_".$v."',  ss_text = '".$row['ss_text']."'  "); // , ss_status = 'Y'
			}
		}
	}
				...
				<tr>
					...
					<td>
						<div class="lineup-row type_multi">
							<span class="fr_tx" style="width:60px">아이디</span>
							<input type="text" name="_smsid" class="design" value="<?=$siteInfo['s_smsid']?>" placeholder="아이디"/>
						</div>
						<div class="dash_line"><!-- 점선라인 --></div>
						<div class="lineup-row type_multi">
							<span class="fr_tx" style="width:60px">비밀번호</span>
							<input type="text" name="_smspw" placeholder="비밀번호(변경시 입력)" class="design" value=""  autocomplete="off""/>
							<span class="c_btn h34 red bold "><input type="submit" name="" value="정보저장" /></span>
						</div>
					</td>
					<th>충전내역</th>
					<td>
						<div class="lineup-row">
							<?php
								...
								if($SMSUser['code'] == 'U04') {
									echo '
										<a class="c_btn h34 line blue " href="_config.default.form.php#sms_send_tel" >발신번호수정</a>
									';
								}
								if($SMSUser['code'] == 'U00') {
									echo '
										<a class="c_btn h34 black line gray bold " href="_config.sms.out_list.php" >'.number_format($SMSUser['data'],1).'건 남음</a>
									';
								}
							?>
							<a class="c_btn h34 black only_pc_view" href="_config.sms.out_list.php?type=charge" >충전하기</a>
							<a class="c_btn h34 sky only_pc_view"  href="http://mobitalk.gobeyond.co.kr/" target="_blank">모비톡 바로가기</a>
						</div>
						<div class="dash_line"><!-- 점선라인 --></div>

						<div class="tip_box">
							<?=_DescStr("현재 아이피 : <strong style='cursor: pointer; outline: none;' class='_copy'>" . $_SERVER['SERVER_ADDR'] . "</strong>")?>
							<?=_DescStr("위 아이피 등록후에 문자가 정상 발송되니 모비톡에서 꼭 등록해주세요.")?>
							<?=_DescStr("충전 및 결제는 PC에서만 가능합니다.")?>
						</div>
						<?php
							if($SMSUser['code'] != 'U00' && $SMSUser['code']!='') {
								$Uniq = uniqid();
								echo _DescStr("<span class='blink_text_" . $Uniq . "'><strong class='t_red t_bold'>" . $SMSUser['data'] . "</strong></span>");
...
<form name="frm" method="POST" action="_config.sms.pro.php" enctype="multipart/form-data">
	...
	<div class="total_sms">
		...
		<div class="phone_wrap">
			<div class="double">
				<div class="sms_phone normal_phone">
					<div class="check me"><label class="design"><input type="checkbox" name="m_status" class="m_status" value="Y" checked /><span class="tx">사용자 문자전송<span class="icon"></span></span></label></div>
				...
				<div class="sms_phone kakao_phone">
					<div class="check me"><label class="design"><input type="checkbox" name="mk_status" class="mk_status" value="Y" checked /><span class="tx">사용자 알림톡전송<span class="icon"></span></span></label></div>
			...
			<div class="double">
				<div class="sms_phone normal_phone">
					<div class="check ad"><label class="design"><input type="checkbox" name="a_status" class="a_status" value="Y" checked /><span class="tx">통합관리자 문자전송<span class="icon"></span></span></label></div>
				...
				<div class="sms_phone kakao_phone">
					<div class="check ad"><label class="design"><input type="checkbox" name="ak_status" class="ak_status" value="Y" checked /><span class="tx">통합관리자 알림톡전송<span class="icon"></span></span></label></div>
			...
			<?php if( $SubAdminMode === true){ // LCY : 2023-05-31 : 입점업체 SMS추가 { ?>
			<div class="double js_sms_form_com" style="<?php echo in_array($_uid,$arr_sms_text_com_allow_type) < 1 ? 'display:none;':'' ?>">
				<div class="sms_phone normal_phone">
					<div class="check ad"><label class="design"><input type="checkbox" name="c_status" class="c_status" value="Y" checked /><span class="tx">입점관리자 문자전송<span class="icon"></span></span></label></div>
					<div class="title_box"><input type="text" class="input_design c_title" name="c_title" placeholder="문자메세지 제목 입력" style="outline:0;" /></div>
					<div class="fix_box c_box textarea_wrap" style="cursor: text;">
						<div class="message_box">
							<textarea name="c_text" rows="" cols="" class="c_text chk_length textarea_content" tabindex="2" data-ma="c" placeholder="" style="outline:0;resize:none;"></textarea>
						</div>
					</div>
					<div class="total_box">
						<dl>
							<dt><span class="byte"><span class="c_len" style="color:inherit;">0</span> byte <strong class="a_type">SMS</strong></span></dt>
							<dd>
								<div class="file_box">
									<div class="input_file_sms">
										<a href="#none" onclick="return false;" class="buttonImg_delete realFile_delete" data-ma="c" data-delete="Y" title="이미지삭제">&nbsp;</a>
										<input type="text" id="c_fakeFileTxt" class="fakeFileTxt" readonly="readonly" disabled>
										<div class="fileDiv" title="이미지첨부">
											<input type="button" class="buttonImg" value="이미지첨부" />
											<input type="file" accept="image/jpeg" name="c_file" class="realFile c_file" data-ma="c" onchange="javascript:document.getElementById('c_fakeFileTxt').value = this.value.match(/[^\/\\]+$/)" />
											<input type="hidden" name="c_file_OLD" class="realFile_old c_file_OLD" data-ma="c" value=""/>
										</div>
									</div>
								</div>
							</dd>
							<dd><a href="#none" class="btn_rollback" onclick="return false;" title="메시지를 초기 설정으로 되돌립니다." data-ma="c">기본문구</a></dd>
						</dl>
					</div>
				</div><!-- end sms_phone -->

				<div class="sms_phone kakao_phone">
					<div class="check ad"><label class="design"><input type="checkbox" name="ck_status" class="ck_status" value="Y" checked /><span class="tx">입점관리자 알림톡전송<span class="icon"></span></span></label></div>
					<div class="title_box tit"><input type="text" class="input_design ck_kakao_templet_num" name="ck_kakao_templet_num" placeholder="알림톡 템플릿 고유번호 입력" style="outline:0; "></div>
					<div class="title_box code" style="display: none;"><input type="text" class="input_design ck_kakao_btn_link" name="ck_kakao_btn_link" placeholder="알림톡 버튼 사용 시 링크 입력" style="outline:0; " /></div>
					<?php for($i=1; $i<=8; $i++) { ?>
						<div class="title_box code"><input type="text" class="input_design ck_kakao_add<?php echo $i; ?>  js_drop_me" name="ck_kakao_add<?php echo $i; ?>" placeholder="알림톡 치환용 추가정보<?php echo $i; ?>  입력" style="outline:0; "></div>
					<?php } ?>
				</div><!-- end sms_phone -->
			</div><!-- end double -->
			<?php } // LCY : 2023-05-31 : 입점업체 SMS추가 } ?>
		</div><!-- end phone_wrap -->
			...
			<ul>
				<?php
					foreach($arr_sms_text_type as $uid => $value) {
						...
						$_com_check = ${"com_".$uid."_status"}=='Y'?true:false; // 입점관리자에게
						if( $_com_check === true){ $_ad_check = true; }

						$null_check = ($_me_check === true || $_ad_check === true?true:false); // 2016-12-07 LDD
				?>
					<li class="sms_types" id="sms_item_<?=$uid?>" data-type="<?=$uid?>" data-com="<?php echo in_array($uid, $arr_sms_text_com_allow_type)  ? 'Y':'N' // // LCY : 2023-05-31 : 입점업체 SMS추가 ?>">
						<!-- hit할 경우 나오는 아이콘 -->
	...
	<div class="data_form">
		<table class="table_form">
			...
			<tbody>
				<tr>
					<th class="">사용가능 치환자</th>
					<td>
						<div class="mobile_tip">모바일에서 드래그 기능은 제한되니 복사, 붙여넣기 해주세요.</div>
						<div class="sms_code">
							<ul class="replace_item">
								...
								<li data-text="{임시비밀번호}"><strong>{임시비밀번호}</strong></li>

								<li data-text="{주문번호}"><strong>{주문번호}</strong></li>
								<li data-text="{주문자명}"><strong>{주문자명}</strong></li>
								<li data-text="{주문일}"><strong>{주문일}</strong></li>
								<li data-text="{결제금액}"><strong>{결제금액}</strong></li>
								<li data-text="{입금계좌번호}"><strong>{입금계좌번호}</strong></li>

								<li data-text="{주문상품명}"><strong>{주문상품명}</strong></li>
								<li data-text="{택배사}"><strong>{택배사}</strong></li>
								<li data-text="{운송장번호}"><strong>{운송장번호}</strong></li>
								<li data-text="{배송일}"><strong>{배송일}</strong></li>

								<li data-text="{후기(문의)상품명}"><strong>{후기(문의)상품명}</strong></li>
								<li data-text="{후기(문의)타이틀}"><strong>{후기(문의)타이틀}</strong></li>
							</ul>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<div class="tip_box">
							...
							<div class="c_tip">{주문상품명}은 주문취소 및 부분취소 전용입니다.</div>
						</div>
					</td>
				</tr>
			</tbody>
...
// LCY : 2023-05-31 : 입점업체 SMS추가
var SubAdminMode = <?=$SubAdminMode === true ? true: 0?>;
...
$(document).ready(function(){
	...
	$('.sms_types').on('click',function(){
		...
		// LCY : 2023-05-31 : 입점업체 SMS추가
		var com = 'N';
		$('.js_sms_form_com').hide();
		if( SubAdminMode == true){
			com = $(this).data('com');
			if( com == 'Y'){ $('.js_sms_form_com').show(); }
		}
		...
		$.ajax({
			...
			success: function(data) {
				...
				// LCY : 2023-05-31 : 입점업체 SMS추가
				if( SubAdminMode == true && com == 'Y'){ 

					// 문구 출력
					$('.c_text').val(data.com._text);

					// 제목 출력
					$('.c_title').val(data.com._title);

					// 문구 placeholder 출력
					$('.c_text').attr({ 'placeholder' : data.com._name + ' - 입점관리자에게 전송할 내용을 입력하세요' });

					// 발송여부 선택
					if(data.com._status=='Y') { $('.c_status').prop('checked',true); } else { $('.c_status').prop('checked',false); }

					// 첨부파일 있으면 출력
					$('.c_img').remove(); $('.c_file, .c_file_OLD').val('');
					if(data.com._file) {
						$('.c_box .message_box').prepend('<div class="img_box c_img"><a href="#none" onclick="return false;" data-ma="c" data-delete="Y" class="realFile_delete btn_delete" title="이미지삭제"><img src="../images/new_sms/btn_img_delete.png" alt="" /></a><img src="/upfiles/'+data.com._file+'" alt="" /></div>');
						$('.c_file_OLD').val(data.com._file);
					}

					// 알림톡 데이터 적용
					if(data.kakao_com._status == 'Y') { $('.ck_status').prop('checked',true); } else { $('.ck_status').prop('checked',false); }
					$('.ck_kakao_templet_num').val(data.kakao_com._knum);
					$('.ck_kakao_btn_link').val(data.kakao_com._klink);
					<?php for($i=1; $i<=8; $i++) { ?>
						$('.ck_kakao_add<?php echo $i; ?>').val(data.kakao_com._kadd<?php echo $i; ?>);
					<?php } ?>
	...
	// 문구, 제목 수정시 editing 상태 변경 , // LCY : 2023-05-31 : 입점업체 SMS추가 
	$('.m_text, .a_text, c_text,  .m_title, .a_title, c_title').on('focus',function(){ $editing.val('Y'); });
	...
	$('.btn_rollback').on('click',function(){
		...
		// LCY : 2023-05-31 : 입점업체 SMS추가 
		if(SubAdminMode == true){
			confirm_txt = ma == 'c' ? '입점관리자':confirm_txt;
		}
...
$('.c_status').on('click', function(e) {
	var _ck = $(this).is(':checked');
	if(_ck === false) $('.ck_status').prop('checked', false);
});
$('.ck_status').on('click', function(e) {
	var _ck = $('.c_status').is(':checked');
	if(_ck === false) {
		alert('알림톡은 단독발송이 불가능합니다.');
		e.preventDefault();
	}
