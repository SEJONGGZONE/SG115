...
?>
	...
	<?php
		...
		$s_query = " from smart_product as p where p_type = 'ticket' ";

		$pass_type = 'ticket';
		$search_detail_name = 'ticket';
		...
		<table class="table_list">
			...
			<thead>
				<tr>
					...
					<th scope="col">No</th>
					<?php  if( $AdminPath == 'totalAdmin' ) { ?> <th scope="col">순위</th> <?php } ?>
				...
				<?php
				 if(sizeof($res) > 0){
					foreach($res as $k=>$v){
						...
						$_mod = '<a href="_product_ticket.form.php?_mode=modify&_code=' . $v['p_code'] . '&_PVSC=' . $_PVSC . '" class="c_btn gray ">수정</a>';
						$_del = '<a href="#none" onclick="del(\'_product_ticket.pro.php?_mode=delete&_code=' . $v['p_code'] . '&_PVSC=' . $_PVSC . '\');" class="c_btn h22 dark line">삭제</a>';
						$preview = '<a href="#none" onclick="window.open(\'/?pn=product.view&pcode='.$v['p_code'].'\')" class="c_btn sky line">미리보기</a>';
							...
							<td class="this_item t_left">
								<div class="order_item_thumb type_simple">
									...
									<div class="order_item">
										...
										<?php if($v['p_time_sale']=='Y'){?>
											<div class="item_timesale"><strong>타임세일</strong><em><?php echo $time_sale_sdate.' '.$time_sale_sclock.' ~ '.$time_sale_edate.' '.$time_sale_eclock;?></em></div>
										<?php }?>
									</div>
								</div>
								...
								<div class="lineup-row">
									...
									<?php  if( $AdminPath == 'totalAdmin' && $edit_img_cnt['cnt']>0 ) { ?>
										<a href="#none" onclick="edit_img_pop('<?php echo $v['p_code'] ?>')" class="c_btn h22 gray line">이미지 관리</a>
									<?php } ?>
								</div>
							</td>
	...
	<script>
		...
		$(document).on("click" , ".js_open_excel_box", function(){
			window.open('_product_ticket.upload.pop.php?','reqinfo','width=1120,height=600,scrollbars=yes');
		});
		// --- KAY : 상품일괄업로드 개선 : 2021-07-02 ---
		...
		function selectDelete() {
			if($('.js_ck').is(":checked")){
				if(confirm("정말 삭제하시겠습니까?")){
					...
					$("form[name=frm]").attr("action" , "_product_ticket.pro.php");
					document.frm.submit();
				}
			}
			else {
		...
		function sort_up(pcode,mode,query) {
			<?php if($st  == 'p_idx' && $so == 'asc' ){ ?>
				common_frame.location.href='_product_ticket.sort.php?pcode='+pcode+'&_mode='+mode+'&query='+query;
			<?php }else{ ?>
		...
		function sort_group(pcode){
			...
			common_frame.location.href='_product_ticket.sort.php?pcode='+pcode+'&_mode=modify_group&_group='+group;
		}
		// 선택순위그룹 수정
		function selectSortModify() {
			if($('.js_ck').is(':checked')){
				$('form[name=frm]').attr({'action':'_product_ticket.sort.php' , 'target':'common_frame'});
		...
		function downloadExcel(_mode){
			...
			$("form[name=frm]").attr("action" , "_product_ticket.download.php");
