...
if($_change_apw == 'Y') {
	...
	$sql_que .= ", s_adpwd = '".db_password($_adpwd)."' ";

	// -- smart_amdin 테이블 업데이트
	$queAdminModify .= " , a_pw = '".db_password($_adpwd)."' ";
}
...
//KAY :: 2022-08-02 :: 호스팅 사 추가
$sql_que .= "	, s_hosting_by = '".$_hosting_by."' " ;

// 답변용 작성자명
$sql_que .= "  , s_reply_writer = '".addslashes($_reply_writer)."' ";
...
$que = "
	update smart_setup set
		...
		, s_glbkwd = '{$_glbkwd}'
		, s_gmeta = '{$_gmeta}'
		, s_mobile_header_color = '{$_mobile_header_color}'
		{$sql_que}
	where s_uid = '1'
";
_MQ_noreturn($que);
