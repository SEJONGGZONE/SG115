		...
		<table class="category_list">
			...
			?>
				...
				<tr class="category-list-tr <?=in_array($v['c_uid'],array($locUid1,$locUid2,$_uid)) == true ? 'hit':''?>"  data-depth= "<?=$v['c_depth']?>" data-uid="<?=$v['c_uid']?>"  >
					<td>
						<span class="c_tag <?=$cViewClass?> h22 t2"><?=$cViewName?></span>
						<div class="ctg_name" <?=$cOnclickEvt?>><?=$v['c_name']?></div>
						<div class="ctrl">
							...
							<a href="#none" onclick="viewCategoryForm('modify','<?=$v['c_depth']?>','<?=$v['c_uid']?>'); return false;" class="c_btn h22 t2 dark line scrollto" data-scrollto="view-form">수정</a>
						</div>
					</td>
				</tr>
			<?php } ?>
