...
<form action="_product_option.pro.php" name="frm_option" id="frm_option" target="common_frame" method="post" ENCTYPE="multipart/form-data">
	...
	<div class="table_option">
		...
		<div class="first_step_box">
			<dl>
				...
				<dd>
					<div class="option_name">
						...
						<div class="name"><input type="text" name="po_info[<?=$r['po_uid']?>][po_poptionname]" class="design" placeholder="옵션명을 입력해주세요." value="<?php echo str_replace($op_str,"",$r['po_poptionname']); ?>"  /></div>
						<?php
							// ----- 컬러형일 경우 -----
			...
			if(sizeof($res2) <= 0){
				echo '
					<div class="common_none"><div class="no_icon"></div><div class="gtxt">등록된 2차 옵션이 없습니다.</div></div>
				';
				}
				else {
			...
			?>
				<div class="next_step_box">
					<dl>
						<dt>
							<div class="first_tit if_have_next">2차 옵션</div>
						</dt>
						<dd>
							<div class="option_name">
								<?php //KAY : 상품일괄업로드 개선 : 2021-07-02 -- 옵션명에서 특수문자 제거 ?>
								<div class="name"><input type="text" name="po_info[<?php echo $r2['po_uid']; ?>][po_poptionname]" class="design" placeholder="옵션명을 입력해주세요." value="<?php echo str_replace($op_str,"",$r2['po_poptionname']); ?>" /></div>
								<?php
									// ----- 컬러형일 경우 -----
					...
					<div class="last_step_box">
						<?php
							...
							$res3 = _MQ_assoc($que3);
							if(sizeof($res3) <= 0){
								echo '<div class="common_none"><div class="no_icon"></div><div class="gtxt">등록된 3차 옵션이 없습니다.</div></div>';
							}
							else {
						...
						?>
							...
							<dl>
								<dt>
									<div class="first_tit">3차 옵션</div>
								</dt>
								<dd>
									<div class="duplicate_box depth_2<?php echo ($r3['po_view'] == 'Y'?null:' if_option_hide'); ?>">
										<div class="option_name">
											<?php //KAY : 상품일괄업로드 개선 : 2021-07-02 -- 옵션명에서 특수문자 제거 ?>
											<div class="name"><input type="text" name="po_info[<?php echo $r3['po_uid']; ?>][po_poptionname]" class="design" placeholder="옵션명을 입력해주세요." value="<?php echo str_replace($op_str,"",$r3['po_poptionname']); ?>" /></div>
											<?php
												// ----- 컬러형일 경우 -----
										...
										<div class="option_price_box">
											...
											<ul>
												<li>
													<input type="text" name="po_info[<?php echo $r3['po_uid']; ?>][po_poption_supplyprice]" class="design number_style" placeholder="" value="<?php echo number_format($r3['po_poption_supplyprice']); ?>" />
												</li>
												<li>
													<input type="text" name="po_info[<?php echo $r3['po_uid']; ?>][po_poptionprice]" class="design number_style" placeholder="" value="<?php echo number_format($r3['po_poptionprice']); ?>"  />
												</li>
												<li class="li_mount">
													<input type="text" name="po_info[<?php echo $r3['po_uid']; ?>][po_cnt]" class="design number_style" placeholder="" value="<?php echo number_format($r3['po_cnt']); ?>" />
												</li>
												<li class="li_mount">
													<input type="text" name="" class="design number_style" placeholder="" value="<?php echo number_format($r3['po_salecnt']); ?>" disabled />
												</li>
											</ul>
										</div>
										...
										<div class="step_ctrl">
											...
											<div class="lineup-row type_end">
												...
												<a href="#none" onclick="f_insert('3', '<?php echo $r3['po_uid']; ?>'); return false;" class="c_btn blue line">추가</a><!-- 끼워넣기 추가버튼 -->
												<a href="#none" onclick="category_apply_save('3depth_del', '<?php echo $r3['po_uid']; ?>'); return false;" class="c_btn black line">삭제</a>
											</div>
										</div>
					...
					<div class="step_ctrl if_have_next">
						...
						<div class="lineup-row type_end">
							...
							<a href="#none" onclick="category_apply_save('3depth_add', '<?php echo $r2['po_uid']; ?>'); return false;" class="c_btn blue line t5">3차 삽입</a><!-- 끼워넣기 추가버튼 -->
							<a href="#none" onclick="category_apply_save('2depth_del', '<?php echo $r2['po_uid']; ?>'); return false;" class="c_btn h22 black line t5">2차 삭제</a>
						</div>
					</div>
				</div><!-- end next_step_box -->
			<?php
				}} // ------------ 2차 옵션 loop ------------
			?>
			...
			<div class="step_ctrl if_have_next">
				...
				<div class="lineup-row type_end">
					...
					<a href="#none" onclick="category_apply_save('2depth_add', '<?php echo $r['po_uid']; ?>'); return false;" class="c_btn blue t5">2차 삽입</a><!-- 끼워넣기 추가버튼 -->
					<a href="#none" onclick="category_apply_save('1depth_del', '<?php echo $r['po_uid']; ?>'); return false;" class="c_btn black t5">1차 삭제</a>
				</div>
			</div>
		</div><!-- end first_step_box -->
		<?php
			} // ------------ 1차 옵션 loop ------------
		?>
