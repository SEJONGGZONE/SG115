		...
		case "viewVisitList":
			...
			if($TotalCount > 0) {
				foreach( $res as $datek => $datev ){
					...
					$printVisitList .= '<tr>';
					$printVisitList .= '	<td class="this_num">'. $_num .'</td>';
					$printVisitList .= '	<td class="this_state"><span class="shop_state_pack">'. $_device .'</span></td>';
					$printVisitList .= '	<td>'. $datev['sc_ip'] .'</td>';
					$printVisitList .= '	<td>'. $datev['sc_keyword'] .'</td>';
					$printVisitList .= '	<td class="t_left">' . ( $datev['sc_referer'] ? '<a href="'.$datev['sc_referer'].'" target="_blank">'.$datev['sc_referer'].'</a>' : '<span class="t_none">직접입력</span>' ) . '</td>';
					$printVisitList .= '	<td class="this_date">'. printDateInfo($datev['sc_date']) .'</td>';
					$printVisitList .= '	<td class="this_ctrl t_sky">'. $datev['sc_browser'] .'</td>';
					$printVisitList .= '</tr>';
				}
			}
