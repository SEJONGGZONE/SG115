...
<div class="group_title type_first">
	<strong class="">쿠폰 발급하기</strong>
	<div class="btn_box">
		<?php
			// 사용기간
			// 발급내역 버튼
			echo '
			<a href="_coupon.list.php?_uid='.$_uid.'"  class="c_btn blue line h22">발급내역 ('.number_format($row['issued_cnt']).') 확인하기</a>
			';
		?>
	</div>
</div>
...
<div class="data_form type_info">
	<table class="table_form">
		...
		<tbody>
			<tr>
				<td colspan="4" class="t_blue t_big t_bold">
...
<form name="frm" id="frm" method="post" action="_coupon.pro.php">
	...
	<div class="list_ctrl">
		<div class="left_box">
			...
			<a href="#none" onclick="ctrlIssued('select'); return false;" class="c_btn h27 blue">선택회원 쿠폰발급</a>
			<a href="#none" onclick="ctrlIssued('search'); return false;" class="c_btn h27 blue">검색회원 쿠폰발급(<?=number_format($TotalCount)?>)</a>
		</div>
		<div class="right_box">
		...
		<?php
			foreach($res as $k=>$v) {
				...
				if($v['in_auth']  != 'Y' ){
					...
					$printAuth = '<span class="c_tag h18 blue line t3">승인</span>';
				}
				...
				echo '	<td class="this_ctrl">'.$printAuth.'</td>';
				echo '	<td>'.$v['in_name'].' ('.$v['in_id'].')</td>';
				...
				echo '	<td class="t_blue t_bold this_state">'.number_format($v['issuedMemCnt']).'개</td>';
				echo '</tr>';
			}
		?>
		</tbody>
