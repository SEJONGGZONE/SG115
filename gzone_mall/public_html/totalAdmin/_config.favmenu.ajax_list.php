		...
		<table class="category_list">
			...
			<?php
				foreach($resFavMenu as $k=>$v){
					...
					if($viewDepth > 1){
						// 메뉴 삭제 체크
						if(!$v['fm_menu']) $_error = '<div class="c_tip">메뉴 미선택</div>';
						else if($v['am_uid'] == '') $_error = '<div class="c_tip">삭제된 메뉴</div>';
						// 메뉴 노출여부 체크
						else if($v['am_view'] <> 'Y') $_error = '<div class="c_tip">미사용 메뉴</div>';
						// 메뉴 접근권한 체크
						else if($v['ams_uid'] == '' && is_master() == false) $_error = '<div class="c_tip">권한없음</div>';
						// 메뉴 파일명이 등록되지 않음
						else if($v['am_link'] == '') $_error = '<div class="c_tip">연결페이지 없음</div>';
					}
			?>
				...
				<tr class="favmenu-list-tr <?php echo (in_array($v['fm_uid'],array($locUid1,$locUid2)) == true ? 'hit':''); ?>"  data-depth= "<?php echo $v['fm_depth']; ?>" data-uid="<?php echo $v['fm_uid']; ?>"  >
					<td>
						<span class="c_tag <?php echo $fmViewClass; ?> h22 t2"><?php echo $fmViewName; ?></span>
						<div class="ctg_name" <?php echo $fmOnclickEvt; ?>><span class="fr_tx"><?php echo $v['fm_menuName']; ?></span><?php echo $_error; ?></div>
						<div class="ctrl">
							...
							<a href="#none" onclick="viewFavMenuForm('modify','<?php echo $v['fm_depth']; ?>','<?php echo $v['fm_uid']; ?>'); return false;" class="c_btn h22 t2 gray scrollto" data-scrollto="view-form">수정</a>
						</div>
					</td>
				</tr>
			<?php } ?>
