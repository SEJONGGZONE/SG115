        ...
        <div class="lineup-row type_multi">
            <input type="text" name="_hashtag_search" id="_hashtag_search" class="design" placeholder="해시태그 자동완성" value="" style="width:150px">
            <a href="#none" onclick="addField(document.frm._hashtag);" class="c_btn blue">추가하기</a>
        </div>
		<div class="dash_line"><!-- 점선라인 --></div>
		<textarea name="_hashtag" rows="5" cols="" class="design" placeholder="해시태그 입력"><?php echo stripslashes($row['p_hashtag']); ?></textarea>
		<label class="design"><input type="checkbox" name="_hashtag_shuffle" value="Y" <?php echo ($row['p_hashtag_shuffle']=='Y'?'checked':''); ?>>무작위 순서로 노출</label>
		<div class="tip_box">
			<?php echo _DescStr('해시태그 등록 시 상품상세페이지에 노출되며, 클릭 시 같은 해시태그로 등록된 상품을 검색할 수 있습니다.'); ?>
