<?PHP
	...
	if( in_array($_mode,array('add','modify')) == true){
		...
		$sque = "
			mgs_name ='".$_name."', 
			mgs_condition_totprice = '".$_condition_totprice."', 
			mgs_condition_totcnt = '".$_condition_totcnt."', 
			mgs_give_point_per = '".$_give_point_per."',
			mgs_sale_price_per = '".$_sale_price_per."',  
			mgs_rdate = now(),
			mgs_icon = '".$_icon_name."',
			mgs_mobile_icon = '".$_mobile_icon_name."' ,
			mgs_idx = '".delComma($_idx)."'
		";
	...
	switch($_mode){
		...
		case "add":
			...
			_MQ_noreturn("insert smart_member_group_set set ".$sque.",mgs_rank='".$_rank."' 	");
			$_uid = _MQ_insert_id();
		...
		case "modify":
			...
			_MQ_noreturn("update smart_member_group_set set ".$sque." where mgs_uid = '".$_uid."'  ");
		...
		// -- 등급삭제
		case "delete":
			$row = _MQ("select *, (select count(*) as cnt from smart_individual where in_mgsuid = mgs_uid ) as cnt from smart_member_group_set where mgs_uid = '".$_uid."' ");
			if( count($row) < 1){
			...
			if($row['cnt'] > 0){ error_alt("등록된 회원이 있는 등급은 삭제가 불가능합니다."); }
		...
		case "selectDelete":
			...
			foreach($res as $k=>$v) {
				$row = _MQ_result("select (select count(*) as cnt from smart_individual where in_mgsuid = mgs_uid ) as cnt from smart_member_group_set where mgs_uid = '".$v['mgs_uid']."' ");

				if($row > 0){ error_alt("등록된 회원이 있는 등급은 삭제가 불가능합니다."); }

				_PhotoDel( "../upfiles/icon/" , $v['mgs_icon'] );
				_PhotoDel( "../upfiles/icon/" , $v['mgs_mobile_icon'] );
			}
