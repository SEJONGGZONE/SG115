...
?>	
		...
		<!-- ● 단락타이틀 -->
		<div class="group_title"><strong>선택 브랜드 <?php echo $_mode == 'add' ? '추가':'수정' ?></strong><!-- 메뉴얼로 링크 --><?php echo openMenualLink('선택브랜드설정')?></div>
				...
				<tbody>
					<tr>
						...
						<td>
							<?php if($_depth > 1) {?> 
								<!-- 상위카테고리가 있을 경우 -->
								<div class="category_upper"><span class="fr_bullet">상위 브랜드</span><?php echo $printParent?></div>
							<?php } ?>
							<input type="text" name="_name" class="design bold t_black" placeholder="브랜드명" value="<?php echo $rowAdminMenu['c_name']?>" style="width:240px" />
							<div class="c_tip black">브랜드명을 입력해 주세요.</div>
						</td>
						<th>노출여부</th>
						<td>
							<label class="design"><input type="radio" value="Y" name="_view" <?php echo !$rowAdminMenu['c_view'] || $rowAdminMenu['c_view'] == 'Y' ? 'checked':''?>>노출</label>
							<label class="design"><input type="radio" value="N" name="_view" <?php echo $rowAdminMenu['c_view'] == 'N' ? 'checked':''?>>숨김</label>
						</td>
					</tr>
