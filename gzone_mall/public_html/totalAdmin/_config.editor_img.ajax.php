...
?>
		<div class="pop_title">에디터 이미지 사용페이지<a href="#none" onclick="return false;" class="close btn_close " title="닫기"></a></div>
		<div class="pop_conts">
			<!-- ● 데이터 리스트 -->
			<div class="data_list">
				<table class="table_list type_inside">
					<colgroup>
						<col width="70"><col width="120"><col width="*"><col width="100">
					</colgroup>
					<thead>
						<tr>
							<th scope="col">No</th>
					...
					<tbody>
							...
							// $res = 에디터 이미지 정보
							 if(sizeof($res) > 0){
								foreach($res as $k=>$r){
									...
									// 바로가기 버튼
									$ei_use_link = ($edit_img_info['link'] ? '<div class="lineup-vertical"><a href="'.$edit_img_info['link'].'" class="c_btn gray line h27" target="_blank">바로가기</a></div> <!--//바로가기 버튼-->' : '');
						...
						?>
								<tr>
									...
									<td><span class="hidden_tx">구분</span><?php echo $ei_use_name; ?></td><!-- 사용처 구분 -->
									<td class="t_left"><span class="hidden_tx">페이지명</span><?php echo $ei_title_name; ?></td><!-- 사용하는곳 타이틀 -->
									<td class="this_ctrl"><?php echo $ei_use_link; ?></td><!-- 바로가기 -->
								</tr>
						<?php
								}
							}
			...
			<div class="tip_box">
				<div class="c_tip">기존이미지 패치 시 구분이 없는 경우 이미지 바로가기가 생성되지 않을 수 있습니다.</div>
				<div class="c_tip">생성되지 않은 바로가기는 다음날 이미지 구분 지정 후 생성됩니다.</div>
			</div>
		</div>
