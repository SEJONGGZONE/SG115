...
// 엑셀 간략 검증
$OCheck = _MQ(" select o_ordernum from smart_order where o_ordernum = '{$excel_values[0]['주문번호']}' ");
				...
				?>
					<tr>
						...
						<td>
							...
							echo _InputSelect('_sendstatus['.$_ordernum.']', $arr_order_product_sendstatus, ($v['배송상태']?$v['배송상태']:'배송대기'), ' class="js_sendstatus '.($DClass?$DClass:null).'"', '', '');
							?>
						</td>
						<td>
							<?php echo _InputSelect('_sendcompany['.$_ordernum.']', array_keys($arr_delivery_company), $v['택배사'], '', '', ''); ?>
						</td>
						<td>
							<input type="text" name="_sendnum[<?php echo $_ordernum; ?>]" class="design" placeholder="송장번호" value="<?php echo $v['송장번호']; ?>">
						</td>
						...
						<td><?php echo $v['받는분 주소']; ?></td>
						<td><?php echo $v['받는분 지번주소']; ?></td>
						<td class="t_left"><?php echo $v['상품정보']; ?></td>
						<td class="t_left"><?php echo $v['배송시 유의사항']; ?></td>
						<td class="t_left"><?php echo $v['상품정보']; ?></td>
