<?PHP
	...
	if( in_array( $_mode , array("add" , "modify") ) ) {
		...
		$CorpName = addslashes(trim($CorpName));
		$CEOName = addslashes(trim($CEOName));
		$CorpNum = addslashes(trim($CorpNum));
		$Addr = addslashes(trim($Addr));
		$BizType = addslashes(trim($BizType));
		$BizClass = addslashes(trim($BizClass));
		$ContactName = addslashes(trim($ContactName));
		$HP = addslashes(trim($HP));
