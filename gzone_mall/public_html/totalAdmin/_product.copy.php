...
$ProductReqInfo		= _MQ_assoc("select `old`.*, '{$_code}' as `pri_pcode` from `smart_product_req_info` as `old` where `pri_pcode` = '{$pcode}'  order by pri_uid asc"); // 상품정보제공고시
...
$arr_imgname = array('p_img_list_square', 'p_img_b1', 'p_img_b2', 'p_img_b3', 'p_img_b4', 'p_img_b5','p_img_list_over');
foreach($arr_imgname as $k=>$v){
	if(trim($Product[$v]) != '') {
		$thumb_type = 's';
		if( $v == 'p_img_list_over'){  $thumb_type = null; }
		$Product[$v] = _PhotoCopy($Product[$v] , $thumb_type);
	}
}
...
// 입점관리자 복사 가능하도록
if( $AdminPath== 'subAdmin'){ $Product['p_view'] = 'N'; }
...
if(sizeof($ProductTableText)>0){
	foreach($ProductTableText as $k=>$v) {
		unset($v['ttt_uid']);
		$v['ttt_value'] = addslashes($v['ttt_value']);
