...
// 쿠폰유형
$ocs_type = $row['ocs_type'] == '' ? array_shift(array_keys($arrCouponSet['ocs_type_view'])):$row['ocs_type'];
...
<input type="hidden" name="saveType" value="<?php echo $_mode == 'add' ? 'temp':'real' ; ?>"><?php // 추가일시에는 저장타입을 임시로 설정 ?>
	...
	<div class="group_title"><strong>기본정보</strong></div>
	<div class="data_form">
		<table class="table_form">
			...
			<tbody>
				<tr>
					<th class="ess">쿠폰명</th>
					<td colspan="3">
						<input type="text" name="ocs_name" class="design" style="width:280px;" maxlength="30" placeholder="쿠폰명" value="<?php echo $row['ocs_name']; ?>">
						<?php echo _DescStr("위 쿠폰명으로 사용자에게 발급됩니다."); ?>
					</td>
				</tr>
				<tr>
					<th>쿠폰유형</th>
					<td colspan="3">
						<?php echo _InputRadio( 'ocs_type' , array_keys($arrCouponSet['ocs_type_view']), ($ocs_type) , '' , array_values($arrCouponSet['ocs_type_view']) , ''); ?>
						<div class="tip_box">
							<?php echo _DescStr("<em>주문쿠폰</em> : 배송비까지 포함한 전체 결제금액 기준으로 할인/적립이 적용됩니다.",""); ?>
							<?php echo _DescStr("<em>배송쿠폰</em> : 배송비 기준으로 할인이 됩니다.",""); ?>
						</div>
					</td>
				</tr>
				<tr>
                    ...
                    <td colspan="3">
                        ...
                        <div class="tip_box">
                            ...
                            <?php echo _DescStr("<em>자동발급</em> : 설정한 조건에 따라 자동으로 발급됩니다. ",""); ?>
                        </div>

						<div class="js_issued_type_auto" style="display: none;">
							...
							<div class="tip_table">
								<ul class="thead">
									<li class="th" style="width:100px">구분</li>
									<li class="th">발급조건</li>
									<li class="th">제한사항</li>
								</ul>
								<ul>
									<li class="th"><?php echo ($arrCouponSet['ocs_issued_type_auto']['1']); ?></li>
									<li>최초 결제완료 시</li>
									<li>ID당 최초 1회</li>
								</ul>
								<ul>
									<li class="th"><?php echo ($arrCouponSet['ocs_issued_type_auto']['2']); ?></li>
									<li>첫 결제를 포함, 결제완료 시 마다</li>
									<li>중복발급 설정에 따름, 배송/발급완료 후 사용가능</li>
								</ul>
								<ul>
									<li class="th"><?php echo ($arrCouponSet['ocs_issued_type_auto']['3']); ?></li>
									<li>매년 생일마다</li>
									<li>생년월일을 입력한 경우</li>
								</ul>
								<ul>
									<li class="th"><?php echo ($arrCouponSet['ocs_issued_type_auto']['4']); ?></li>
									<li>회원가입 시</li>
									<li>승인 완료 후 첫 로그인</li>
								</ul>
								<ul>
									<li class="th"><?php echo ($arrCouponSet['ocs_issued_type_auto']['5']); ?></li>
									<li>출석체크 조건 만족 시</li>
									<li>쿠폰 설정에 따름</li>
								</ul>
								...
								?>
								<ul>
									<li class="th"><?php echo ($arrCouponSet['ocs_issued_type_auto']['6']); ?></li>
									<li>앱 설치 시</li>
									<li>앱 혜택설정에 따름</li>
								</ul>
							...
							<?php echo _DescStr("결제완료란 실시간 결제확인이 되는 수단 외 무통장 등 입금확인이 필요한 경우에는 입금까지 완료된 후 발급됩니다.",""); ?>
							<?php echo _DescStr("생일쿠폰을 사용하기 위해서는 회원 부가정보에 생년월일을 사용해야합니다. <a href='/totalAdmin/_config.join.php'><em>가입항목 바로가기</em></a>",""); ?>
							<?php echo _DescStr("출석체크 쿠폰은 출석체크 이벤트의 해당 설정된 조건에 따라 지급됩니다. <a href='/totalAdmin/_promotion_attend.list.php'><em>출석체크 바로가기</em></a>",""); ?>
						</div>
                    </td>
				</tr>
				<tr class="js_issued_due_type">
					<th>자동발급 조건</th>
					<td colspan="3">
						...
						<?php echo _DescStr("첫 구매/결제완료 또는 회원가입 쿠폰은 설정과 상관없이 한번만 발급됩니다.", ''); ?>
						<?php echo _DescStr("생일축하 쿠폰은 설정과 상관없이 매년 한번만 발급됩니다.", ''); ?>
						<div class="dash_line"><!-- 점선라인 --></div>
						...
						<div class="lineup-row">
							...
							<div class="js_issued_cnt_type_cnt" style="display:none;">
								<input type="text" name="ocs_issued_cnt" class="design number_style" style="width:70px;" value="<?=$row['ocs_issued_cnt']?>" />
								<span class="fr_tx">개 까지만 발급</span>
							</div>
						</div>
						<?php echo _DescStr("중복발급을 적용한 경우에 발급 개수를 제한할 수 있습니다.", ''); ?>
						<div class="dash_line"><!-- 점선라인 --></div>

						<?php echo $printGroupChk; ?>
						<?php echo _DescStr("회원가입 쿠폰은 설정에 상관없이 가입 후 무조건 발급됩니다.", ''); ?>
						<?php echo _DescStr("쿠폰을 지급받을 회원등급은 최소 1개이상 설정해야합니다. <a href='/totalAdmin/_member_group_set.list.php'><em>회원등급 관리 바로가기</em></a>", ''); ?>
					</td>
				</tr>
			</tbody>
		</table>
	...
	<div class="group_title"><strong>혜택설정</strong></div>
		<div class="data_form">
		<table class="table_form">
			<colgroup>
				<col width="180"/><col width="*"/><col width="180"/><col width="*"/>
			...
			<tbody>
				<tr>
                    ...
                    <td>
                        ...
                        <div class="tip_box">
                            <?php echo _DescStr("<em>배송비 할인</em>은 <em>배송쿠폰</em>유형일 경우에만 설정 가능합니다.",""); ?>
                        </div>
                    </td>
					<th class="ess">금액설정</th>
					<td>
                        <div class="lineup-row">
                            <input type="text" name="temp_pricePer" class="design number_style" style="width:100px; text-align: right;" placeholder="0" value="<?=$temp_pricePer?>" />
                            <?php echo _InputSelect( "ocs_dtype" , array_keys($arrCouponSet['ocs_dtype']) ,$ocs_dtype  , "" , array_values($arrCouponSet['ocs_dtype']) , ""); ?>
                            <span class="fr_tx js_boon_type_print t_blue t_bold">할인</span>
                        </div>
						<?php echo _DescStr("소수점은 적용되지 않으니 정수만 입력해 주세요."); ?>
						<div class="lineup-row js_price_type_per" style="display: none;">
							...
							<input type="text" name="ocs_price_max" class="design number_style" style="width:100px;" placeholder="0" value="<?php echo $row['ocs_price_max']?>" />
							<span class="fr_tx">원까지</span><span class="fr_tx js_boon_type_print t_blue t_bold">할인</span>
						</div>
	...
	<div class="group_title"><strong>사용제한</strong></div>
	<div class="data_form">
		<table class="table_form">
			<colgroup>
				<col width="180"/><col width="*"/><col width="180"/><col width="*"/>
			...
			<tbody>
				<tr>
					...
					<td colspan="3">
						...
						<div class="dash_line"><!-- 점선라인 --></div>
						<div class="lineup-row type_date js_use_date_type_date" data-desc="사용기간 지정일경우" style="display: none; margin:0">
							<input type="text" name="ocs_sdate" value="<?php echo rm_str($row['ocs_sdate']) < 1 ? date('Y-m-d'): $row['ocs_sdate'] ?>" class="design js_datepic_min_today" placeholder="날짜 선택" readonly style="width:95px">
							<span class="fr_tx"> ~ </span>
							<input type="text" name="ocs_edate" value="<?php echo rm_str($row['ocs_edate']) < 1 ? date('Y-m-d'): $row['ocs_edate'] ?>" class="design js_datepic_min_today" placeholder="날짜 선택" readonly style="width:95px">
						</div>
						<div class="lineup-row js_use_date_type_expire" data-desc="유효기간 지정일경우" style="display: none; margin:0">
							 <span class="fr_tx">발급일로 부터</span>
							 <input type="text" name="ocs_expire" class="design" placeholder="0" style="width:70px; text-align: right;" value="<?php echo $row['ocs_expire'] == '' ? 0 : $row['ocs_expire'] ?>"><span class="fr_tx">일 까지</span>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
                        <div class="lineup-row">
                            <input type="text" name="ocs_limit" class="design number_style" style="width:100px; text-align: right;" placeholder="0" value="<?=$row['ocs_limit']?>" />
                            <span class="fr_tx">원 이상 결제 시 사용가능</span>
                        </div>
						<?php echo _DescStr("위 설정금액은 배송비까지 포함한 금액입니다."); ?>
					</td>
					<th>중복사용</th>
					<td>
						<?php echo _InputRadio( 'ocs_due_use' ,array_keys($arrCouponSet['ocs_due_use']), $ocs_due_use , '' , array_values($arrCouponSet['ocs_due_use']) , ''); ?>
						<?php echo _DescStr('<em>중복사용 가능</em> : 한 주문에서 같은 쿠폰을 사용가능한 만큼 모두 사용할 수 있습니다.', ''); ?>
						<?php echo _DescStr('<em>중복사용 불가</em> : 한 주문에서 같은 쿠폰을 1장만 사용할 수 있습니다.', ''); ?>
					</td>
				</tr>
			</tbody>
		</table>
		...
		<table class="table_form">
			...
			<tbody>
				<tr>
                    <th>쿠폰설명</th>
                    <td>
						<textarea name="ocs_desc" rows="3" cols="" class="design" placeholder="쿠폰설명"><?php echo $row['ocs_desc']; ?></textarea>
						<?php echo _DescStr('사용자 페이지에 노출되지 않으며, 발급내역이 있는 경우에는 쿠폰설명도 수정이 불가합니다.', ''); ?>
                    </td>
				</tr>
			</tbody>
		</table>
...
<script>
	...
	function couponSet_boon_type()
	{
		...
		if( couponSet.ocs_type == 'delivery'){ // 쿠폰유형이 배송이라면
			...
			$('input[name="ocs_boon_type"][value="discount"]').prop({disabled : true});
			$('input[name="ocs_boon_type"][value="save"]').prop({disabled : true});
	...
	$(document).ready(function() {
		...
		$('form[name=frm]').validate({
			...
			submitHandler : function(form) {
				...
				// 할인금액
				else if(couponSet.ocs_dtype == 'price'){

					if( couponSet.temp_pricePer < 1){
						alert("할인금액은 0이상 입력해 주세요.");
						$('input[name="temp_pricePer"]').focus();
						return false;
					}
