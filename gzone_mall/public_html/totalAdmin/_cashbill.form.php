...
<form id="frm" name="frm" method="post" ENCTYPE='multipart/form-data' action="_cashbill.pro.php" >
	...
	<div class="data_form">
		<table class="table_form">
			...
			<tbody>
				...
				<tr>
					<th>거래용도</th>
					<td colspan="3"><?php echo _InputRadio( "TradeUsage" , array_keys($arr_tradeUsage), ($row["TradeUsage"] ? $row["TradeUsage"] : "1") , "" , array_values($arr_tradeUsage) , ''); ?></td>
				</tr>
				<tr>
					<th>신분확인번호 구분</th>
					<td colspan="3">
						<?php echo _InputRadio( "TradeMethod" , array_keys($arr_TradeMethod), ($row["TradeMethod"] ? $row["TradeMethod"] : "5") , "" , array_values($arr_TradeMethod) , ''); ?>
						<input type="hidden" name="_identitynum_valid" value="" />
					</td>
				</tr>
				<tr>
					<th class="ess">신분확인번호</th>
					<td>
						<input type="text" name="IdentityNum" value="<?php echo $row["IdentityNum"]; ?>" class="design js_number_valid" style='width:150px' placeholder="신분확인번호">
						<div class="tip_box">
							<?php echo _DescStr("위에서 선택한 신분확인번호 구분에 따라 현금영수증카드, 휴대폰번호, 사업자등록번호를 알맞게 입력해주세요."); ?>
							<?php echo _DescStr("사업자번호를 입력한 경우 거래용도를 지출증빙용으로만 선택할 수 있습니다."); ?>
						</div>
					</td>
					<th>수신 이메일</th>
					<td>
						<input type="text" name="Email" value="<?php echo $row["Email"]; ?>" class="design" style='width:350px' placeholder="수신 이메일">
						<div class="tip_box">
							<?php echo _DescStr("이메일을 입력하면 바로빌에서 현금영수증 발급완료 안내메일을 받을 수 있습니다."); ?>
				...
				<tr>
					...
					<td>
						<div class="lineup-row">
							<input type="text" name="Amount" value="<?php echo $row["Amount"]; ?>" class="design number_style calc_total calc_amount" style='width:100px' placeholder="0">
					...
					<td>
						<div class="lineup-row">
							<input type="text" name="Tax" value="<?php echo $row["Tax"]; ?>" class="design number_style calc_total calc_tax" style='width:100px' placeholder="0">
							<span class="fr_tx">원</span>
							<a href="/totalAdmin/_config.vat.form.php" class="c_btn sky line" target="_blank">부가세 설정 바로가기</a>
						</div>
						<?php
				...
				<tr>
					...
					<td>
						<div class="lineup-row">
							<input type="text" name="ServiceCharge" value="<?php echo $row["ServiceCharge"]; ?>" class="design number_style calc_total" style='width:100px' placeholder="0">
							<span class="fr_tx">원</span>
						</div>
						<?php echo _DescStr("부가세 외에 추가적으로 판매금액에 포함되는 봉사료 항목입니다."); ?>
					</td>
					<th>판매금액</th>
					<td>
						<div class="lineup-row">
							<input type="text" name="" value="<?php echo number_format($row["Amount"] + $row["Tax"] + $row["ServiceCharge"]); ;?>" class="design number_style calc_price_sum" style='width:100px'	placeholder="0" disabled="disabled">
							<span class="fr_tx">원</span>
						</div>
						<?php echo _DescStr("공급가와 부가세입력 후 자동계산됩니다."); ?>
					</td>
				</tr>
				<tr>
					<th class="ess">품목명</th>
					<td colspan="3"><input type="text" name="ItemName" value="<?php echo $row["ItemName"]; ?>" class="design" style='width:350px' placeholder="품목명"></td>
				</tr>
			</tbody>
		</table>
	</div>
	...
	<div class="c_btnbox type_full">
		<ul>
			<li><a href="#none" onclick="submit_btn('issue')" class="c_btn h46 red">현금영수증 발행</a></li>
			<li><a href="#none" onclick="submit_btn('save')" class="c_btn h46 blue line">임시저장</a></li>
	...
	$(document).ready(function(){
		...
		$("form[name=frm]").validate({
			...
			messages: {
					...
					_identitynum_valid : { required: "신분확인번호를 입력해주시기 바랍니다." },
			...
			submitHandler : function(form) {
				...
				var _type = $("#frm input[name=_mode]").val();
				if( _type == 'issue'){
					if( confirm("바로 현금영수증을 발행하시겠습니까?") == false){ return false; }
				}
