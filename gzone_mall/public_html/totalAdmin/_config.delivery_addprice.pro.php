<?PHP
	...
	switch( $_mode ){
		...
		case "ajax_form":
			...
			// 주소저장 배열
			$arr = array("road"=>array(),"jibun"=>array());
						...
						<tr>
							...
							<td>
								<div class="lineup-row">
									<input type="text" name="_addprice" id="_addprice" value="'. number_format($addprice) .'" class="design number_style t_right" style="width:100px" placeholder="0">
			...
			foreach($arr as $k=>$v){
				...
				foreach($v as $k2=>$v2){
					...
					$form_str .= '
						<div class="lineup-row type_multi">
							...
							<a href="#none" onclick="insert_addprice('.$idx.')" class="c_btn h27 blue line">선택추가</a>
						</div>
					';
				}
				$idx++;
				$form_str .= '
					<div class="lineup-row type_multi">
						...
						<a href="#none" onclick="insert_addprice('.$idx.')" class="c_btn h27 gray blue">수정추가</a>
					</div>
				';
				$form_str .= '</td></tr>';
			}
		...
		case "ins_excel":
			...
			# 첨부파일 확인
			if($_FILES['excel_file']['size'] <= 0) { error_loc_msg("_config.delivery_addprice.list.php?", "첨부파일이 없습니다."); }

			$exFilename = end(@explode(".",$_FILES[$fileKey[0]]['name']));
			if(preg_match("/(xlsx)||(xls)/i", $exFilename)==false){ error_loc_msg("_config.delivery_addprice.list.php?", "엑셀파일을 등록해주세요.");  } 
