<?php
	if($_REQUEST['_mode'] == 'modify') {
		$app_current_name = '배송상품 수정';
		$app_current_link = '_product.list.php';
	}
	else{
		$app_current_name = '배송상품 등록';
		$app_current_link = '_product.list.php';
	}

	// 상품타입을 고정한다.
	$app_product_type = 'delivery';
	...
	if( $_mode == "modify" ) {
		...
		if( $row['p_code'] == ''){ error_loc_msg("_product.list.php","삭제된 상품입니다."); }
		...
		// 배송상품이 아닐경우
		if( $row['p_type'] != 'delivery'){ error_loc("_product_ticket.form.php?".$_SERVER['QUERY_STRING']); }
...
<?php } ?>
	...
	<?php 
		...
		echo _submitBTN('_product.list.php', '', ($_mode == 'modify'  ? '<li><a href="#none" class="c_btn h46 blue line" id="product_copy">복사하기</a></li>' : null)); 
	?>
...
<script language="javascript">
		...
		if(!cpid) {
			...
			<?php
				if( $AdminPath == 'totalAdmin'){
					...
					echo "window.open('/subAdmin/_config.delivery.form.php');";
				}
			?>
		}
	}
	...
	function reqinfo_popup() {
		window.open('_product_reqinfo.popup.php?pass_code=<?php echo $_code; ?>','reqinfo','width=1350,height=700,scrollbars=yes');
	}
