		...
		<table class="category_list">
			...
			<?php 
				foreach($res as $k=>$v){ 
					...
					$printOnclickEvt = $v['dms_depth'] != 2 ? " onclick=\"configDisplayMain.viewList('".$v['dms_depth']."','".$v['dms_uid']."');\" style='cursor:pointer;' ":"";
			?>
				...
				<tr class="item-list-tr <?=in_array($v['dms_uid'],array($locUid1,$locUid2)) == true ? 'hit':''?>"  data-depth= "<?=$v['dms_depth']?>" data-uid="<?=$v['dms_uid']?>"  >
					<td>
						...
						<div class="ctg_name" <?=$printOnclickEvt?>><?=$v['dms_name']?></div>
						<div class="ctrl">
							<?php if($viewDepth != 1) { ?> 
							...
							<?php } ?>
							<a href="#none" onclick="configDisplayMain.viewItemForm('modify','<?=$v['dms_depth']?>','<?=$v['dms_uid']?>'); return false;" class="c_btn h22 t2 dark line scrollto" data-scrollto="view-form">수정</a>
						</div>
					</td>
				</tr>
			<?php } ?>
