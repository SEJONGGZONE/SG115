...
?>
    <form name="searchfrm" id="searchfrm" method=get action='<?=$_SERVER["PHP_SELF"]?>' class="data_search">
		...
		<div class="group_title">
			...
			<div class="btn_box">
                ...
                <?if ($searchMode == 'true') { ?>
                    <a href="_config.admin.list.php" class="c_btn h34 black line normal btn_reset">검색 초기화</a>
                <?php } ?>
                <a href="_config.admin.form.php?_mode=add&_PVSC=<?=$_PVSC?>" class="c_btn h46 red">운영자 등록</a>
            </div>
		</div>
					...
					<tr>
						...
						<td>
                            <div class="lineup-row type_multi">
                                <?php echo _InputSelect( "pass_input_type" , array('all','id','name','email','htel') , $pass_input_type , "" , array('전체검색','아이디','성명','이메일','휴대폰'), '' ); ?>
    ...
    <div class="data_list">
        ...
        <div class="list_ctrl">
            <div class="left_box">
                <a href="#none" onclick="selectAll('Y'); return false;" class="c_btn line h27">전체선택</a>
                <a href="#none" onclick="selectAll('N'); return false;" class="c_btn line h27">선택해제</a>
                <a href="#none" onclick="selectDeleteAdmin(); return false;" class="c_btn h27 line dark">선택삭제</a>
            </div>
        </div>

        <table class="table_list">
            <colgroup>
                <col width="40"/><col width="70"/><col width="100"/><col width="*"/><col width="*"/><col width="*"/><col width="*"/><col width="110"/><col width="110"/>
            </colgroup>
            <thead>
                <tr>
            ...
            <?php
                foreach($res as $k=>$v){
                    ...
                    $printAdminUse = ''.($v['a_use'] == 'Y' ? '<span class="c_tag blue line h18 t3">승인</span>':'<span class="c_tag light h18 t3">미승인</span>').''; // 승인상태
                    $printAdminRdate = rm_str($v['a_rdate']) > 0 ? date("Y-m-d",strtotime($v['a_rdate'])) : '-'; // 등록일
                    $printAdminRdate = rm_str($v['a_rdate']) > 0 ? printDateInfo($v['a_rdate']) : '-'; // 등록일
                    $printBtn = '
                        <div class="lineup-row type_center">
                            <a href="_config.admin.form.php?_uid='.$v['a_uid'].'&_mode=modify&_PVSC='.$_PVSC.'" class="c_btn gray h22">수정</a>
                            '.($v['a_type'] == 'admin' ? '<a href="#none" onclick="return false;" class="c_btn h22 dark line on-admin-delete" data-admin-id="'.$v['a_id'].'" data-admin-uid="'.$v['a_uid'].'" >삭제</a>' : '').'
                        </div>
                    '; // 관리버튼

                    // -- 출력
                    echo '<tr>';
                    echo '	<td class="this_check"><label class="design"><input type="checkbox" name="adminUid[]" value="'.$v['a_uid'].'" class="js_ck"></label></td>';
                    echo '	<td class="this_num">'.$_num.'</td>';
					 echo '	<td class="this_state">'.$printAdminUse.'</td>';
                    echo '	<td>'.$v['a_id'].'</td>';
                    echo '	<td>'.$v['a_name'].'</td>';
                    echo '	<td>'.$printHtel.'</td>';
                    echo '	<td>'.$printEmail.'</td>';
                    echo '	<td class="this_date">'.$printAdminRdate.'</td>';
