...
<form name="form_sms" method="post" action="_sms.pro.php" target="common_frame" enctype="multipart/form-data">
	...
	<div class="total_sms if_sendpage">
		<div class="single_send_layout">
			<div class="left">

				<div class="send_info">
					<div class="user_phone">
						<div class="form_box">
							<div class="title">휴대폰번호 입력</div>
							<textarea name="send_to_num" tabindex="1" id="send_to_num" type="text" class="design result" placeholder="번호를 입력해주세요." ></textarea>
							<span class="c_btn h27 blue line"><input type="button" name="" tabindex="4" value="+ 받는사람 추가" onclick="send_list_add(form_sms)" /></span>
						</div>
						<div class="form_box">
							<div class="title">받는사람 총 <strong id="slt_phonecnt">0</strong> 명</div>
							<select name="send_list" id="send_list" class="result" multiple></select>
							<a class="c_btn h27 dark line" href="#none" onclick="send_list_delete(form_sms);return false;">- 선택 삭제하기</a>
						</div>
					</div><!-- end user_phone -->
					...
					<div class="reserv_time">
						<label class="design"><input type="checkbox" name="_reserv_chk" id="_reserv_chk" value="Y" />예약 전송하기</label>
						<?PHP
			...
			<div class="right">
				...
				<div class="sms_phone">
					<div class="send_number">
						발신번호 <strong><?=$siteInfo[s_glbtel]?></strong>
				...
				<span class="c_btn h46 red this_send_btn"><input type="button" name="" onclick="send_ok(form_sms); return false;" value="문자 전송하기" /></span>

			</div>
		</div><!-- end single_send_layout -->
				...
				<td>
					<div class="lineup-row">
						<?php
							if($SMSUser['code'] == 'U04') {
								echo '
									<a class="c_btn h46 line gray  " href="_config.default.form.php#sms_send_tel" >발신번호 수정</a>
								';
							}
							if($SMSUser['code'] == 'U00') {
								echo '
									<a class="c_btn h46 black line gray  " href="_config.sms.out_list.php" >'.number_format($SMSUser['data'],1).'건 남음</a>
								';
							}
						?>
						<a class="c_btn h46 black only_pc_view" href="_config.sms.out_list.php?type=charge" >충전하기</a>
						<a class="c_btn h46 sky only_pc_view"  href="http://mobitalk.gobeyond.co.kr/" target="_blank">모비톡 바로가기</a>
					</div>
				</td>
			</tr>
			<tr>
				...
				<td>
					<div class="tip_box">
						<div class="c_tip">문자 제목은 <strong>LMS, MMS</strong>의 경우에만 발송됩니다.</div>
						<div class="c_tip">이미지 등록 시 자동으로 MMS 형태로 변경되며 <u>JPG만 업로드 가능</u>합니다.</div>
...
<?
    ...
    if($ctrlMode) {
        ...
        switch($ctrlMode){
            ...
            if( count($chk_cellular) < 1){ 
				$chk_cellular = array();
				$arr_chk_cellular = $arrID;
			}
			
            // --- 선택회원 ---
            $sres = _MQ_assoc("select in_tel2 from smart_individual where find_in_set(in_id,'".implode(',',$arr_chk_cellular)."') > 0 and in_sleep_type = 'N'  and in_out = 'N'  and in_smssend = 'Y' and in_userlevel != '9'  ");
...
?>
    <SCRIPT>
        $(document).ready(function(){
            ...
            $("#slt_phonecnt").html("<?php echo sizeof($chk_cellular)?>");
        });
    </SCRIPT>
<?
    }
