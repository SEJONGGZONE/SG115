...
<SCRIPT LANGUAGE="JavaScript">
	$(document).ready(function() {
		opener.$('.js_relation_select').show();
		opener.$('.js_relation_cnt').text('<?php echo count($arr_str_view); ?>');
		opener.$('input[name=_relation]').val('<?php echo $app_str; ?>');
		opener.$('.js_relation_view').text('<?php echo addslashes($app_str_view); ?>');
		opener.controller_realtion_display();
		close();
	});
</SCRIPT>