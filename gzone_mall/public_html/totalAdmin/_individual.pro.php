<?php // -- LCY -- 회원관리 AJAX 처리
	...
	switch ($_mode) {
		...
		case "modify":
			...
			// 승인상태가 Y이고 기존 상태값이랑 다를 경우 처리
			$chk = _MQ("select in_auth from smart_individual where in_id = '".$_id."' ");
			if($_auth == 'Y' && $chk['in_auth'] != $_auth){
				couponIssuedAutoType4($_id);
			}
