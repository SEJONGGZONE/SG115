<?
	...
	switch($mode){
		...
		case "rollback":
			...
			if( $SubAdminMode === true && in_array($uid, $arr_sms_text_com_allow_type)){
				if($ma == 'c'){ $uid = 'com_'.$uid; }
			}
			...
			$rollback = array(
				...
				"com_order_pay"		=> "[{사이트명}] 주문결제가 완료되었습니다. 관리자페이지에서 확인해 주십시오. [주문번호: {주문번호}]",

				// 상품배송시
				"delivery"				=> "[{사이트명}] 고객님이 주문하신 상품이 발송되었습니다. 마이페이지에서 배송상황을 확인하실 수 있습니다. 감사합니다. [주문번호: {주문번호}]",
				"admin_delivery"		=> "[{사이트명}] 상품이 발송되었습니다. [주문번호: {주문번호}]",
				"com_delivery"		=> "[{사이트명}] 상품이 발송되었습니다. [주문번호: {주문번호}]",

				// 티켓발급 시
				"ticket"			=> "[{회원명}] 고객님 주문하신 티켓이 발급되었습니다.",
				"admin_ticket"	=> "[{회원명}] 고객님의  주문  티켓이 발급되었습니다.",
				"com_ticket"	=> "[{회원명}] 고객님의  주문  티켓이 발급되었습니다.",

				// 티켓사용완료 시
				"ticket_use"			=> "{주문자명}님 티켓이 사용완료되었습니다. 티켓번호: {티켓번호}",
				"admin_ticket_use"	=> "{주문자명}님의 티켓이 사용완료되었습니다. 티켓번호: {티켓번호}",
				"com_ticket_use"	=> "{주문자명}님의 티켓이 사용완료되었습니다. 티켓번호: {티켓번호}",

				// 티켓사용취소 시
				"ticket_unuse"			=> "{주문자명}님 티켓이 사용취소되었습니다. 티켓번호: {티켓번호}",
				"admin_ticket_unuse"	=> "{주문자명}님의 티켓이 사용취소되었습니다. 티켓번호: {티켓번호}",								
				"com_ticket_unuse"	=> "{주문자명}님의 티켓이 사용취소되었습니다. 티켓번호: {티켓번호}",								


				// 주문취소시
				"order_cancel"			=> "[{사이트명}] 주문이 정상적으로 취소되었습니다. [주문번호: {주문번호}]",
				"admin_order_cancel"	=> "[{사이트명}] 주문이 정상적으로 취소되었습니다. [주문번호: {주문번호}]",
				"com_order_cancel"	=> "[{사이트명}] 주문이 정상적으로 취소되었습니다. [주문번호: {주문번호}]",
				...
				"com_order_cancel_part"   => "[{사이트명}] 주문하신 상품 중 일부 상품의 주문이 취소되었습니다. [주문번호: {주문번호}] [취소된상품: {주문상품명}]",
                ...
                "com_product_review"          => "[{사이트명}] {회원명} 님께서 상품후기를 등록하셨습니다. [상품: {후기(문의)상품명}]"
                ...
                "com_product_talk"            => "[{사이트명}] {회원명} 님께서 상품문의를 접수하셨습니다. 관리자페이지에서 답변을 등록해 주십시오. [상품: {후기(문의)상품명}]"
...
감사합니다."
		...
		case "load":
			...
			// LCY : 2023-05-31 : 입점업체 SMS추가
			if( $SubAdminMode === true && in_array($type, $arr_sms_text_com_allow_type) ){

				// 입점관리자에게 발송 정보
				$r = _MQ_assoc("select * from smart_sms_set where ss_uid = 'com_".$type."'");
				foreach($r as $k => $v) {
					$c_name = $arr_sms_text_type[$type];
					$c_uid = $v[ss_uid];
					$c_status = $v[ss_status];
					$c_text = $v[ss_text];
					$c_title = $v[ss_title];
					$c_file = $v[ss_file];

					$ck_name = $arr_sms_text_type[$type];
					$ck_uid = $v['ss_uid'];
					$ck_status = $v['kakao_status'];
					$ck_knum = $v['kakao_templet_num'];
					$ck_kadd1 = $v['kakao_add1'];
					$ck_kadd2 = $v['kakao_add2'];
					$ck_kadd3 = $v['kakao_add3'];
					$ck_kadd4 = $v['kakao_add4'];
					$ck_kadd5 = $v['kakao_add5'];
					$ck_kadd6 = $v['kakao_add6'];
					$ck_kadd7 = $v['kakao_add7'];
					$ck_kadd8 = $v['kakao_add8'];
				}	
