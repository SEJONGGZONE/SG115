...
if(in_array($_mode, array('get_excel', 'get_search_excel'))) { // Excel 다운로드 Start
	...
	# 모드별 쿼리 조건
	if($_mode == 'get_excel') $s_query = " and o.o_canceled = 'N' and o.o_paystatus = 'Y' and npay_order = 'N' and op.op_cancel = 'N' and op.op_uid in ('".implode("', '", $_uid)."') and  op_ptype = 'delivery' /*티켓제외*/ ";
	else $s_query = enc('d', $_search_que);
	if(!$st) $st = 'o_rdate';
	if(!$so) $so = 'desc';
	$res = _MQ_assoc("
		select
			op.*, o.*, p.p_name, p.p_img_list_square , p.p_img_list
		from
...
?>
	<table>
		<thead>
			<tr>
			<?php 
				foreach($excel_keys as $k=>$v){
			?>
				<th<?php echo $THStyle; ?>><?php echo $v; ?></th>
			<?php } ?>
			</tr>
		</thead>
		<tbody>
...
<?php
	...
	foreach($r as $k=>$v) {
		...
		_MQ_noreturn(" update smart_order_product set op_sendstatus = '{$select_sendstatus}', op_sendcompany = '{$_sendcompany[$v['op_uid']]}', op_sendnum = '{$_sendnum[$v['op_uid']]}' {$que_tmp} where op_uid = '{$v['op_uid']}' and op_ptype = 'delivery' /*티켓제외*/  ");
		...
		if( !IN_ARRAY($v['op_sendstatus'] , array('배송중' , '배송완료')) && IN_ARRAY($select_sendstatus , array('배송중' , '배송완료')) ) {
			if(!$arr_ordernum_sms[$v['op_oordernum']] ) {
				...
				if($v['o_otel'] || $v['o_ohp']) {
					...
					$sms_pname = trim($v['op_pname']) . implode(" / ",$arr_option_name);
					...
					// LCY : 2023-05-31 : 입점업체 SMS추가 -- 업체 코드를 추출하여 보낸다. 
					$arr_sms_replace['{comid}'] = $v['op_partnerCode'];
	...
	if($select_sendstatus == '배송중') {
		...
		foreach($OrderData as $k=>$v) {
			...
			$opr = _MQ_assoc("
						select op.* , p.p_name, p.p_img_list_square , p.p_img_list , p.p_code
						from smart_order_product as op
	...
	foreach($w_check as $k=>$v) {
		...
		$EData[$k] = array(
			'_sendstatus'=>($_sendstatus[$k]?$_sendstatus[$k]:'배송대기'), // LCY : 2023-06-13 : 송장예외처리 추가 :: 상태없을 경우 기본 배송대기
	...
	foreach($EData as $k=>$v) {
		...
		// LCY : 2023-06-13 : 송장예외처리 추가 :: 업로드시 송장 또는 택배사 없을 경우 패스
		if( !IN_ARRAY($v['_sendstatus'] , array('배송대기','배송준비')) ){
			if( $v['_sendcompany'] == '' || $v['_sendnum'] == ''){continue;}
		}

		// LCY : 2023-06-28 : 송장오류 -- 아이디 보안 추가
		$partner_query = '';
		if( $AdminPath != 'totalAdmin'){
			$partner_query = " and  op_partnerCode = '".addslashes($pass_com)."' ";
		}
		$r = _MQ("select * from smart_order_product where op_uid = '{$k}' ".$partner_query);
		...
		// 문자발송
		if(($r['op_sendstatus'] == '배송대기' || $r['op_sendstatus'] == '배송준비') && ($v['_sendstatus'] == '배송중' || $v['_sendstatus'] == '배송완료')) {
			if(!$arr_ordernum_sms[$k]) {
				...
				if($order_info['o_otel'] || $order_info['o_ohp']) {
					...
					// KAY :: 2022-12-07 :: 옵션 구분 수정
					$arr_option_name = array($r['op_option1'] ,$r['op_option2'],$r['op_option3'] );
					$arr_option_name = array_filter($arr_option_name);

					$sms_pname = trim($r['op_pname']) . implode(" / ",$arr_option_name);
					$arr_sms_replace = array('{주문번호}'=>$order_info['op_oordernum'], '{주문상품명}'=>$sms_pname, '{택배사}'=>$v['_sendcompany'], '{운송장번호}'=>$v['_sendnum'], '{배송일}'=>date('Y-m-d'));

					// LCY : 2023-05-31 : 입점업체 SMS추가 -- 업체 코드를 추출하여 보낸다. 
					$arr_sms_replace['{comid}'] = $r['op_partnerCode'];
	...
	if(count($op_oordernum) > 0) {
		...
		foreach($OrderData as $k=>$v) {
			...
			$opr = _MQ_assoc("
						...
						from smart_order_product as op
						left join smart_product as p on ( p.p_code=op.op_pcode )
						where op_oordernum = '{$v}' and op.op_cancel = 'N' and op_ptype = 'delivery' /*티켓제외*/
					");
			include(OD_MAIL_ROOT.'/shop.order.mail_delivery.php');
			$_title = "[{$siteInfo['s_adshop']}] 주문하신 상품이 발송되었습니다!";
			$_content = get_mail_content($mailing_app_content); // 메인 본문조합
	...
	// 배송완료가 아닌 상품을 걸러냄
	$chk = _MQ_assoc(" select op_uid from smart_order_product where op_uid in ('".implode("' , '", array_values($_uid))."') and op_sendstatus != '배송완료' and op_ptype = 'delivery' /*티켓제외*/ ");
	$error_msg = '';
	if(count($chk) > 0) {
	...
	// 정산대기 전환
	_MQ_noreturn(" update smart_order_product set op_settlementstatus='ready', op_settlement_reday = now() where op_uid in ('".implode("' , '", array_values($_uid))."') and op_settlementstatus='none' and op_ptype = 'delivery' /*티켓제외*/ ");
