<?php
	...
	switch ($ajaxMode) {
		...
		case 'execAddfile': // 폼에서 파일추가를 클릭 시
			...
			$html = '		<div class="lineup-row tr-files" data-mode="add" data-idx="'.$nextIdx.'">';
			$html .= '		<div class="input_file" style="width:250px">';
			$html .= '			<input type="text" id="fakeFileTxt'.$nextIdx.'" class="fakeFileTxt" readonly="readonly" placeholder="파일찾기" disabled>';
			...
			$html .= '			</div>';
			$html .= '		</div>';
			$html .= '		<a href="#none" onclick="return false;" class="c_btn h27 black line exec-delfile">삭제</a>';
			$html .= '	</div>';
		...
		case 'listComment':
			...
			$printList .= '<table class="table_list type_nocheck">';
			$printList .= '	<colgroup>';
			$printList .= '	<col width="80"/><col width="*"/><col width="130"/><col width="100"/><col width="110"/>';
			$printList .= '	</colgroup>';
			$printList .= '	<thead>';
			$printList .= '		<tr>
									<th scope="col">No</th>
			...
			foreach($res as $k=>$v){
				...
				$printList .= '			<td class="this_num">'.$_num.'</td>';
				$printList .= '			<td class="t_left">
											<div class="comment-content" data-uid="'.$v['bt_uid'].'">'.nl2br(stripslashes($v['bt_content'])).'</div>
											<div class="modify-comment-form" style="display:none; width:100%" data-uid="'.$v['bt_uid'].'">
												...
												<div class="lineup-row">
													<a href="#none" class="c_btn blue modify-comment-submit" data-uid="'.$v['bt_uid'].'">수정</a>
													<a href="#none" class="c_btn h34 gray cancel-comment-item" data-uid="'.$v['bt_uid'].'">취소</a>
												</div>
											</div>
										</td>';
				$printList .= '			<td class="this_state">'.showUserInfo($v['bt_inid'],$v['bt_writer'],$v).'</td>';
				$printList .= '			<td class="this_date">'.printDateInfo($v['bt_rdate']).'</td>';
				$printList .= '			<td class="this_ctrl">';
				$printList .= '				<div class="lineup-row type_center">';
				$printList .= '					<a href="#none" onclick="return false;" class="c_btn h22 gray modify-comment-item" data-uid = "'.$v['bt_uid'].'">수정</a>';
				$printList .= '					<a href="#none" onclick="return false;" class="c_btn h22 dark line delete-comment-item" data-uid = "'.$v['bt_uid'].'">삭제</a>';
				$printList .= '				</div>';
				$printList .= '			</td>';
				$printList .= '		</tr>';
			}
