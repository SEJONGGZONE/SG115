<?php // -- LCY -- 회원관리 AJAX 처리
	...
	switch ($_mode) {
		...
		// -- 선택회원,검색회원 휴면처리 
		case "getSleepReturnId":

			if( $_id=='' ){ error_msg("회원을 선택해 주세요."); }

			$id_chk = _MQ_result("select count(*) as cnt from smart_individual_sleep where 1 and in_sleep_type = 'Y' AND in_out = 'N' and in_id='".$_id."' ");
			if( $id_chk < 1){ error_msg('회원검색에 실패하였습니다.'); }

			$result =  member_sleep_return($_id);

			if( $result =='Y'){
				error_loc_msg("_individual_sleep.list.php?".enc('d' , $_PVSC ),'회원이 휴면해제 되었습니다.');				
			}else{
				error_loc_msg("_individual_sleep.list.php?".enc('d' , $_PVSC ),'회원 휴면해제에 실패하였습니다.');
			}
