...
if($ordr['o_ordernum'] == ''){
	...
	if( in_array($ordr['o_paymethod'] , $arr_cancel_part_payment_type ) && $ordr['o_paystatus']=='Y') {
		if($_force_cancel || $_total_amount < 1 || $ordr['op_cancel_type'] == 'point') {
			...
			switch($siteInfo['s_pg_type']) {
				...
				case "inicis" :
					...
					require(OD_PROGRAM_ROOT."/pg.cancle_part_inicis_api.php");
					// LCY : 2022-12-28 : 이니시스TX모듈 교체 -- 결제수단 추가 }
					break;
				case "allthegate" :
		...
		if ($is_pg_status) {	// pg모듈 호출 상태
			...
			// LCY : 2021-03-22 : 상품 티켓유형 추가 -- 티켓주문이 있을 경우 티켓취소
			if( $ordr['op_ptype'] == 'ticket'){
				include(OD_PROGRAM_ROOT."/shop.order.ticketdel_part.php");
			}
			...
			$arr_sms_replace = array('{주문번호}'=>$_ordernum, '{주문상품명}'=>$sms_pname, '{comid}'=>$ordr['op_partnerCode']);
			shop_send_sms($sms_to,$smskbn,$arr_sms_replace); // LCY : 2023-05-31 : 입점업체 SMS추가 -- comid 추가
	...
	// - 무통장입금 취소 ---
		...
		$sms_pname = trim($ordr['op_pname']) . implode(" " , array_filter(array(' '.$ordr['op_option1'],$ordr['op_option2'],$ordr['op_option3'])));
		$arr_sms_replace = array('{주문번호}'=>$_ordernum, '{주문상품명}'=>$sms_pname , '{comid}'=>$ordr['op_partnerCode']); // LCY : 2023-05-31 : 입점업체 SMS추가 -- comid 추가
