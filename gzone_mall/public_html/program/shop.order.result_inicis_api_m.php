...
if($r[o_paymethod] == "card") $P_INI_PAYMENT = "CARD";
if($r[o_paymethod] == "iche") $P_INI_PAYMENT = "BANK";
if($r[o_paymethod] == "virtual") $P_INI_PAYMENT = "VBANK";
if($r[o_paymethod] == "hpp") $P_INI_PAYMENT = "MOBILE"; // 휴대폰 결제

?>
<!-- 스크립트-->
<form id="form1" name="ini" method="post" action="" encoding="euc-kr" accept-charset="EUC-KR" >

<input type="hidden" name="P_INI_PAYMENT" id="textfield2" value="<?=$P_INI_PAYMENT?>"/> <?php // LCY : 2022-12-28 : 이니시스TX모듈 교체 -- 결제수단 추가 ?>
...
<input type="hidden" name="P_NEXT_URL" value="<?php echo $siteDomain ?><?php echo OD_PROGRAM_DIR; ?>/shop.order.result_inicis_api_m.pro.php">  <?php  // 인증결과수신 Url  ?>
<input type="hidden" name="P_NOTI_URL" value="<?php echo $siteDomain ?><?php echo OD_PROGRAM_DIR; ?>/shop.order.result_inicis_api_m_paying.php"><?php // 승인결과통보 Url :: 한글도메인 사용불가  ?>
...
function call_pay_form()
{
	...
	order_form.action = "//mobile.inicis.com/smart/payment/";
	order_form.submit();
}
<?php // LCY : 2022-12-28 : 이니시스TX모듈 교체 } ?>
