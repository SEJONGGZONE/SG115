			...
			if($MobileAdmin === true) {
				$str = "
					...
					<div class=\"select\">
						...
						". _InputSelect( 'pass_parent01' , array_keys($arr_parent01) , $app_depth1 , 'id="pass_parent01" onchange="category_select(1);" ' , array_values($arr_parent01) , '1차분류 선택')  ."
					</div>
					<div class=\"select\">
						<span class=\"shape\"></span>
						". _InputSelect( 'pass_parent02' , array_keys($arr_parent02) , $app_depth2 , 'id="pass_parent02" onchange="category_select(2);" ' , array_values($arr_parent02) , '2차분류 선택')  ."
					</div>
					<div class=\"select\">
						<span class=\"shape\"></span>
						". _InputSelect( '_cuid' , array_keys($arr_parent03) , $app_depth3, ' onchange="category_chk();" hname="상품분류" '.($pass_parent03_no_required=='Y' ? '':'required').'  ' , array_values($arr_parent03) , '3차분류 선택')  ."
					</div>
				";
			}
			else {
			...
			$.ajax({
				...
				success: function(data){
					if(_idx == 2) {
						...
						$("select[name=_cuid]").find("option").remove().end().append('<option value="">3차분류 선택</option>');
						var option_str = '';
						...
						$("select[name=pass_parent02]").find("option").remove().end().append('<option value="">2차분류 선택</option>');
						var option_str = '';
						for (var i = 0; i < data.length; i++) {
							option_str += '<option value="' + data[i].optionValue + '" >' + data[i].optionDisplay + '</option>';
						}
