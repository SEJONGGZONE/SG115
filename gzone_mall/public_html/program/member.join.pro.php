...
switch( $_mode ){
	...
	case "join":
		...
		if($join_pw && $join_repw && $join_pw == $join_repw) {
			...
			if($siteInfo['join_pw_up_use'] == 'Y' && $siteInfo['join_pw_up_length'] > 0) {
				...
				$pw_up_pattern_result = array_filter($pw_up_pattern_result);

				if(count($pw_up_pattern_result[0]) < $siteInfo['join_pw_up_length']) error_alt('비밀번호에는 대문자가 '.$siteInfo['join_pw_up_length'].'개 이상 포함되어야합니다');
			}
			...
			if($siteInfo['join_pw_sp_use'] == 'Y' && $siteInfo['join_pw_sp_length'] > 0) {
				...
				// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -------- 인증 정책
				$pw_sp_pattern_result = array_filter($pw_sp_pattern_result);

				if(count($pw_sp_pattern_result[0]) < $siteInfo['join_pw_sp_length']) error_alt('비밀번호에는 특수문자(~!@#$%^&*()_+|<>?:{})가 '.$siteInfo['join_pw_sp_length'].'개 이상 포함되어야합니다');
			}
		}
		// --사전 체크 ---
		...
		// --- JJC : 하이앱 : 2020-04-22 : 접속 디바이스 체크 ---
		if( function_exists('is_app')  ) { 
			$this_device = is_app() === true ? 'APP' : $this_device;
		}
		// --- JJC : 하이앱 : 2020-04-22 : 접속 디바이스 체크 ---
		...
		if($siteInfo['join_approve'] == 'Y') { // 승인후 로그인 처리
			...
			$enc_login_id = site_secure_encode($join_id);
		...
		/*
			if($AuthSNSEncID) {
				...
				if($_COOKIE['AuthSNSEncID']) samesiteCookie('AuthSNSEncID', '', time()-3600 , '/', '.'.str_replace('www.', '', $system['host'])); // SNS 고유정보 제거
			}
		*/
		...
		// 승인형태가 자동 승인일 경우에만 처리
		if($siteInfo['join_approve'] == 'Y'){
			// 회원가입 쿠폰 발급
    ...
    case "modify":
        ...
        if( (mailCheck($join_email) && ( $mem_info['in_emailsend'] <> $_mailling || $mem_info['in_smssend'] <> $_sms ) && ( $_mailling || $_sms ) && count($mem_info) > 0 ) && $mem_info['in_userlevel']!='9' ){
        ...
        ";
		if( $join_pw && $join_repw && ( $join_pw == $join_repw )){
			$sque .= " , in_pw = '".db_password($join_pw)."', in_pw_rdate = now() ";// --- modify source ---
		}
		// -- query 사전 준비 ---
	...
	case "delete":
		// 비밀번호 체크
		$r = _MQ("select count(*) as cnt from smart_individual where in_id='". get_userid() ."' and in_pw = '".db_password($leave_pw)."'  ");
...
//		_MQ_noreturn($que);
	...
	case 'password_change' :
		...
		if($_cpw && $_rcpw && $_cpw == $_rcpw) {
			...
			if($siteInfo['join_pw_limit_max'] > 4 && strlen($_cpw) > $siteInfo['join_pw_limit_max']) error_alt('비밀번호는 최대 '.$siteInfo['join_pw_limit_max'].'자 까지만 입력가능합니다'); // 최대 글자 수 체크
		...
		_MQ_noreturn("update smart_individual set in_pw_rdate = now(), in_pw = '".db_password($_cpw)."'  where in_id = '".$_id."' ");
