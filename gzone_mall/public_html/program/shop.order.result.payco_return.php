<?PHP
	...
	if($code != '0'){ // 사용자 취소
		 ...
		 if($code == 2222){
			echo "<script>alert('결제요청을 취소하셧습니다. [ERROR CODE ".$code."]'); opener.location.href='/?pn=shop.order.form'; window.close();  </script>";
			exit;
		 }
			echo "<script>alert('결제에 실패하였습니다. [ERROR CODE  ".$code."]'); opener.location.href='/?pn=shop.order.form'; window.close();  </script>";
			exit;
	}
	...
	if( $totalPaymentAmt != $ItemTotalOrderAmt ){    // 위에서 파라메터로 받은 totalPaymentAmt 값과 주문값이 같은지 비교합니다.
		...
		echo "<script>alert('결제요청 금액과 결제승인된 금액이 일치하지 않습니다.'); opener.location.href='/?pn=shop.order.form'; window.close();  </script>";
		exit;
	...
	if ( $doApproval == true ){
       ...
       // $paymethod_code = $Read_Data['result']['paymentDetails'][0]['paymentMethodCode']; // 결제코드
		...
		if($Read_Data["code"] == '0'){	// 결제 성공
			...
			switch($paymethod_code){
				case "02": // 무통장 입금
					...
					echo "<script>opener.location.href='/?pn=shop.order.complete&".$orderNo."'; window.close();  </script>";
					exit;
			...
			echo "<script>alert('중복결제가 확인 되어 결제 처리가 되지 않았습니다.'); opener.location.href='/?pn=shop.order.form'; window.close();  </script>";
			...
			_MQ_noreturn("update smart_order set o_status='결제실패' where o_ordernum='". $ordernum ."' ");
			echo "<script>alert('결제승인에 실패하였습니다.[".$Err_message."]'); opener.location.href='/?pn=shop.order.form'; window.close();  </script>";
			exit;
		...
		if($code != 0){
			...
			echo "<script>alert('결제승인요청에 실패하였습니다. 에러코드[".$code."]'); opener.location.href='/?pn=shop.order.form'; window.close();  </script>";
			exit;
		}
	}
