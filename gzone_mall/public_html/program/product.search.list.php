...
$search_hashtag = urldecode($search_hashtag);
$detail_search = urldecode($detail_search);
...
if(isset($search_word) && $search_word != '') {
	...
	$search_word = trim($search_word); // 검색키워드
...
if(isset($search_price) && $search_price != '') {
	...
	*/
	...
	$sub_query .= " and p_price ".($search_price_arr[1] == 'lower'?' <= ':' >= ') . rm_str($search_price_arr[0]);
}
...
if($TotalCount <= 0) {
	...
	$HitProduct = _MQ_assoc(" select *, (select count(*) as cnt from smart_product_talk where pt_pcode=p_code and pt_type='상품리뷰' and pt_intype='normal') as review_cnt from smart_product where p_stock > 0 and p_soldout_chk = 'N' and p_view = 'Y' and p_option_valid_chk = 'Y' order by ".($siteInfo['s_search_diff_orderby'] == 'salecnt'?' p_salecnt desc':'review_cnt desc')." limit 0, {$HitProductCount} ");
