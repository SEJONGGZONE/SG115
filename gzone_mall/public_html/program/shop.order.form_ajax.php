...
switch ($ajaxMode) {
	case 'couponSelete': // return => JSON ::  사용자 쿠폰 선택 시
		...
		if( $rowCouponInfo['ocs_issued_type'] == 'auto' && in_array($rowCouponInfo['ocs_issued_type_auto'], array(1,2)) > 0 ){
			// 주문조회하여 취소되지 않은 주문건중 배송완료 또는 발급완료 주문건에 한해서 처리 
			$check_order = _MQ("select count(*) as cnt from smart_order where o_paystatus = 'Y' and o_canceled = 'N' and o_status in('배송완료','발급완료') and o_ordernum = '".$rowCouponInfo['coup_checksum']."' ");
			if( $check_order['cnt'] < 1){
				echo json_encode(array('rst'=>'fail','msg'=>'본 쿠폰은 주문시 지급된 이벤트 쿠폰으로 해당 주문건의 배송 완료이후 사용 가능합니다.')); exit; 	
			}
		}
