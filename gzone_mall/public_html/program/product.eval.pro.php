...
if( in_array($_mode , array("add","delete" )) ){
	...
	if( $_FILES['_img']['name'] ) {
		...
		// -- SSJ : 상품리뷰 이미지 리사이즈 및 회전방지 : 2021-05-24 ----
...
switch( $_mode ){
	...
	case "add":
		...
		if( true ) { // SSJ : 상품리뷰 작성제한 기능강화 : 2020-08-13 : 모든 상품리뷰 설정 적용
			// 포토리뷰 작성횟수 추출
			$talk_type = 'eval'; // 상품리뷰
			$que = "
				select count(*) as cnt
				...
				where 1
					...
					and pt_depth = 1
			"; // SSJ : 상품리뷰 작성제한 기능강화 : 2020-08-13 : 모든 상품리뷰 설정 적용
			$er = _MQ($que);
			...
			if($siteInfo['s_producteval_limit']<>'N'){
				...
				if($or['cnt'] < 1){
					error_alt('상품리뷰는 상품을 구매한 회원만 작성 가능합니다.'); // SSJ : 상품리뷰 작성제한 기능강화 : 2020-08-13 : 포토리뷰 문구를 상품리뷰로 변경
				}
				else if($siteInfo['s_producteval_limit']=='B'){
					if($or['cnt'] <= $er['cnt']){
						error_alt('상품리뷰는 상품을 구매한 횟수 만큼만 등록할 수 있습니다.'); // SSJ : 상품리뷰 작성제한 기능강화 : 2020-08-13 : 포토리뷰 문구를 상품리뷰로 변경
					}
				}
			}

            if($_img_name){ // SSJ : 상품리뷰 작성제한 기능강화 : 2020-08-13 : 포토리뷰일 경우에만 포인트 지급
			    // 포인트 지급 체크
        ...
        if( $trigger_point ) {
            shop_pointlog_insert( get_userid() , "포토리뷰 등록 (상품코드: {$pcode})" , $siteInfo[s_productevalpoint] , "N" , $siteInfo[s_productevalprodate]);
        }

        // -- 2019-04-09 SSJ :: 상품리뷰 등록 시 문자 연동 ----
        ...
        // LCY : 2023-05-31 : 입점업체 SMS추가 p_cpid
        $stringsAdd['{comid}'] = _MQ_result(" select p_cpid from smart_product where p_code = '{$pcode}' ");

        shop_send_sms($sms_to,"product_review",$stringsAdd);
        // -- 2019-04-09 SSJ :: 상품리뷰 등록 시 문자 연동 ----

		$review_cnt = get_talk_total($pcode,"eval","normal");

		echo "<script>parent.eval_frm.reset();parent.eval_view(".$review_cnt.");parent.iframe_init(false);</script>";
	...
	case "delete":
		...
		if( $tmp_r[pt_img] ) {
			...
			if($trigger_point){
				if($trigger_date) shop_pointlog_insert( get_userid() , "포토리뷰 삭제 (상품코드: ".$tmp_r['pt_pcode'].")" , $siteInfo['s_productevalpoint']*-1 , "N" , 0);
				else shop_pointlog_insert( get_userid() , "포토리뷰 삭제 (상품코드: ".$tmp_r['pt_pcode'].")" , $siteInfo['s_productevalpoint']*-1 , "N" , $point_days);
			}
		}
	...
	case "view":
		...
		$que = "
			select
				pt.* , (select count(*) as cnt from smart_product_talk_report where ptr_ptuid = pt_uid and ptr_inid = '".get_userid()."' ) as repcnt 

				/* {LCY} : 하이앱 -- 상품후기 회원 차단하기 기능 */
				, ( select count(*) as cnt from smart_individual_block where ib_inid = '".get_userid()."' and ib_type = '상품후기' and ib_block_inid = pt_inid  ) as blockcnt
			".$s_query."
