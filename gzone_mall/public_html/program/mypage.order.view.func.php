	...
	// ---------- JJC : 부분취소 개선 : 2021-02-10 : 함수부분  ----------
		...
		function partcancel_orderinfo($op_uid) {
			return _MQ("
				...
				WHERE 
					op_uid = '". $op_uid ."'
					". (is_admin() !== true && is_login() ? " AND o_mid = '".  get_userid() ."'" : "" ) ."
			");
		}
		// ------ 부분취소 전용 주문정보 추출 함수 -------
		...
		function partcancel_before($op_uid , $partcancel_orderinfo = array()) {
			...
			// --- 주문상품정보 ---
				// 환불요청 : 상품쿠폰 가격
					...
					if($arr_order['product_discount_price'] > 0 ) {
						$subres = _MQ(" SELECT COUNT(*) AS cnt FROM smart_order_product WHERE op_oordernum = '". addslashes($ordernum) ."' AND op_cancel = 'N' AND op_pcode='". $r['op_pcode'] ."'  AND op_is_addoption = 'N' "); // JJC : 2023-01-13 : 추가옵션 부분취소
						if($subres['cnt'] == 1 ) {
				...
				// 환불요청 : 배송가격
					...
					if($r['op_delivery_type'] == '개별') {
						...
						$subres = _MQ(" SELECT COUNT(*) AS cnt FROM smart_order_product  WHERE op_oordernum = '". addslashes($ordernum) ."' AND op_uid != '". addslashes($op_uid) ."' AND op_cancel = 'N' AND op_delivery_type='입점' AND op_partnerCode='". $r['op_partnerCode'] ."' AND op_is_addoption = 'N' "); // JJC : 2023-01-13 : 추가옵션 부분취소
					}
						...
						$subres = _MQ(" SELECT COUNT(*) AS cnt FROM smart_order_product  WHERE op_oordernum = '". addslashes($ordernum) ."' AND op_uid != '". addslashes($op_uid) ."' AND op_cancel = 'N' AND op_delivery_type='상품별' AND op_pcode='". $r['op_pcode'] ."' AND op_is_addoption = 'N' "); // JJC : 2023-01-13 : 추가옵션 부분취소
					}
			...
			// --- 환불정보 ---
				...
				$_addoption_price = $_addoption_cnt = $_addoption_name = $_addoption_addcnt =  "";
				$arr_addoption = array();
				$aores = _MQ_assoc("
					SELECT
						concat(op_option1,' / ',op_option2) as option_name , op_price , op_cnt 
					FROM smart_order_product 
					WHERE (1)
						AND op_oordernum = '".addslashes($ordernum)."' AND op_pcode='". $r['op_pcode'] ."'
						AND op_cancel = 'N' AND op_is_addoption = 'Y' AND op_addoption_parent = '".$r['op_pouid']."'
				");
				foreach( $aores as $k=>$v) {
					$arr_addoption['price'] += $v['op_price'] * $v['op_cnt'];
					$arr_addoption['cnt'] += $v['op_cnt'];
					$arr_addoption['name'][] = $v['option_name'];
					$arr_addoption['addcnt'][] = $v['op_cnt'];
				}
				$_addoption_price = $arr_addoption['price'];
				$_addoption_cnt = $arr_addoption['cnt'];
				$_addoption_name = sizeof($aores) > 0 ? implode(" , " , $arr_addoption['name']) : '';
				$_addoption_addcnt = sizeof($aores) > 0 ? implode(" , " , $arr_addoption['addcnt']) : '';
				// --- JJC : 2023-01-13 : 추가옵션 부분취소 ---
				...
				$__result_array2 = array(
					// - 요청 상품정보 -
						...
						'addoptioncnt' => trim($_addoption_addcnt), // 추가 옵션개수
						...
						'pro_cnt' => number_format($r['op_cnt']), // 주문 옵션 개수
		...
		function partcancel_request($op_uid , $cancel_type , $cancel_bank , $cancel_bank_account , $cancel_bank_name , $save_myinfo , $cancel_msg) {
			...
			// 공통 Query
			$sque = " WHERE op_oordernum = '". $ordernum ."' AND op_uid != '". $op_uid ."' AND op_cancel = 'N' AND op_is_addoption = 'N' "; // JJC : 2023-01-13 : 추가옵션 부분취소
				...
				if($subres['op_uid']) { 
					...
					_MQ_noreturn(" 
						UPDATE smart_order_product SET 
							op_delivery_price = '". rm_str($res['op_delivery_price'])."' , 
							op_add_delivery_price = '". rm_str($res['op_add_delivery_price']) ."'
						WHERE 
			...
			//			할인액 분할액 , 취소 할인액 변경
			$add_que .= " op_use_discount_price = '". rm_str($__result_array['return_discount']) ."' , op_cancel_discount_price = '". rm_str($__result_array['return_discount'])  ."' ,  "; // 추가 query // JJC : 2022-12-21 : 부분취소개선
			...
			_MQ_noreturn("
				UPDATE smart_order_product SET
					...
					op_cancel_mem_type = '".$cancel_mem_type."'
				WHERE (1)
					AND op_oordernum = '".addslashes($ordernum)."'  AND op_pcode='". $res['op_pcode'] ."'
					AND op_cancel = 'N' AND op_is_addoption = 'Y' AND op_addoption_parent = '".$res['op_pouid']."'
			");
			// --- JJC : 2023-01-13 : 추가옵션 부분취소 ---
		...
		function partcancel_restore($op_uid) {
			...
			// 환불 신청이 있었을 경우 처리
			$uque = "";
			if( $res['op_complain'] != ''){  $uque = " , op_complain ='교환/반품신청' "; }
