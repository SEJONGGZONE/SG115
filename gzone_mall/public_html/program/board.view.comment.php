<?php
	...
	foreach($resComment as $k=>$v){
		...
		$listComment[$k]['inid'] = $v['bt_inid'];
		$listComment[$k]['writer'] = $boardInfo['bi_writer_view_use'] == 'Y' || $v['bt_inid'] == get_userid()? $v['bt_writer'] : LastCut($v['bt_writer'],1); // 글쓴이
		$listComment[$k]['deleteAuth'] = $v['bt_inid'] == get_userid() || is_admin($ONLY_USER) === true ? true : false;
