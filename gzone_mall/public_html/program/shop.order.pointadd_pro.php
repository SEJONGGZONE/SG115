...
if($osr['o_paymethod'] == 'online' && $osr['o_get_tax'] == 'Y'){
	...
	foreach($pr as $sk=>$sv) {
		...
		// 과세
		if($sv['op_vat'] == 'Y') {
			// LCY : 2023-07-18 : 무통장 현금영수증 패치 -- 쿠폰 할인금액 추가
			// $data2['vatY'] += $sv['op_price'] * $sv['op_cnt'] - $sv['op_usepoint']; 
			$data2['vatY'] += $sv['op_price'] * $sv['op_cnt'] - $sv['op_usepoint'] - $sv['op_use_discount_price'] - $sv['op_use_product_coupon'];
		}
		// 면세
		else if($sv['op_vat'] == 'N') {
			// LCY : 2023-07-18 : 무통장 현금영수증 패치 -- 쿠폰 할인금액 추가
			// $data2['vatN'] += $sv['op_price'] * $sv['op_cnt'] - $sv['op_usepoint'];
			$data2['vatN'] += $sv['op_price'] * $sv['op_cnt'] - $sv['op_usepoint'] - $sv['op_use_discount_price'] - $sv['op_use_product_coupon'];
		}
