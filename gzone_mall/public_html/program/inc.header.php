...
if(isset($_GET['pcode']) && $_GET['pn'] == 'product.view') {
	...
	if($plr['cnt'] == 0){
		_MQ_noreturn(" insert smart_product_latest set pl_pcode='{$pcode}', pl_uniqkey='{$appAuthSDProductLatest}', pl_rdate=now()  ");
	}else{

		_MQ_noreturn(" 
		update smart_product_latest set pl_rdate=now() where pl_pcode = '{$pcode}' and pl_uniqkey='{$appAuthSDProductLatest}'
		");
	}
	// - 최근 본 상품 업데이트 ---
}
...
$og_title = htmlentities(addslashes($siteInfo['s_glbtlt']));  // 사이트명
$og_description = htmlentities(str_replace(array('/', '\\', '"', "'"), '', $siteInfo['s_glbdsc'])); // 사이트 설명
$og_url = $canonical_url;  // 사이트 주소
$og_site_name = htmlentities(addslashes($siteInfo['s_glbtlt'])); // 사이트명
