...
switch($_mode) {
	...
	case 'find_pw':
		...
		_MQ_noreturn(" update smart_individual set in_pw = '".db_password($tmp_pw)."'  where in_id = '{$r['in_id']}'");
		...
		if($n_type != 'email') { // 임시비밀번호 SMS발송
			...
			// 앱 이용시 푸시 발송 구간 LCY : {하이앱}
			$push_send_rst = false;
			if( function_exists('shop_etc_push_send') == true){
				$push_send_rst = shop_etc_push_send('temp_password',array('in_name'=>$r['in_name'], 'tmp_pw'=>$tmp_pw));
			}
			if( $push_send_rst !== true){
