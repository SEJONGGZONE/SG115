...
if($osr['o_price_usepoint']) {// JJC : 부분취소 개선 : 2021-02-10
	...
	// 총구매상품가격
	$opres = _MQ("select IFNULL(sum(op_price * op_cnt + op_delivery_price + op_add_delivery_price),0) as op_sum from smart_order_product where op_oordernum = '". $_ordernum ."' AND op_is_addoption = 'N' "); // JJC : 2023-01-13 : 추가옵션 부분취소
	$app_opsum = $opres['op_sum'];

	// --- 사용포인트적용 - 개별적용 ---
	$sum_usepoint = 0;
	$opres = _MQ_assoc("SELECT op_uid , (op_price * op_cnt + op_delivery_price + op_add_delivery_price) AS app_opsum FROM smart_order_product WHERE op_oordernum = '". $_ordernum ."' AND op_is_addoption = 'N' "); // JJC : 부분취소 개선 : 2021-02-10// JJC : 2023-01-13 : 추가옵션 부분취소
	...
	if( $osr['o_price_usepoint']  <> $sum_usepoint ) {
		$opres = _MQ("select op_uid from smart_order_product where op_oordernum = '". $_ordernum ."' AND op_is_addoption = 'N' order by op_usepoint desc limit 0 , 1 ");// JJC : 2023-01-13 : 추가옵션 부분취소
		if(sizeof($opres) > 0 ) {
...
if( $order_discount > 0 ) {
	...
	if( $_use_discount_price  <> $sum_use_discount_price ) {
		$opres = _MQ("select op_uid from smart_order_product where op_oordernum = '". $_ordernum ."' AND op_is_addoption = 'N' order by op_use_discount_price desc limit 0 , 1  ");// JJC : 2023-01-13 : 추가옵션 부분취소
		if(sizeof($opres) > 0 ) {
