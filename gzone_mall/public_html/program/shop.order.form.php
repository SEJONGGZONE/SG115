...
update_cart_update_check(); // 카트의 가격 정보를 업데이트 한다. 
clean_cart(); // 장바구니 판매불가 상품 삭제



// shop.cart.inc.php -> $order_type 을 초기화
...
if(function_exists('is_app') == true && $AppUseMode === true && is_app() === true){
    // 앱 추가할인 설정 사용 시
    if($siteInfo['as_app_sale_use'] == 'Y'){

        // LCY : 2023-10-05 : 구매가능한 총주문상품 계산식 수정
        $app_total_price = _MQ_result(" select ifnull(sum(c_price*c_cnt) , 0) as sum_c_price from smart_cart where c_cookie='".$_COOKIE["AuthShopCOOKIEID"]."' and c_direct = 'Y' ");
