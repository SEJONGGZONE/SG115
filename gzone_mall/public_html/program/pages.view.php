...
if($current_menu['np_view'] == "N") error_loc_msg("/","비공개된 페이지입니다.");
...
if($type == 'agree') {
	...
	*/
	if($data == 'guide') { // 이용안내
		...
		$page_content['content'] = $siteInfo['s_information_use_mobile'];
	}
	else if($data == 'agree') { // 이용약관

		$AgreeInfo = arr_policy($data);
		$page_content['title'] = '이용약관';
		$page_content['content'] = $AgreeInfo[$data.'_html_m']['po_content'];
	}
		...
		if($page_row['np_uid'] && $page_row['np_view'] == 'Y') { // 페이지가 노출 상태로 있는 경우만 추가
			...
			$page_content['content'] = $page_row['np_content'];
		}
	}
