...
$sns_short = array('nv_', 'fb_', 'ko_','go_');

if(isset($AppUseMode) && $AppUseMode === true){
	$sns_short[] = 'ap_';
}
...
if(count($SNSField) > 0) {
	...
	if(str_replace($sns_short, '', $mem_info['in_id'])) {
	...
	foreach($sns_channel as $k=>$v) {
		...
		if($mem_info[$SNSField[$k]['join']] == 'Y' && $mem_info[$SNSField[$k]['id']]) { // 정보 수정 과는 반대로 조건이 걸린다.
			...
			if( function_exists('is_app') && is_app() === true && $k != 'apple'){
                $app_info = app_info();
                $schema = $app_info['scheme'];
                $$v = $schema.'://action?mode='.$k.'_login';
			    $SNSField[$k]['callback_url'] = $schema.'://action?mode='.$k.'_login&SNSOauthMode=leave';
			}else{
