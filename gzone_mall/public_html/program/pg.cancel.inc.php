            ...
            // JJC : 간편결제 - 페이플 : 2021-06-05
				...
				switch($siteInfo[s_pg_type]) {
					...
					case "inicis" :
						...
						require(OD_PROGRAM_ROOT."/pg.cancle_inicis_api.php");
						// LCY : 2022-12-28 : 이니시스TX모듈 교체 -- 결제수단 추가 }
					break;
					case "allthegate" :
			...
			if ($is_pg_status) {	// pg모듈 호출 상태
				...
				if( $osr['o_canceled'] <> "Y" ){
					...
					if( in_array($osr['o_order_type'], array('both','ticket')) > 0 ){
						include(OD_PROGRAM_ROOT."/shop.order.ticketdel_pro.php");
					}
				...
				$cancel_status = "N"; // 실패처리
                $cancel_msg = $rResMsg ? iconv('euc-kr','utf-8',$rResMsg) : ($cancel_msg ? $cancel_msg : '결제취소 시 오류가 발생하였습니다.');
			...
			if( $osr['o_canceled'] <> "Y" ){
				if($osr['o_apply_point'] == "Y") {
                    ...
                    // LCY : 2024-01-18 : 무통장/가상계좌 재고차감 시점 변경 패치 {
                        // // 무통장/가상계좌는 결제완료 여부와 상관없이 제고가 차감되어야함 (주석)   
                        // // 상품 재고 증가 및 판매량 차감
                        // $_ordernum = $_ordernum;
                        // include(OD_PROGRAM_ROOT."/shop.order.salecntdel_pro.php");
                    // LCY : 2024-01-18 : 무통장/가상계좌 재고차감 시점 변경 패치 }
