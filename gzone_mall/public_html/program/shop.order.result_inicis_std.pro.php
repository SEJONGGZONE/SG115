...
// LCY : 2023-01-04 : 성공/실패에 따른 URL 정의 (주문서를 다시 작성해 주세요)
      ...
      if($access_result <> true){ // 보안코드값 없이 왔다면
     	...
     	*/
     	...
     	if (isset($resultMap["payMethod"]) && strcmp("VBank", $resultMap["payMethod"]) == 0) { //가상계좌 => 무통장
				...
				// 결제완료페이지 이동
				error_loc($response_location_url['success']."&ordernum=".$ordernum);
				...
				// 결제실패 처리
					...
					error_loc_msg($response_location_url['fail'] , $resultMap["resultMsg"]." - 결제에 실패하였습니다. 다시한번 확인 바랍니다.(주문서를 다시 작성해 주세요)");
                ...
                if ($httpUtil->processHTTP($netCancel, $authMap)) {
                    ...
                    $netcancelResultString = $httpUtil->body;
					 ...
					 error_loc_msg($response_location_url['fail'] , $s."(주문서를 다시 작성해 주세요)" );
 		 ...
 		 error_loc_msg($response_location_url['fail'] , '결제요청에 실패하였습니다. ['.$_REQUEST["resultCode"].'](주문서를 다시 작성해 주세요)');
 	}
}catch (Exception $e) {
        $s = $e->getMessage() . ' (오류코드:' . $e->getCode() . ')';
        error_loc_msg($response_location_url['fail'] , $s );
}
