...
// SSJ : 상품리뷰 작성제한 기능강화 : 2020-08-13 : 상품리뷰 작성전 관리자 설정에 따른 안내문구 노출
$trigger_eval_msg = ''; // 상품리뷰 작성제한 시 문구
// 포토후기 작성횟수 추출
$talk_type = 'eval'; // 상품리뷰
$que = "
    select count(*) as cnt
...
if($siteInfo['s_producteval_limit']<>'N'){
    ...
    if($or['cnt'] < 1){
        $trigger_eval_msg = '상품리뷰는 상품을 구매한 회원만 작성 가능합니다.';
    }
    else if($siteInfo['s_producteval_limit']=='B'){
        if($or['cnt'] <= $er['cnt']){
            $trigger_eval_msg = '상품리뷰는 상품을 구매한 횟수 만큼만 등록할 수 있습니다.';
        }
    }
}
// SSJ : 상품리뷰 작성제한 기능강화 : 2020-08-13 : 상품리뷰 작성전 관리자 설정에 따른 안내문구 노출
