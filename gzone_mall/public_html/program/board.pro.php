...
// -- 관리자를 위한 처리
if( is_admin($ONLY_USER) === true && is_login() !== true){
...
if( in_array( $_mode , array("add" , "modify" , "reply") ) ){
	...
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 파일 업로드 {
	if( count($_FILES['addFile']['name']) > 0){
		foreach($_FILES['addFile']['name'] as $k=>$v){
			if( empty($v) ) { continue; }
			$app_ext = end(explode(".",$v));
			if( !preg_match("/".implode("|",$arrUpfileConfig['ext']['all'])."/i" , $app_ext)   ){ 
				error_alt("첨부하신 `".$v."` 파일은 등록이 불가능합니다."); 
			}
		}
	}
	// LCY : 2022-09-22 : 한국진흥원 보안패치 적용 -- 파일 업로드 }
	...
	$boardFormData['noticeUse'] =  is_admin($ONLY_USER) === true  ? true : false; // 공지사항 사용여부 (관리자 일시에만 적용가능)
	$boardFormData['passwdUse'] =  is_admin($ONLY_USER) !== true && is_login() !== true   ? true : false; // 비밀번호 입력 사용유무 (회원이 아니거나 관리자가 아닐 시 적용)
	...
	if( $boardInfo['bi_recaptcha_use'] === 'Y' && ($siteInfo['recaptcha_api'] != '' && $siteInfo['recaptcha_secret'] != '') ){
		...
		if( is_admin($ONLY_USER) === true) {  $boardFormData['recaptchaUse'] = false; }
	}else{
	...
	$sque .= " ,b_content = '".strip_tags(addslashes($_content))."' ";
...
switch( $_mode ){
	...
	case "add":
		...
		$_writer_type = $boardFormData['writeType'] =  is_admin($ONLY_USER) === true ? 'admin' : (is_login() === true ? 'member':'guest'); // 등록회원 타입
		...
		$sque .= " , b_passwd = '".db_password($_passwd)."' ";
		$sque .= " , b_rdate = now() ";
	...
	case "modify":
		...
		if( is_admin($ONLY_USER) !== true){
			if( $postInfo['b_writer_type'] == 'member'){ // 회원이 남긴 글이라면
				...
				$chk_passwd = db_password($_passwd);
