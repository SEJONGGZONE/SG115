<?php
	...
	if( $osr['o_memtype']=="Y" && $osr['o_apply_point'] == "Y" ) { // 연동처리 된 상태라면..
		...
		// LCY : 2023-03-15 : 첫구매/구매 시 받은 쿠폰은 사용이 안되었을 시 해당 주문건 조회하여 삭제처리한다. {
		$resCoupon = _MQ_assoc("select * from smart_individual_coupon where coup_use = 'N' and coup_checksum = '".$_ordernum."' "); // 주문번호를 통해 받은 쿠폰이고, 사용이 안되었을 시

		foreach($resCoupon as $sk=>$sv){
			if($sv['coup_ocsinfo'] == ''){ continue; }
			$resInfo = unserialize($sv['coup_ocsinfo']);

			// 데이터가 없을 경우 
			if( empty($resInfo['ocs_uid'])){ continue; }

			// 첫구매, 구매 쿠폰이 아닐 경우
			if( in_array($resInfo['ocs_issued_type_auto'], array('1','2')) < 1){ continue; }

			// 미사용 상태이기때문에 삭제처리한다.
			_MQ_noreturn("delete from smart_individual_coupon where coup_uid = '".$sv['coup_uid']."' ");	
		}
		// LCY : 2023-03-15 : 첫구매/구매 시 받은 쿠폰은 사용이 안되었을 시 해당 주문건 조회하여 삭제처리한다. }
