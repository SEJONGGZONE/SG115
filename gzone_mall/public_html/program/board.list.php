...
if($boardInfo['bi_view'] == "N") error_loc_msg("/","사용할 수 없는 게시판입니다."); 
...
if($_GET['searchWord']) {
	$s_query .= "and (replace(b_title,' ','') like '%".trim($searchWord)."%' or replace(b_content,' ','') like '%".trim($searchWord)."%') and ( IF(b_secret = 'Y' and b_writer_type = 'guest', 1 , 0) = 0 ) ";
...
// -- event 게시판일경우 마감은 뒤로 이벤트 진행순으로 정렬
if( in_array($boardInfo['bi_uid'], array('event')) == true){
	$ev_if .=" IF( b_edate >= CURDATE( ) , 1, 2 ) AS app_sort , ";
	$ev_query .=" ,app_sort asc";
}
...
$res = _MQ_assoc("
	...
	ORDER BY b_notice='Y' desc {$ev_query} , b_mdate desc , b_orderuid desc , b_depth asc
...
foreach($res as $k=>$v){
	...
	$listPost[$k]['content'] = strip_tags($v['b_content']); // 게시물 내용
	$listPost[$k]['rdate'] = date('Y-m-d',strtotime($v['b_rdate'])); // 작성일
	...
	// -- event
	$listPost[$k]['eventDate'] = date('Y.m.d',strtotime($v['b_sdate'])).' ~ '.date('Y.m.d',strtotime($v['b_edate']));
	if( $v['b_edate'] < date('Y-m-d') ){
		...
		if($v['b_sdate'] <= date('Y-m-d') && $v['b_edate'] >= date('Y-m-d')){ // 진행중
			...
			$listPost[$k]['eventStatusName'] = '진행전';
			$listPost[$k]['eventStatusVal'] = 'wait';
			$tempTime = strtotime($v['b_sdate'])-time();
	...
	// -- 일반형, 공지형, Q&A 게시판 공지글일 경우 if_notice 클래스 추가
	if( in_array($boardInfo['bi_list_type'], array('basic','notice','qna')) == true){
		if( $v['b_notice'] == 'Y'){ $listPost[$k]['noticeClass'] = 'if_notice'; } // 공지일경우
	}
