<?php
	...
	$brand_prefix = $brand_prefix == '' ? 'all':$brand_prefix; 
	...
	if( $brand_prefix ) {
		if($uid!=''){
			...
			if($brand_prefix!='all'){
				// 해당 초성의 전체인경우 
				$arr_brand_info = _MQ_assoc("select c_uid from smart_brand where c_view = 'Y' and c_prefix_str ='".$brand_prefix."' order by c_name asc ");
				foreach($arr_brand_info as $abk=>$abv){
					$arr_brand_uid[] = $abv['c_uid'];
				}
				$arr_brand = '"'.implode('","',$arr_brand_uid).'"';
				$s_query = " where  p_brand in (". $arr_brand .") and p_view = 'Y' ";
			}else{
				// 해당 초성의 전체인경우 
				$arr_brand_info = _MQ_assoc("select c_uid from smart_brand where c_view = 'Y' order by c_name asc ");
				foreach($arr_brand_info as $abk=>$abv){
					$arr_brand_uid[] = $abv['c_uid'];
				}
				$arr_brand = '"'.implode('","',$arr_brand_uid).'"';
		...
		$listmaxcount = $siteInfo['s_brand_limit'];
