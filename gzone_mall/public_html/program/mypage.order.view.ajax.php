	...
	// ---------- JJC : 부분취소 개선 : 2021-02-10  ----------
		...
		switch($mode) {
			...
			case 'view': // 상품정보
				...
				$_addoption_price = $_addoption_cnt = $_addoption_name = "";
				$arr_addoption = array();
				$aores = _MQ_assoc("
					SELECT
					...
					WHERE (1)
						AND op_oordernum = '".addslashes($ordernum)."' AND op_pcode='". $res['p_code'] ."'
				...
				$__result_array = array(
					...
					'pro_cnt' => number_format($res['op_cnt']),
			...
			case 'cancel':
				...
				if($return != ""){ $__result = 'FAIL'; $__result_text = $return; }//실패처리
					...
					$arr_sms_replace = array('{주문번호}'=>$ordernum, '{주문상품명}'=>$sms_pname, '{comid}'=>$chk['op_partnerCode']); // LCY : 2023-05-31 : 입점업체 SMS추가 -- comid 추가
