...
if($p_option_type_chk == 'nooption'){ // 옵션이 없을 경우
	...
	if(is_login() == true && $pinfo['p_groupset_use'] == 'Y'  ){ // 로그인 중이고 등급할인혜택 적용이 Y 라면
		...
		$c_old_point = ( ($c_old_price*$op_cnt_res)*($pinfo[p_point_per]/100) );
		//$c_price = $c_old_price-getGroupSetPer( $c_old_price,'price',$pinfo['p_code']);
		//$c_point = $c_old_point + getGroupSetPer( ($c_old_price*$op_cnt_res),'point',$pinfo['p_code']);
...
foreach($res_que as $k=>$v){
	...
	if(is_login() == true && $pinfo['p_groupset_use'] == 'Y'  ){ // 로그인 중이고 등급할인혜택 적용이 Y 라면
		...
		$c_point = $c_old_point + getGroupSetPer( ($c_old_price*$op_cnt_res),'point',$code);
		$c_groupset_price_per = $groupSetInfo['mgs_sale_price_per'] > 0 ? $groupSetInfo['mgs_sale_price_per'] : 0;
		$c_groupset_point_per = $groupSetInfo['mgs_give_point_per'] > 0 ? $groupSetInfo['mgs_give_point_per'] : 0;
	}else{
		$c_old_price = $arr_option_data[$v[op_pouid]]['option_price']; // 기존금액
		$c_old_point = ( ($c_old_price*$op_cnt_res)*($pinfo[p_point_per]/100) );  // 기존금액
		$c_price = $c_old_price;
		$c_point = $c_old_point;
		$c_groupset_price_per = 0;
		$c_groupset_point_per = 0;
	...
	$sque = "
		insert smart_cart set
			c_pcode          = '". $code ."'
			, c_option1      = '". addslashes($arr_option_data[$v['op_pouid']]['option_name1'])."'
			, c_option2      = '". addslashes($arr_option_data[$v['op_pouid']]['option_name2'])."'
			, c_option3      = '". addslashes($arr_option_data[$v['op_pouid']]['option_name3'])."'
			, c_cnt          = '".$op_cnt_res."'
			, c_pouid        = '".$v['op_pouid']."'
			, c_cookie       = '".$_COOKIE["AuthShopCOOKIEID"]."'
			, c_rdate        = now()
