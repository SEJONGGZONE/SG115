					...
					if($app_log_chk){
						...
						if( $info['ata_coupon'] > 0 ) {
							...
							if($couponSetData['ocs_uid'] == $info['ata_coupon'] && $couponSetIssuedChk == true){
								...
								$coupon_title = ''. $arrCouponSet['ocs_type_view'][$couponSetData['ocs_type']] .' ' . trim(stripslashes($couponSetData['ocs_name'])) . ' - ' . printCouponSetBoon($couponSetData);
								$coupon_title = addslashes($coupon_title);
