...
if(isset($cuid) && $cuid != '') {
	$category_info = info_category($cuid);
	if( $category_info['c_view'] != 'Y'){  error_loc_msg("/","비공개된 카테고리입니다."); }
}
...
if(isset($_event) && $_event != '') {
	...
	if($_event == 'main_category_best') { // 메인 카테고리 베스트
		...
		// 메인 카테고리 베스트 정렬 순서
		if($siteInfo['s_main_best_order']=='date'){	// 등록순 // 가장 먼저 등록한 상품이 먼저 노출
			$_order='';
			$order_field = 'p_rdate asc, p_idx ';
			$order_sort = 'asc';
		}else if($siteInfo['s_main_best_order']=='recent'){	// 최신순 // 가장 최근에 등록한 상품이 먼저 노출
			$_order = 'date';
		}else if($siteInfo['s_main_best_order']=='sales'){	// 판매순
			$_order = 'sale';
		}else{
			$_order='';
		...
		$listmaxcount = $siteInfo['s_main_best_limit'];
	}
		...
		*/
		...
		// 타입별 이벤트 페이지별 노출개수 추출
		$EventProCnt = $TopEventInfo['dts_list_limit'];

		if($EventProCnt>0){
			$listmaxcount = $EventProCnt;
		}else{
			$listmaxcount = '9999';
		}
	...
	else if($_event == 'main_timesale') { // 메인 타임세일
		// 메인타임세일은 종료된 타임세일 노출안되도록 and concat(p_time_sale_edate,' ', p_time_sale_eclock) >= now() 
		$s_query .= " and p_time_sale = 'Y'  ";
		$query_from='';
		$timesale_if =" ,if( p_time_sale_edate >= CURDATE( ) , 1, 2 ) AS app_timesale_sort  ";

		// 메인 카테고리 베스트 정렬 순서
		if($siteInfo['s_main_timesale_order']=='edate_asc'){	// 종료 빠른순
			$_order='';
			$order_field = 'app_timesale_sort asc, p_time_sale_edate asc, p_idx ';
			$order_sort = 'asc';
		}else if($siteInfo['s_main_timesale_order']=='edate_desc'){	// 최신순 // 가장 최근에 등록한 상품이 먼저 노출
			$_order='';
			$order_field = 'app_timesale_sort asc, p_time_sale_edate desc, p_idx ';
			$order_sort = 'asc';
		}else{
			$_order='';
			$order_field = 'app_timesale_sort asc, p.p_idx ';
			$order_sort = 'asc';
		}
		$listmaxcount = $listmaxcount;
	}
	else if($_event == 'product_timesale') { // 메인 타임세일
		$s_query .= " and p_time_sale = 'Y'  ";
		$query_from='';
		$order_field = 'p.p_idx ';
		$order_sort = 'asc';
	...
	if(isset($cuid) && $cuid != '') { // 카테고리 검색
		...
		// 1차카테고리 정보 추출
		$cate_1depth_info = info_category($category_info['depth1_cuid']);
		// 페이지당 카테고리 상품 개수 노출
		$listmaxcount = $cate_1depth_info['c_list_limit'];
	}
}
...
// 정렬방식
switch($_order) {
	case 'price_asc': $s_order = ' order by p_price asc, p_idx asc , p_rdate desc '; break; // 가격 낮은순
	case 'price_desc': $s_order = ' order by p_price desc, p_idx asc, p_rdate desc '; break; // 가격 높은순
	case 'pname': $s_order = ' order by p_name asc, p_idx asc, p_rdate desc '; break; // 상품이름순(abc..)
	case 'date': $s_order = ' order by p_rdate desc, p_idx asc  '; break; // 등록일 순
	case 'sale': $s_order = ' order by p_salecnt desc, p_idx asc, p_rdate desc '; break; // 판매순(인기순)
	default: $s_order = ' order by p_idx asc, p_rdate desc '; break; // 기본 설정 정렬 순
}
...
// 데이터 조회 및 페이지 계산
$listmaxcount = (isset($listmaxcount) && $listmaxcount > 0 ?$listmaxcount:20); // 페이지당 갯수
...
$res = _MQ_assoc("
    select
		...
		".(isset($timesale_if) && $timesale_if != ''?$timesale_if:'')."
...
// 카테고리 설정 숨김인경우
$category_info['c_list_product_view'] = $category_info['c_list_product_mobile_view'];
...
$freedelivery_icon_src = $_tmp_arr['mo'];
...
$promotion_icon_src = $_tmp_arr['mo'];
