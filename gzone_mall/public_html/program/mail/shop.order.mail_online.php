	...
	foreach( $group_opr as $key=>$opv ){
		...
		foreach($arrOrderPrdocutOption[$opv['p_code']] as  $sk => $sv){
			if( $sv['op_pouid'] > 0){
				...
				$arrOptionInfo['optionName'][] = ($sv['op_is_addoption'] == 'Y' ? '추가 : ':'선택 : ' ).implode(" / ",$arr_option_name)." ".number_format($sv['op_price'])."원 x ".$sv['op_cnt'];
			}
		...
		<tr>
			<td style='padding-top:10px;'>
				<table style='width:100%; border-spacing:0;font-size:inherit; color:inherit'>
					...
					<tbody>
						<tr>
							<td style='padding-top:5px;'>사용자 이름</td>
							<td style='padding-top:5px;'>".$arrMailItem['userName']."</td>
						</tr>
						<tr>
							<td style='padding-top:5px;'>사용자 휴대폰</td>
							<td style='padding-top:5px;'>".$arrMailItem['userHp']."</td>
						</tr>
						<tr>
							<td style='padding-top:5px;'>사용자 이메일</td>
...
$mailing_app_content = "
		...
		<table style='width:100%;border-spacing:0;font-size:inherit; color:inherit'>
			<tbody>
				<tr>
					<td style='text-align:left; border-bottom:2px solid #000; color:#000; font-weight:700; font-size:2em; padding:20px 0 10px'>결제가 확인되었습니다.</td>
				</tr>
				<tr>
					<td>
						...
						<table style='width:100%; border-spacing:0; margin-top:30px;font-size:inherit; color:inherit'>
							...
							<thead>
								<tr>
									<th colspan='3' style='text-align:left; border-bottom:1px solid #EBEBF2; color:#000; font-size:16px; font-weight:700; padding-bottom:5px'>주문상품</th>
								</tr>
							</thead>
							<tbody>
							".$mailing_order_product_content."
						...
						<table style='width:100%; border-spacing:0; margin-top:30px;font-size:inherit; color:inherit'>
							<thead>
								<tr>
									<th style='text-align:left; border-bottom:1px solid #EBEBF2; color:#000; font-size:16px; font-weight:700; padding-bottom:5px'>결제정보 : ".$arrMailItem['orderPaymethod']."
