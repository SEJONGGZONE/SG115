...
# 데이터 호출
// 노출여부 확인
if($boardInfo['bi_view'] == "N" && is_admin($ONLY_USER) !== true ) error_msg("현재 비공개로 전환된 게시판입니다.");
...
$boardViewData['content'] = nl2br(stripslashes($postInfo['b_content']));
...
if( $boardViewData['replyMode'] === true){
	...
	if( count($rowReply) > 0){
		$boardViewData['replyChk']=true;
		$boardViewData['replyTitle'] = '<span class="name">관리자 답변</span> <span class="date">'.date('Y-m-d',strtotime($rowReply['b_rdate'])).'</span>';
		$boardViewData['replyContent'] = (stripslashes($rowReply['b_content']));

		// -- qna의 답글 권한체크 :: 답글작성자가 본인이 아니고, 관리자가 아닐 시
		if( $rowReply['b_inid'] != get_userid() && is_admin($ONLY_USER) !== true){
			$boardAuthChk['reply'] = false;
		}else{
			$boardViewData['replyType'] = '답변수정';
		}
	}else{
		$boardViewData['replyChk']=false;
		$boardViewData['replyTitle'] = '';
		$boardViewData['replyContent'] = '';
...
$boardViewData['replyType'] = $boardViewData['replyType'] == '' ? '답글' :$boardViewData['replyType'];
