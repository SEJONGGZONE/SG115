		...
		if( $res_cd == "0000" )
		{
			...
			if($use_pay_method == '001000000000') { // 가상계좌
				...
				_MQ_noreturn("
					...
					'$ordernum', '$r[o_mid]', now(), '$tno', '$ool_type', '$app_time', '$amount', '$amount', '$account', '', '$r[o_oname]', '$ool_bank_name_array[$bank_code_purify]', '$bankcode', '$escw_yn', '', '$buyr_tel2', '$depositor'
					)
				");
...
if($oc_res_cnt['cnt'] == 1 ) {
	...
	error_loc_msg($response_location_url['fail'] , "결제에 실패하였습니다. 다시한번 확인 바랍니다.(주문서를 다시 작성해 주세요)");
