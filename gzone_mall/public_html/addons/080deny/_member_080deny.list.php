		...
		<table class="table_form">
			...
			<tbody>
				<tr>
					<th>수신거부 요청번호</th>
					<td>
						<input type="text" name="pass_hp" class="design" style="width:190px" value="<?=$pass_hp?>" placeholder="수신거부 요청번호" />
					</td>
					<th>적용상태</th>
					<td>
	...
	<form name="frm080deny" id="frm080deny" method="post" action="/addons/080deny/_member_080deny.pro.php">
		...
		<div class="list_ctrl">
			<div class="left_box">
				...
				<a href="#none" onclick="ctrlDelete('select'); return false;" class="c_btn h27 line black">선택삭제</a>
				<a href="#none" onclick="ctrlDelete('search'); return false;" class="c_btn h27 line black">검색삭제 (<?=$TotalCount?>)</a>
			</div>
		</div>

		<table class="table_list">
			<colgroup>
				<col width="40"/><col width="70"/><col width="*"/><col width="*"/><col width="160"/><col width="160"/><col width="80"/>
			</colgroup>
			<thead>
				<tr>
				...
				foreach($res as $k=>$v) {
					...
					$printBtn = '
						<div class="lineup-row type_center">
							<a href="#none" onclick="return false;" class="c_btn h22 dark line get-delete"  data-uid="'.$v['md_uid'].'" data-apply = "true">삭제</a>
						</div>
					'; // 관리버튼

					// -- 출력
					echo '<tr>';
					echo "	<td class='this_check'><label class='design'><input type='checkbox' class='js_ck md-uid' name='arrUid[]' value='".$v['md_uid']."'></label></td>";
					echo "	<td class='this_num'>" . $_num ."</td>";
					echo "	<td><span class='hidden_tx'>적용상태</span>".$arrAddonsService['080denyStatus'][$v['md_status']]."</td>";
					echo "	<td><span class='hidden_tx'>수신거부요청번호</span>".$v['md_hp']."</td>";
					echo "	<td><span class='hidden_tx'>거부요청시간</span>".( $v['md_refusal_time'] > 0 ? printDateInfo($v['md_refusal_time'])  : '-')."</td>";
					echo "	<td class='this_date'>".printDateInfo($v['md_rdate'])."</td>";
