...
function foldDaumPostcode() {
    ...
    <?php if( in_array($AdminPath, array('totalAdmin','subAdmin')) < 1){?>
    // $(".post_hide_section").show();
    <?php } ?>
}

// 도로명주소 우편번호 열기
function post_popup_show(_type) {
    ...
    new daum.Postcode({
        oncomplete: function(data) {
            ...
            if( typeof _type == 'undefined' || !_type){ _type = ''; }
            if( _type == 'product_address'){    
                // 우편번호와 주소 정보를 해당 필드에 넣는다.
                document.getElementById("_com_juso").value = fullRoadAddr;

                // iframe을 넣은 element를 안보이게 한다.
                foldDaumPostcode();                
                mapPositionUpdate();

                document.getElementById("_com_juso").focus();
            }
            else{
...
function new_post_view(_type) {
    post_popup_show(_type);
}
</script>
