	...
	function hook_log_id_update($type , $login_id , $login_pw) {
		...
		if( !is_admin()) {
			...
			if( IN_ARRAY($type , array('login' , 'join')) && $login_id && $login_pw){
				$mem_info = _MQ(" select * from smart_individual where in_id = '" . addslashes($login_id) . "' and in_pw = '".db_password($login_pw)."'  ");
			}
			...
			if($mem_info['in_id']) {
				...
				_MQ_noreturn($queCntlogList);
				$app_cnt = _MQ_affected_rows();//최근 MySQL 작업으로 변경된 행 개수를 얻음
