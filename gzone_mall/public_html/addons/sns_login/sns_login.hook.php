...
$SNSField = array(
	...
	'apple'=>array(
		'name'=>'애플',
		'config_use'=>'apple_login_use',
		'config_key'=>'apple_clientid',
		'config_secret'=>'nope',
		'join'=>'ap_join',
		'id'=>'ap_encid',
		'sub'=>'ap_sub', // 애플 고유키이지만 너무 길어서 따로 처리
		'short'=>'ap_'
	), // 
	// KAY : 2023-11-06 :: 구글 로그인 추가
	'google'=>array(
		'name'=>'구글',
		'config_use'=>'s_google_login_use',
		'config_key'=>'s_google_client_id',
		'config_secret'=>'s_google_client_pw',
		'join'=>'go_join',
		'id'=>'go_encid',
		'short'=>'go_'
	), // 구글 관련 데이터 필드
...
function SNSAutoJoin($sns_info) {
	...
	if( mb_strlen($join_id) > 10 ){ $join_id = creat_num_uniqueID($SNSField[$sns_info['type']]['short']); }


	if($sns_info['type']=='google'){
		$FindID = _MQ(" select * from smart_individual where go_encid = '".$sns_info['id']."' ");
	}else{
	...
	$join_name = $sns_info['name'];
	
	// KAY : 2023-11-06 :: 구글 로그인 추가
	$join_tel = $sns_info['tel']!=''?$sns_info['tel']:'';
	...
	if($siteInfo['join_approve'] == 'N') { // 승인후 로그인 처리
		 error_loc_msgPopup('/','소셜로그인 가입이 환영합니다.\\n로그인은 관리자 승인 후 가능합니다.');
	}
	else {
		...
		$_rurl = '/?pn=member.join.complete';
		if( function_exists('is_app') && function_exists('hook_app_token') && is_app() === true){
            echo "<script language='javascript'>alert('소셜로그인 가입이 완료되었습니다.\\n정보수정에서 부가정보를 입력해주세요.');</script>";
            hook_app_token($join_id);
		}
		else{
			error_loc_msgPopup($_rurl,'소셜로그인 가입이 완료되었습니다.\\n정보수정에서 부가정보를 입력해주세요.');
		}
	}
}
...
function SNSAutoLeave($mem_info) {
	...
	die("<script language='javascript'>if(opener&&!opener.closed){opener.location.href=('/');alert('정상적으로 탈퇴처리하였습니다.\\n\\n그동안 이용해 주셔서 감사합니다.');window.close();}else{document.location.href=('/');alert('정상적으로 탈퇴처리하였습니다.\\n\\n그동안 이용해 주셔서 감사합니다.');}</script>");
